...
namespace SlaveMatrix.GameClasses
{
    public class Ele : IDisposable
    {
        ...
        {
            foreach (var item in Body.EnumAllPar().Zip(e.Body.EnumAllPar(), static (a0, a1) => new { a0, a1 }))
            {
        ...
        {
            Ele[] array = (Ele[])ThisType.GetField(接続情報.ToString()[(ThisType.Name.Length + 1)..]).GetValue(this);
            if (array != null)
            {
                Ele[] array2 = array;
        ...
        public ColorP GetParOfColorP(Par p)
        {
            return (from e in ThisType.GetFields()
                    ...
                    select e.GetValue(this)).Cast<ColorP>().FirstOrDefault(e => e.Par == p);
        }
    }
}
