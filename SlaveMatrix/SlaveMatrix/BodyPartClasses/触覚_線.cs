using System.Collections.Generic;
using _2DGAMELIB;

namespace SlaveMatrix
{
    public class 触覚_線 : 触覚
    {
    	public Par X0Y0_節1;

    	public Par X0Y0_節2;

    	public Par X0Y0_節3;

    	public Par X0Y0_節4;

    	public Par X0Y0_節5;

    	public Par X0Y0_節6;

    	public Par X0Y0_節7;

    	public Par X0Y0_節8;

    	public Par X0Y0_節9;

    	public Par X0Y0_節10;

    	public Par X0Y0_節11;

    	public Par X0Y0_節12;

    	public Par X0Y0_節13;

    	public Par X0Y0_節14;

    	public Par X0Y0_節15;

    	public Par X0Y0_節16;

    	public Par X0Y0_節17;

    	public Par X0Y0_節18;

    	public Par X0Y0_節19;

    	public Par X0Y0_節20;

    	public Par X0Y0_節21;

    	public Par X0Y0_節22;

    	public Par X0Y0_節23;

    	public Par X0Y0_節24;

    	public Par X0Y0_節25;

    	public Par X0Y0_節26;

    	public Par X0Y0_節27;

    	public Par X0Y0_節28;

    	public Par X0Y0_節29;

    	public Par X0Y0_節30;

    	public Par X0Y0_節31;

    	public Par X0Y0_節32;

    	public Par X0Y0_節33;

    	public Par X0Y0_節34;

    	public Par X0Y0_節35;

    	public Par X0Y0_節36;

    	public Par X0Y0_節37;

    	public Par X0Y0_節38;

    	public Par X0Y0_節39;

    	public Par X0Y0_節40;

    	public Par X0Y0_節41;

    	public Par X0Y0_節42;

    	public Par X0Y0_節43;

    	public Par X0Y0_節44;

    	public Par X0Y0_節45;

    	public Par X0Y0_節46;

    	public Par X0Y0_節47;

    	public Par X0Y0_節48;

    	public Par X0Y0_節49;

    	public Par X0Y0_節50;

    	public Par X0Y0_節51;

    	public Par X0Y0_節52;

    	public Par X0Y0_節53;

    	public Par X0Y0_節54;

    	public Par X0Y0_節55;

    	public Par X0Y0_節56;

    	public Par X0Y0_節57;

    	public Par X0Y0_節58;

    	public Par X0Y0_節59;

    	public Par X0Y0_節60;

    	public ColorD 節1CD;

    	public ColorD 節2CD;

    	public ColorD 節3CD;

    	public ColorD 節4CD;

    	public ColorD 節5CD;

    	public ColorD 節6CD;

    	public ColorD 節7CD;

    	public ColorD 節8CD;

    	public ColorD 節9CD;

    	public ColorD 節10CD;

    	public ColorD 節11CD;

    	public ColorD 節12CD;

    	public ColorD 節13CD;

    	public ColorD 節14CD;

    	public ColorD 節15CD;

    	public ColorD 節16CD;

    	public ColorD 節17CD;

    	public ColorD 節18CD;

    	public ColorD 節19CD;

    	public ColorD 節20CD;

    	public ColorD 節21CD;

    	public ColorD 節22CD;

    	public ColorD 節23CD;

    	public ColorD 節24CD;

    	public ColorD 節25CD;

    	public ColorD 節26CD;

    	public ColorD 節27CD;

    	public ColorD 節28CD;

    	public ColorD 節29CD;

    	public ColorD 節30CD;

    	public ColorD 節31CD;

    	public ColorD 節32CD;

    	public ColorD 節33CD;

    	public ColorD 節34CD;

    	public ColorD 節35CD;

    	public ColorD 節36CD;

    	public ColorD 節37CD;

    	public ColorD 節38CD;

    	public ColorD 節39CD;

    	public ColorD 節40CD;

    	public ColorD 節41CD;

    	public ColorD 節42CD;

    	public ColorD 節43CD;

    	public ColorD 節44CD;

    	public ColorD 節45CD;

    	public ColorD 節46CD;

    	public ColorD 節47CD;

    	public ColorD 節48CD;

    	public ColorD 節49CD;

    	public ColorD 節50CD;

    	public ColorD 節51CD;

    	public ColorD 節52CD;

    	public ColorD 節53CD;

    	public ColorD 節54CD;

    	public ColorD 節55CD;

    	public ColorD 節56CD;

    	public ColorD 節57CD;

    	public ColorD 節58CD;

    	public ColorD 節59CD;

    	public ColorD 節60CD;

    	public ColorP X0Y0_節1CP;

    	public ColorP X0Y0_節2CP;

    	public ColorP X0Y0_節3CP;

    	public ColorP X0Y0_節4CP;

    	public ColorP X0Y0_節5CP;

    	public ColorP X0Y0_節6CP;

    	public ColorP X0Y0_節7CP;

    	public ColorP X0Y0_節8CP;

    	public ColorP X0Y0_節9CP;

    	public ColorP X0Y0_節10CP;

    	public ColorP X0Y0_節11CP;

    	public ColorP X0Y0_節12CP;

    	public ColorP X0Y0_節13CP;

    	public ColorP X0Y0_節14CP;

    	public ColorP X0Y0_節15CP;

    	public ColorP X0Y0_節16CP;

    	public ColorP X0Y0_節17CP;

    	public ColorP X0Y0_節18CP;

    	public ColorP X0Y0_節19CP;

    	public ColorP X0Y0_節20CP;

    	public ColorP X0Y0_節21CP;

    	public ColorP X0Y0_節22CP;

    	public ColorP X0Y0_節23CP;

    	public ColorP X0Y0_節24CP;

    	public ColorP X0Y0_節25CP;

    	public ColorP X0Y0_節26CP;

    	public ColorP X0Y0_節27CP;

    	public ColorP X0Y0_節28CP;

    	public ColorP X0Y0_節29CP;

    	public ColorP X0Y0_節30CP;

    	public ColorP X0Y0_節31CP;

    	public ColorP X0Y0_節32CP;

    	public ColorP X0Y0_節33CP;

    	public ColorP X0Y0_節34CP;

    	public ColorP X0Y0_節35CP;

    	public ColorP X0Y0_節36CP;

    	public ColorP X0Y0_節37CP;

    	public ColorP X0Y0_節38CP;

    	public ColorP X0Y0_節39CP;

    	public ColorP X0Y0_節40CP;

    	public ColorP X0Y0_節41CP;

    	public ColorP X0Y0_節42CP;

    	public ColorP X0Y0_節43CP;

    	public ColorP X0Y0_節44CP;

    	public ColorP X0Y0_節45CP;

    	public ColorP X0Y0_節46CP;

    	public ColorP X0Y0_節47CP;

    	public ColorP X0Y0_節48CP;

    	public ColorP X0Y0_節49CP;

    	public ColorP X0Y0_節50CP;

    	public ColorP X0Y0_節51CP;

    	public ColorP X0Y0_節52CP;

    	public ColorP X0Y0_節53CP;

    	public ColorP X0Y0_節54CP;

    	public ColorP X0Y0_節55CP;

    	public ColorP X0Y0_節56CP;

    	public ColorP X0Y0_節57CP;

    	public ColorP X0Y0_節58CP;

    	public ColorP X0Y0_節59CP;

    	public ColorP X0Y0_節60CP;

    	public override bool 欠損
    	{
    		get
    		{
    			return 欠損_;
    		}
    		set
    		{
    			欠損_ = value;
    		}
    	}

    	public override bool 筋肉
    	{
    		get
    		{
    			return 筋肉_;
    		}
    		set
    		{
    			筋肉_ = value;
    		}
    	}

    	public override bool 拘束
    	{
    		get
    		{
    			return 拘束_;
    		}
    		set
    		{
    			拘束_ = value;
    		}
    	}

    	public bool 節1_表示
    	{
    		get
    		{
    			return X0Y0_節1.Dra;
    		}
    		set
    		{
    			X0Y0_節1.Dra = value;
    			X0Y0_節1.Hit = value;
    		}
    	}

    	public bool 節2_表示
    	{
    		get
    		{
    			return X0Y0_節2.Dra;
    		}
    		set
    		{
    			X0Y0_節2.Dra = value;
    			X0Y0_節2.Hit = value;
    		}
    	}

    	public bool 節3_表示
    	{
    		get
    		{
    			return X0Y0_節3.Dra;
    		}
    		set
    		{
    			X0Y0_節3.Dra = value;
    			X0Y0_節3.Hit = value;
    		}
    	}

    	public bool 節4_表示
    	{
    		get
    		{
    			return X0Y0_節4.Dra;
    		}
    		set
    		{
    			X0Y0_節4.Dra = value;
    			X0Y0_節4.Hit = value;
    		}
    	}

    	public bool 節5_表示
    	{
    		get
    		{
    			return X0Y0_節5.Dra;
    		}
    		set
    		{
    			X0Y0_節5.Dra = value;
    			X0Y0_節5.Hit = value;
    		}
    	}

    	public bool 節6_表示
    	{
    		get
    		{
    			return X0Y0_節6.Dra;
    		}
    		set
    		{
    			X0Y0_節6.Dra = value;
    			X0Y0_節6.Hit = value;
    		}
    	}

    	public bool 節7_表示
    	{
    		get
    		{
    			return X0Y0_節7.Dra;
    		}
    		set
    		{
    			X0Y0_節7.Dra = value;
    			X0Y0_節7.Hit = value;
    		}
    	}

    	public bool 節8_表示
    	{
    		get
    		{
    			return X0Y0_節8.Dra;
    		}
    		set
    		{
    			X0Y0_節8.Dra = value;
    			X0Y0_節8.Hit = value;
    		}
    	}

    	public bool 節9_表示
    	{
    		get
    		{
    			return X0Y0_節9.Dra;
    		}
    		set
    		{
    			X0Y0_節9.Dra = value;
    			X0Y0_節9.Hit = value;
    		}
    	}

    	public bool 節10_表示
    	{
    		get
    		{
    			return X0Y0_節10.Dra;
    		}
    		set
    		{
    			X0Y0_節10.Dra = value;
    			X0Y0_節10.Hit = value;
    		}
    	}

    	public bool 節11_表示
    	{
    		get
    		{
    			return X0Y0_節11.Dra;
    		}
    		set
    		{
    			X0Y0_節11.Dra = value;
    			X0Y0_節11.Hit = value;
    		}
    	}

    	public bool 節12_表示
    	{
    		get
    		{
    			return X0Y0_節12.Dra;
    		}
    		set
    		{
    			X0Y0_節12.Dra = value;
    			X0Y0_節12.Hit = value;
    		}
    	}

    	public bool 節13_表示
    	{
    		get
    		{
    			return X0Y0_節13.Dra;
    		}
    		set
    		{
    			X0Y0_節13.Dra = value;
    			X0Y0_節13.Hit = value;
    		}
    	}

    	public bool 節14_表示
    	{
    		get
    		{
    			return X0Y0_節14.Dra;
    		}
    		set
    		{
    			X0Y0_節14.Dra = value;
    			X0Y0_節14.Hit = value;
    		}
    	}

    	public bool 節15_表示
    	{
    		get
    		{
    			return X0Y0_節15.Dra;
    		}
    		set
    		{
    			X0Y0_節15.Dra = value;
    			X0Y0_節15.Hit = value;
    		}
    	}

    	public bool 節16_表示
    	{
    		get
    		{
    			return X0Y0_節16.Dra;
    		}
    		set
    		{
    			X0Y0_節16.Dra = value;
    			X0Y0_節16.Hit = value;
    		}
    	}

    	public bool 節17_表示
    	{
    		get
    		{
    			return X0Y0_節17.Dra;
    		}
    		set
    		{
    			X0Y0_節17.Dra = value;
    			X0Y0_節17.Hit = value;
    		}
    	}

    	public bool 節18_表示
    	{
    		get
    		{
    			return X0Y0_節18.Dra;
    		}
    		set
    		{
    			X0Y0_節18.Dra = value;
    			X0Y0_節18.Hit = value;
    		}
    	}

    	public bool 節19_表示
    	{
    		get
    		{
    			return X0Y0_節19.Dra;
    		}
    		set
    		{
    			X0Y0_節19.Dra = value;
    			X0Y0_節19.Hit = value;
    		}
    	}

    	public bool 節20_表示
    	{
    		get
    		{
    			return X0Y0_節20.Dra;
    		}
    		set
    		{
    			X0Y0_節20.Dra = value;
    			X0Y0_節20.Hit = value;
    		}
    	}

    	public bool 節21_表示
    	{
    		get
    		{
    			return X0Y0_節21.Dra;
    		}
    		set
    		{
    			X0Y0_節21.Dra = value;
    			X0Y0_節21.Hit = value;
    		}
    	}

    	public bool 節22_表示
    	{
    		get
    		{
    			return X0Y0_節22.Dra;
    		}
    		set
    		{
    			X0Y0_節22.Dra = value;
    			X0Y0_節22.Hit = value;
    		}
    	}

    	public bool 節23_表示
    	{
    		get
    		{
    			return X0Y0_節23.Dra;
    		}
    		set
    		{
    			X0Y0_節23.Dra = value;
    			X0Y0_節23.Hit = value;
    		}
    	}

    	public bool 節24_表示
    	{
    		get
    		{
    			return X0Y0_節24.Dra;
    		}
    		set
    		{
    			X0Y0_節24.Dra = value;
    			X0Y0_節24.Hit = value;
    		}
    	}

    	public bool 節25_表示
    	{
    		get
    		{
    			return X0Y0_節25.Dra;
    		}
    		set
    		{
    			X0Y0_節25.Dra = value;
    			X0Y0_節25.Hit = value;
    		}
    	}

    	public bool 節26_表示
    	{
    		get
    		{
    			return X0Y0_節26.Dra;
    		}
    		set
    		{
    			X0Y0_節26.Dra = value;
    			X0Y0_節26.Hit = value;
    		}
    	}

    	public bool 節27_表示
    	{
    		get
    		{
    			return X0Y0_節27.Dra;
    		}
    		set
    		{
    			X0Y0_節27.Dra = value;
    			X0Y0_節27.Hit = value;
    		}
    	}

    	public bool 節28_表示
    	{
    		get
    		{
    			return X0Y0_節28.Dra;
    		}
    		set
    		{
    			X0Y0_節28.Dra = value;
    			X0Y0_節28.Hit = value;
    		}
    	}

    	public bool 節29_表示
    	{
    		get
    		{
    			return X0Y0_節29.Dra;
    		}
    		set
    		{
    			X0Y0_節29.Dra = value;
    			X0Y0_節29.Hit = value;
    		}
    	}

    	public bool 節30_表示
    	{
    		get
    		{
    			return X0Y0_節30.Dra;
    		}
    		set
    		{
    			X0Y0_節30.Dra = value;
    			X0Y0_節30.Hit = value;
    		}
    	}

    	public bool 節31_表示
    	{
    		get
    		{
    			return X0Y0_節31.Dra;
    		}
    		set
    		{
    			X0Y0_節31.Dra = value;
    			X0Y0_節31.Hit = value;
    		}
    	}

    	public bool 節32_表示
    	{
    		get
    		{
    			return X0Y0_節32.Dra;
    		}
    		set
    		{
    			X0Y0_節32.Dra = value;
    			X0Y0_節32.Hit = value;
    		}
    	}

    	public bool 節33_表示
    	{
    		get
    		{
    			return X0Y0_節33.Dra;
    		}
    		set
    		{
    			X0Y0_節33.Dra = value;
    			X0Y0_節33.Hit = value;
    		}
    	}

    	public bool 節34_表示
    	{
    		get
    		{
    			return X0Y0_節34.Dra;
    		}
    		set
    		{
    			X0Y0_節34.Dra = value;
    			X0Y0_節34.Hit = value;
    		}
    	}

    	public bool 節35_表示
    	{
    		get
    		{
    			return X0Y0_節35.Dra;
    		}
    		set
    		{
    			X0Y0_節35.Dra = value;
    			X0Y0_節35.Hit = value;
    		}
    	}

    	public bool 節36_表示
    	{
    		get
    		{
    			return X0Y0_節36.Dra;
    		}
    		set
    		{
    			X0Y0_節36.Dra = value;
    			X0Y0_節36.Hit = value;
    		}
    	}

    	public bool 節37_表示
    	{
    		get
    		{
    			return X0Y0_節37.Dra;
    		}
    		set
    		{
    			X0Y0_節37.Dra = value;
    			X0Y0_節37.Hit = value;
    		}
    	}

    	public bool 節38_表示
    	{
    		get
    		{
    			return X0Y0_節38.Dra;
    		}
    		set
    		{
    			X0Y0_節38.Dra = value;
    			X0Y0_節38.Hit = value;
    		}
    	}

    	public bool 節39_表示
    	{
    		get
    		{
    			return X0Y0_節39.Dra;
    		}
    		set
    		{
    			X0Y0_節39.Dra = value;
    			X0Y0_節39.Hit = value;
    		}
    	}

    	public bool 節40_表示
    	{
    		get
    		{
    			return X0Y0_節40.Dra;
    		}
    		set
    		{
    			X0Y0_節40.Dra = value;
    			X0Y0_節40.Hit = value;
    		}
    	}

    	public bool 節41_表示
    	{
    		get
    		{
    			return X0Y0_節41.Dra;
    		}
    		set
    		{
    			X0Y0_節41.Dra = value;
    			X0Y0_節41.Hit = value;
    		}
    	}

    	public bool 節42_表示
    	{
    		get
    		{
    			return X0Y0_節42.Dra;
    		}
    		set
    		{
    			X0Y0_節42.Dra = value;
    			X0Y0_節42.Hit = value;
    		}
    	}

    	public bool 節43_表示
    	{
    		get
    		{
    			return X0Y0_節43.Dra;
    		}
    		set
    		{
    			X0Y0_節43.Dra = value;
    			X0Y0_節43.Hit = value;
    		}
    	}

    	public bool 節44_表示
    	{
    		get
    		{
    			return X0Y0_節44.Dra;
    		}
    		set
    		{
    			X0Y0_節44.Dra = value;
    			X0Y0_節44.Hit = value;
    		}
    	}

    	public bool 節45_表示
    	{
    		get
    		{
    			return X0Y0_節45.Dra;
    		}
    		set
    		{
    			X0Y0_節45.Dra = value;
    			X0Y0_節45.Hit = value;
    		}
    	}

    	public bool 節46_表示
    	{
    		get
    		{
    			return X0Y0_節46.Dra;
    		}
    		set
    		{
    			X0Y0_節46.Dra = value;
    			X0Y0_節46.Hit = value;
    		}
    	}

    	public bool 節47_表示
    	{
    		get
    		{
    			return X0Y0_節47.Dra;
    		}
    		set
    		{
    			X0Y0_節47.Dra = value;
    			X0Y0_節47.Hit = value;
    		}
    	}

    	public bool 節48_表示
    	{
    		get
    		{
    			return X0Y0_節48.Dra;
    		}
    		set
    		{
    			X0Y0_節48.Dra = value;
    			X0Y0_節48.Hit = value;
    		}
    	}

    	public bool 節49_表示
    	{
    		get
    		{
    			return X0Y0_節49.Dra;
    		}
    		set
    		{
    			X0Y0_節49.Dra = value;
    			X0Y0_節49.Hit = value;
    		}
    	}

    	public bool 節50_表示
    	{
    		get
    		{
    			return X0Y0_節50.Dra;
    		}
    		set
    		{
    			X0Y0_節50.Dra = value;
    			X0Y0_節50.Hit = value;
    		}
    	}

    	public bool 節51_表示
    	{
    		get
    		{
    			return X0Y0_節51.Dra;
    		}
    		set
    		{
    			X0Y0_節51.Dra = value;
    			X0Y0_節51.Hit = value;
    		}
    	}

    	public bool 節52_表示
    	{
    		get
    		{
    			return X0Y0_節52.Dra;
    		}
    		set
    		{
    			X0Y0_節52.Dra = value;
    			X0Y0_節52.Hit = value;
    		}
    	}

    	public bool 節53_表示
    	{
    		get
    		{
    			return X0Y0_節53.Dra;
    		}
    		set
    		{
    			X0Y0_節53.Dra = value;
    			X0Y0_節53.Hit = value;
    		}
    	}

    	public bool 節54_表示
    	{
    		get
    		{
    			return X0Y0_節54.Dra;
    		}
    		set
    		{
    			X0Y0_節54.Dra = value;
    			X0Y0_節54.Hit = value;
    		}
    	}

    	public bool 節55_表示
    	{
    		get
    		{
    			return X0Y0_節55.Dra;
    		}
    		set
    		{
    			X0Y0_節55.Dra = value;
    			X0Y0_節55.Hit = value;
    		}
    	}

    	public bool 節56_表示
    	{
    		get
    		{
    			return X0Y0_節56.Dra;
    		}
    		set
    		{
    			X0Y0_節56.Dra = value;
    			X0Y0_節56.Hit = value;
    		}
    	}

    	public bool 節57_表示
    	{
    		get
    		{
    			return X0Y0_節57.Dra;
    		}
    		set
    		{
    			X0Y0_節57.Dra = value;
    			X0Y0_節57.Hit = value;
    		}
    	}

    	public bool 節58_表示
    	{
    		get
    		{
    			return X0Y0_節58.Dra;
    		}
    		set
    		{
    			X0Y0_節58.Dra = value;
    			X0Y0_節58.Hit = value;
    		}
    	}

    	public bool 節59_表示
    	{
    		get
    		{
    			return X0Y0_節59.Dra;
    		}
    		set
    		{
    			X0Y0_節59.Dra = value;
    			X0Y0_節59.Hit = value;
    		}
    	}

    	public bool 節60_表示
    	{
    		get
    		{
    			return X0Y0_節60.Dra;
    		}
    		set
    		{
    			X0Y0_節60.Dra = value;
    			X0Y0_節60.Hit = value;
    		}
    	}

    	public override bool 表示
    	{
    		get
    		{
    			return 節1_表示;
    		}
    		set
    		{
    			節1_表示 = value;
    			節2_表示 = value;
    			節3_表示 = value;
    			節4_表示 = value;
    			節5_表示 = value;
    			節6_表示 = value;
    			節7_表示 = value;
    			節8_表示 = value;
    			節9_表示 = value;
    			節10_表示 = value;
    			節11_表示 = value;
    			節12_表示 = value;
    			節13_表示 = value;
    			節14_表示 = value;
    			節15_表示 = value;
    			節16_表示 = value;
    			節17_表示 = value;
    			節18_表示 = value;
    			節19_表示 = value;
    			節20_表示 = value;
    			節21_表示 = value;
    			節22_表示 = value;
    			節23_表示 = value;
    			節24_表示 = value;
    			節25_表示 = value;
    			節26_表示 = value;
    			節27_表示 = value;
    			節28_表示 = value;
    			節29_表示 = value;
    			節30_表示 = value;
    			節31_表示 = value;
    			節32_表示 = value;
    			節33_表示 = value;
    			節34_表示 = value;
    			節35_表示 = value;
    			節36_表示 = value;
    			節37_表示 = value;
    			節38_表示 = value;
    			節39_表示 = value;
    			節40_表示 = value;
    			節41_表示 = value;
    			節42_表示 = value;
    			節43_表示 = value;
    			節44_表示 = value;
    			節45_表示 = value;
    			節46_表示 = value;
    			節47_表示 = value;
    			節48_表示 = value;
    			節49_表示 = value;
    			節50_表示 = value;
    			節51_表示 = value;
    			節52_表示 = value;
    			節53_表示 = value;
    			節54_表示 = value;
    			節55_表示 = value;
    			節56_表示 = value;
    			節57_表示 = value;
    			節58_表示 = value;
    			節59_表示 = value;
    			節60_表示 = value;
    		}
    	}

    	public override double Intensity
    	{
    		get
    		{
    			return 節1CD.不透明度;
    		}
    		set
    		{
    			節1CD.不透明度 = value;
    			節2CD.不透明度 = value;
    			節3CD.不透明度 = value;
    			節4CD.不透明度 = value;
    			節5CD.不透明度 = value;
    			節6CD.不透明度 = value;
    			節7CD.不透明度 = value;
    			節8CD.不透明度 = value;
    			節9CD.不透明度 = value;
    			節10CD.不透明度 = value;
    			節11CD.不透明度 = value;
    			節12CD.不透明度 = value;
    			節13CD.不透明度 = value;
    			節14CD.不透明度 = value;
    			節15CD.不透明度 = value;
    			節16CD.不透明度 = value;
    			節17CD.不透明度 = value;
    			節18CD.不透明度 = value;
    			節19CD.不透明度 = value;
    			節20CD.不透明度 = value;
    			節21CD.不透明度 = value;
    			節22CD.不透明度 = value;
    			節23CD.不透明度 = value;
    			節24CD.不透明度 = value;
    			節25CD.不透明度 = value;
    			節26CD.不透明度 = value;
    			節27CD.不透明度 = value;
    			節28CD.不透明度 = value;
    			節29CD.不透明度 = value;
    			節30CD.不透明度 = value;
    			節31CD.不透明度 = value;
    			節32CD.不透明度 = value;
    			節33CD.不透明度 = value;
    			節34CD.不透明度 = value;
    			節35CD.不透明度 = value;
    			節36CD.不透明度 = value;
    			節37CD.不透明度 = value;
    			節38CD.不透明度 = value;
    			節39CD.不透明度 = value;
    			節40CD.不透明度 = value;
    			節41CD.不透明度 = value;
    			節42CD.不透明度 = value;
    			節43CD.不透明度 = value;
    			節44CD.不透明度 = value;
    			節45CD.不透明度 = value;
    			節46CD.不透明度 = value;
    			節47CD.不透明度 = value;
    			節48CD.不透明度 = value;
    			節49CD.不透明度 = value;
    			節50CD.不透明度 = value;
    			節51CD.不透明度 = value;
    			節52CD.不透明度 = value;
    			節53CD.不透明度 = value;
    			節54CD.不透明度 = value;
    			節55CD.不透明度 = value;
    			節56CD.不透明度 = value;
    			節57CD.不透明度 = value;
    			節58CD.不透明度 = value;
    			節59CD.不透明度 = value;
    			節60CD.不透明度 = value;
    		}
    	}

    	public 触覚_線(double DisUnit, 配色指定 配色指定, BodyColorSet 体配色, ModeEventDispatcher Med, 触覚_線D e)
    	{
    		ThisType = GetType();
    		Dif dif = new Dif();
    		dif.Tag = "線";
    		dif.Add(new Pars(Sta.肢左["触覚"][0][0]));
    		Body = new Difs();
    		Body.Tag = dif.Tag;
    		Body.Add(dif);
    		Pars pars = Body[0][0];
    		X0Y0_節1 = pars["節1"].ToPar();
    		X0Y0_節2 = pars["節2"].ToPar();
    		X0Y0_節3 = pars["節3"].ToPar();
    		X0Y0_節4 = pars["節4"].ToPar();
    		X0Y0_節5 = pars["節5"].ToPar();
    		X0Y0_節6 = pars["節6"].ToPar();
    		X0Y0_節7 = pars["節7"].ToPar();
    		X0Y0_節8 = pars["節8"].ToPar();
    		X0Y0_節9 = pars["節9"].ToPar();
    		X0Y0_節10 = pars["節10"].ToPar();
    		X0Y0_節11 = pars["節11"].ToPar();
    		X0Y0_節12 = pars["節12"].ToPar();
    		X0Y0_節13 = pars["節13"].ToPar();
    		X0Y0_節14 = pars["節14"].ToPar();
    		X0Y0_節15 = pars["節15"].ToPar();
    		X0Y0_節16 = pars["節16"].ToPar();
    		X0Y0_節17 = pars["節17"].ToPar();
    		X0Y0_節18 = pars["節18"].ToPar();
    		X0Y0_節19 = pars["節19"].ToPar();
    		X0Y0_節20 = pars["節20"].ToPar();
    		X0Y0_節21 = pars["節21"].ToPar();
    		X0Y0_節22 = pars["節22"].ToPar();
    		X0Y0_節23 = pars["節23"].ToPar();
    		X0Y0_節24 = pars["節24"].ToPar();
    		X0Y0_節25 = pars["節25"].ToPar();
    		X0Y0_節26 = pars["節26"].ToPar();
    		X0Y0_節27 = pars["節27"].ToPar();
    		X0Y0_節28 = pars["節28"].ToPar();
    		X0Y0_節29 = pars["節29"].ToPar();
    		X0Y0_節30 = pars["節30"].ToPar();
    		X0Y0_節31 = pars["節31"].ToPar();
    		X0Y0_節32 = pars["節32"].ToPar();
    		X0Y0_節33 = pars["節33"].ToPar();
    		X0Y0_節34 = pars["節34"].ToPar();
    		X0Y0_節35 = pars["節35"].ToPar();
    		X0Y0_節36 = pars["節36"].ToPar();
    		X0Y0_節37 = pars["節37"].ToPar();
    		X0Y0_節38 = pars["節38"].ToPar();
    		X0Y0_節39 = pars["節39"].ToPar();
    		X0Y0_節40 = pars["節40"].ToPar();
    		X0Y0_節41 = pars["節41"].ToPar();
    		X0Y0_節42 = pars["節42"].ToPar();
    		X0Y0_節43 = pars["節43"].ToPar();
    		X0Y0_節44 = pars["節44"].ToPar();
    		X0Y0_節45 = pars["節45"].ToPar();
    		X0Y0_節46 = pars["節46"].ToPar();
    		X0Y0_節47 = pars["節47"].ToPar();
    		X0Y0_節48 = pars["節48"].ToPar();
    		X0Y0_節49 = pars["節49"].ToPar();
    		X0Y0_節50 = pars["節50"].ToPar();
    		X0Y0_節51 = pars["節51"].ToPar();
    		X0Y0_節52 = pars["節52"].ToPar();
    		X0Y0_節53 = pars["節53"].ToPar();
    		X0Y0_節54 = pars["節54"].ToPar();
    		X0Y0_節55 = pars["節55"].ToPar();
    		X0Y0_節56 = pars["節56"].ToPar();
    		X0Y0_節57 = pars["節57"].ToPar();
    		X0Y0_節58 = pars["節58"].ToPar();
    		X0Y0_節59 = pars["節59"].ToPar();
    		X0Y0_節60 = pars["節60"].ToPar();
    		Body.SetJoints();
    		接続根 = new JointD(Body);
    		右 = e.右;
    		反転X = e.反転X;
    		反転Y = e.反転Y;
    		基準C = e.基準C;
    		位置C = e.位置C;
    		角度B = e.角度B;
    		角度C = e.角度C;
    		尺度B = e.尺度B;
    		尺度C = e.尺度C;
    		尺度XB = e.尺度XB;
    		尺度XC = e.尺度XC;
    		尺度YB = e.尺度YB;
    		尺度YC = e.尺度YC;
    		肥大 = e.肥大;
    		身長 = e.身長;
    		Xv = e.Xv;
    		Yv = e.Yv;
    		Xi = e.Xi;
    		Yi = e.Yi;
    		サイズ = e.サイズ;
    		サイズX = e.サイズX;
    		サイズY = e.サイズY;
    		節1_表示 = e.節1_表示;
    		節2_表示 = e.節2_表示;
    		節3_表示 = e.節3_表示;
    		節4_表示 = e.節4_表示;
    		節5_表示 = e.節5_表示;
    		節6_表示 = e.節6_表示;
    		節7_表示 = e.節7_表示;
    		節8_表示 = e.節8_表示;
    		節9_表示 = e.節9_表示;
    		節10_表示 = e.節10_表示;
    		節11_表示 = e.節11_表示;
    		節12_表示 = e.節12_表示;
    		節13_表示 = e.節13_表示;
    		節14_表示 = e.節14_表示;
    		節15_表示 = e.節15_表示;
    		節16_表示 = e.節16_表示;
    		節17_表示 = e.節17_表示;
    		節18_表示 = e.節18_表示;
    		節19_表示 = e.節19_表示;
    		節20_表示 = e.節20_表示;
    		節21_表示 = e.節21_表示;
    		節22_表示 = e.節22_表示;
    		節23_表示 = e.節23_表示;
    		節24_表示 = e.節24_表示;
    		節25_表示 = e.節25_表示;
    		節26_表示 = e.節26_表示;
    		節27_表示 = e.節27_表示;
    		節28_表示 = e.節28_表示;
    		節29_表示 = e.節29_表示;
    		節30_表示 = e.節30_表示;
    		節31_表示 = e.節31_表示;
    		節32_表示 = e.節32_表示;
    		節33_表示 = e.節33_表示;
    		節34_表示 = e.節34_表示;
    		節35_表示 = e.節35_表示;
    		節36_表示 = e.節36_表示;
    		節37_表示 = e.節37_表示;
    		節38_表示 = e.節38_表示;
    		節39_表示 = e.節39_表示;
    		節40_表示 = e.節40_表示;
    		節41_表示 = e.節41_表示;
    		節42_表示 = e.節42_表示;
    		節43_表示 = e.節43_表示;
    		節44_表示 = e.節44_表示;
    		節45_表示 = e.節45_表示;
    		節46_表示 = e.節46_表示;
    		節47_表示 = e.節47_表示;
    		節48_表示 = e.節48_表示;
    		節49_表示 = e.節49_表示;
    		節50_表示 = e.節50_表示;
    		節51_表示 = e.節51_表示;
    		節52_表示 = e.節52_表示;
    		節53_表示 = e.節53_表示;
    		節54_表示 = e.節54_表示;
    		節55_表示 = e.節55_表示;
    		節56_表示 = e.節56_表示;
    		節57_表示 = e.節57_表示;
    		節58_表示 = e.節58_表示;
    		節59_表示 = e.節59_表示;
    		節60_表示 = e.節60_表示;
    		欠損 = e.欠損;
    		筋肉 = e.筋肉;
    		拘束 = e.拘束;
    		if (!e.表示)
    		{
    			表示 = false;
    		}
    		base.配色指定 = 配色指定;
    		配色(体配色);
    		X0Y0_節1CP = new ColorP(X0Y0_節1, 節1CD, DisUnit, abj: true);
    		X0Y0_節2CP = new ColorP(X0Y0_節2, 節2CD, DisUnit, abj: true);
    		X0Y0_節3CP = new ColorP(X0Y0_節3, 節3CD, DisUnit, abj: true);
    		X0Y0_節4CP = new ColorP(X0Y0_節4, 節4CD, DisUnit, abj: true);
    		X0Y0_節5CP = new ColorP(X0Y0_節5, 節5CD, DisUnit, abj: true);
    		X0Y0_節6CP = new ColorP(X0Y0_節6, 節6CD, DisUnit, abj: true);
    		X0Y0_節7CP = new ColorP(X0Y0_節7, 節7CD, DisUnit, abj: true);
    		X0Y0_節8CP = new ColorP(X0Y0_節8, 節8CD, DisUnit, abj: true);
    		X0Y0_節9CP = new ColorP(X0Y0_節9, 節9CD, DisUnit, abj: true);
    		X0Y0_節10CP = new ColorP(X0Y0_節10, 節10CD, DisUnit, abj: true);
    		X0Y0_節11CP = new ColorP(X0Y0_節11, 節11CD, DisUnit, abj: true);
    		X0Y0_節12CP = new ColorP(X0Y0_節12, 節12CD, DisUnit, abj: true);
    		X0Y0_節13CP = new ColorP(X0Y0_節13, 節13CD, DisUnit, abj: true);
    		X0Y0_節14CP = new ColorP(X0Y0_節14, 節14CD, DisUnit, abj: true);
    		X0Y0_節15CP = new ColorP(X0Y0_節15, 節15CD, DisUnit, abj: true);
    		X0Y0_節16CP = new ColorP(X0Y0_節16, 節16CD, DisUnit, abj: true);
    		X0Y0_節17CP = new ColorP(X0Y0_節17, 節17CD, DisUnit, abj: true);
    		X0Y0_節18CP = new ColorP(X0Y0_節18, 節18CD, DisUnit, abj: true);
    		X0Y0_節19CP = new ColorP(X0Y0_節19, 節19CD, DisUnit, abj: true);
    		X0Y0_節20CP = new ColorP(X0Y0_節20, 節20CD, DisUnit, abj: true);
    		X0Y0_節21CP = new ColorP(X0Y0_節21, 節21CD, DisUnit, abj: true);
    		X0Y0_節22CP = new ColorP(X0Y0_節22, 節22CD, DisUnit, abj: true);
    		X0Y0_節23CP = new ColorP(X0Y0_節23, 節23CD, DisUnit, abj: true);
    		X0Y0_節24CP = new ColorP(X0Y0_節24, 節24CD, DisUnit, abj: true);
    		X0Y0_節25CP = new ColorP(X0Y0_節25, 節25CD, DisUnit, abj: true);
    		X0Y0_節26CP = new ColorP(X0Y0_節26, 節26CD, DisUnit, abj: true);
    		X0Y0_節27CP = new ColorP(X0Y0_節27, 節27CD, DisUnit, abj: true);
    		X0Y0_節28CP = new ColorP(X0Y0_節28, 節28CD, DisUnit, abj: true);
    		X0Y0_節29CP = new ColorP(X0Y0_節29, 節29CD, DisUnit, abj: true);
    		X0Y0_節30CP = new ColorP(X0Y0_節30, 節30CD, DisUnit, abj: true);
    		X0Y0_節31CP = new ColorP(X0Y0_節31, 節31CD, DisUnit, abj: true);
    		X0Y0_節32CP = new ColorP(X0Y0_節32, 節32CD, DisUnit, abj: true);
    		X0Y0_節33CP = new ColorP(X0Y0_節33, 節33CD, DisUnit, abj: true);
    		X0Y0_節34CP = new ColorP(X0Y0_節34, 節34CD, DisUnit, abj: true);
    		X0Y0_節35CP = new ColorP(X0Y0_節35, 節35CD, DisUnit, abj: true);
    		X0Y0_節36CP = new ColorP(X0Y0_節36, 節36CD, DisUnit, abj: true);
    		X0Y0_節37CP = new ColorP(X0Y0_節37, 節37CD, DisUnit, abj: true);
    		X0Y0_節38CP = new ColorP(X0Y0_節38, 節38CD, DisUnit, abj: true);
    		X0Y0_節39CP = new ColorP(X0Y0_節39, 節39CD, DisUnit, abj: true);
    		X0Y0_節40CP = new ColorP(X0Y0_節40, 節40CD, DisUnit, abj: true);
    		X0Y0_節41CP = new ColorP(X0Y0_節41, 節41CD, DisUnit, abj: true);
    		X0Y0_節42CP = new ColorP(X0Y0_節42, 節42CD, DisUnit, abj: true);
    		X0Y0_節43CP = new ColorP(X0Y0_節43, 節43CD, DisUnit, abj: true);
    		X0Y0_節44CP = new ColorP(X0Y0_節44, 節44CD, DisUnit, abj: true);
    		X0Y0_節45CP = new ColorP(X0Y0_節45, 節45CD, DisUnit, abj: true);
    		X0Y0_節46CP = new ColorP(X0Y0_節46, 節46CD, DisUnit, abj: true);
    		X0Y0_節47CP = new ColorP(X0Y0_節47, 節47CD, DisUnit, abj: true);
    		X0Y0_節48CP = new ColorP(X0Y0_節48, 節48CD, DisUnit, abj: true);
    		X0Y0_節49CP = new ColorP(X0Y0_節49, 節49CD, DisUnit, abj: true);
    		X0Y0_節50CP = new ColorP(X0Y0_節50, 節50CD, DisUnit, abj: true);
    		X0Y0_節51CP = new ColorP(X0Y0_節51, 節51CD, DisUnit, abj: true);
    		X0Y0_節52CP = new ColorP(X0Y0_節52, 節52CD, DisUnit, abj: true);
    		X0Y0_節53CP = new ColorP(X0Y0_節53, 節53CD, DisUnit, abj: true);
    		X0Y0_節54CP = new ColorP(X0Y0_節54, 節54CD, DisUnit, abj: true);
    		X0Y0_節55CP = new ColorP(X0Y0_節55, 節55CD, DisUnit, abj: true);
    		X0Y0_節56CP = new ColorP(X0Y0_節56, 節56CD, DisUnit, abj: true);
    		X0Y0_節57CP = new ColorP(X0Y0_節57, 節57CD, DisUnit, abj: true);
    		X0Y0_節58CP = new ColorP(X0Y0_節58, 節58CD, DisUnit, abj: true);
    		X0Y0_節59CP = new ColorP(X0Y0_節59, 節59CD, DisUnit, abj: true);
    		X0Y0_節60CP = new ColorP(X0Y0_節60, 節60CD, DisUnit, abj: true);
    		Intensity = e.濃度;
    	}

    	public override void SetAngle0()
    	{
    		double num = (右 ? (-1.0) : 1.0);
    		int num2 = -2;
    		double num3 = 1.0;
    		double num4 = 0.01;
    		X0Y0_節1.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節2.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節3.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節4.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節5.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節6.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節7.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節8.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節9.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節10.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節11.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節12.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節13.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節14.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節15.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節16.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節17.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節18.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節19.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節20.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節21.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節22.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節23.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節24.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節25.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節26.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節27.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節28.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節29.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節30.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節31.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節32.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節33.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節34.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節35.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節36.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節37.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節38.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節39.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節40.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節41.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節42.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節43.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節44.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節45.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節46.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節47.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節48.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節49.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節50.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節51.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節52.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節53.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節54.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節55.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節56.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節57.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節58.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節59.SetAngleBase(num * (double)num2 * num3);
    		num3 -= num4;
    		X0Y0_節60.SetAngleBase(num * (double)num2 * num3);
    		Body.JoinPAall();
    	}

    	public override IEnumerable<Par> Enum軸()
    	{
    		yield return X0Y0_節1;
    		yield return X0Y0_節2;
    		yield return X0Y0_節3;
    		yield return X0Y0_節4;
    		yield return X0Y0_節5;
    		yield return X0Y0_節6;
    		yield return X0Y0_節7;
    		yield return X0Y0_節8;
    		yield return X0Y0_節9;
    		yield return X0Y0_節10;
    		yield return X0Y0_節11;
    		yield return X0Y0_節12;
    		yield return X0Y0_節13;
    		yield return X0Y0_節14;
    		yield return X0Y0_節15;
    		yield return X0Y0_節16;
    		yield return X0Y0_節17;
    		yield return X0Y0_節18;
    		yield return X0Y0_節19;
    		yield return X0Y0_節20;
    		yield return X0Y0_節21;
    		yield return X0Y0_節22;
    		yield return X0Y0_節23;
    		yield return X0Y0_節24;
    		yield return X0Y0_節25;
    		yield return X0Y0_節26;
    		yield return X0Y0_節27;
    		yield return X0Y0_節28;
    		yield return X0Y0_節29;
    		yield return X0Y0_節30;
    		yield return X0Y0_節31;
    		yield return X0Y0_節32;
    		yield return X0Y0_節33;
    		yield return X0Y0_節34;
    		yield return X0Y0_節35;
    		yield return X0Y0_節36;
    		yield return X0Y0_節37;
    		yield return X0Y0_節38;
    		yield return X0Y0_節39;
    		yield return X0Y0_節40;
    		yield return X0Y0_節41;
    		yield return X0Y0_節42;
    		yield return X0Y0_節43;
    		yield return X0Y0_節44;
    		yield return X0Y0_節45;
    		yield return X0Y0_節46;
    		yield return X0Y0_節47;
    		yield return X0Y0_節48;
    		yield return X0Y0_節49;
    		yield return X0Y0_節50;
    		yield return X0Y0_節51;
    		yield return X0Y0_節52;
    		yield return X0Y0_節53;
    		yield return X0Y0_節54;
    		yield return X0Y0_節55;
    		yield return X0Y0_節56;
    		yield return X0Y0_節57;
    		yield return X0Y0_節58;
    		yield return X0Y0_節59;
    		yield return X0Y0_節60;
    	}

    	public override void 色更新()
    	{
    		X0Y0_節1CP.Update();
    		X0Y0_節2CP.Update();
    		X0Y0_節3CP.Update();
    		X0Y0_節4CP.Update();
    		X0Y0_節5CP.Update();
    		X0Y0_節6CP.Update();
    		X0Y0_節7CP.Update();
    		X0Y0_節8CP.Update();
    		X0Y0_節9CP.Update();
    		X0Y0_節10CP.Update();
    		X0Y0_節11CP.Update();
    		X0Y0_節12CP.Update();
    		X0Y0_節13CP.Update();
    		X0Y0_節14CP.Update();
    		X0Y0_節15CP.Update();
    		X0Y0_節16CP.Update();
    		X0Y0_節17CP.Update();
    		X0Y0_節18CP.Update();
    		X0Y0_節19CP.Update();
    		X0Y0_節20CP.Update();
    		X0Y0_節21CP.Update();
    		X0Y0_節22CP.Update();
    		X0Y0_節23CP.Update();
    		X0Y0_節24CP.Update();
    		X0Y0_節25CP.Update();
    		X0Y0_節26CP.Update();
    		X0Y0_節27CP.Update();
    		X0Y0_節28CP.Update();
    		X0Y0_節29CP.Update();
    		X0Y0_節30CP.Update();
    		X0Y0_節31CP.Update();
    		X0Y0_節32CP.Update();
    		X0Y0_節33CP.Update();
    		X0Y0_節34CP.Update();
    		X0Y0_節35CP.Update();
    		X0Y0_節36CP.Update();
    		X0Y0_節37CP.Update();
    		X0Y0_節38CP.Update();
    		X0Y0_節39CP.Update();
    		X0Y0_節40CP.Update();
    		X0Y0_節41CP.Update();
    		X0Y0_節42CP.Update();
    		X0Y0_節43CP.Update();
    		X0Y0_節44CP.Update();
    		X0Y0_節45CP.Update();
    		X0Y0_節46CP.Update();
    		X0Y0_節47CP.Update();
    		X0Y0_節48CP.Update();
    		X0Y0_節49CP.Update();
    		X0Y0_節50CP.Update();
    		X0Y0_節51CP.Update();
    		X0Y0_節52CP.Update();
    		X0Y0_節53CP.Update();
    		X0Y0_節54CP.Update();
    		X0Y0_節55CP.Update();
    		X0Y0_節56CP.Update();
    		X0Y0_節57CP.Update();
    		X0Y0_節58CP.Update();
    		X0Y0_節59CP.Update();
    		X0Y0_節60CP.Update();
    	}

    	private void 配色(BodyColorSet 体配色)
    	{
    		switch (配色指定)
    		{
    		case 配色指定.N0:
    			配色N0(体配色);
    			break;
    		case 配色指定.T0:
    			配色T0(体配色);
    			break;
    		case 配色指定.T1:
    			配色T1(体配色);
    			break;
    		default:
    			配色N0(体配色);
    			break;
    		}
    	}

    	private void 配色N0(BodyColorSet 体配色)
    	{
    		節1CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節2CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節3CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節4CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節5CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節6CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節7CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節8CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節9CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節10CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節11CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節12CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節13CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節14CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節15CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節16CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節17CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節18CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節19CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節20CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節21CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節22CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節23CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節24CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節25CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節26CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節27CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節28CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節29CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節30CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節31CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節32CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節33CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節34CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節35CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節36CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節37CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節38CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節39CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節40CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節41CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節42CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節43CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節44CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節45CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節46CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節47CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節48CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節49CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節50CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節51CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節52CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節53CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節54CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節55CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節56CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節57CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節58CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節59CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    		節60CD = new ColorD(ref Col.Black, ref 体配色.甲1O);
    	}

    	private void 配色T0(BodyColorSet 体配色)
    	{
    		this.配色T(0, "節", ref 体配色.甲1O, ref 体配色.刺青O);
    	}

    	private void 配色T1(BodyColorSet 体配色)
    	{
    		this.配色T(1, "節", ref 体配色.甲1O, ref 体配色.刺青O);
    	}
    }
}
