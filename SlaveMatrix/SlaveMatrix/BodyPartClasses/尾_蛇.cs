using System.Collections.Generic;
using _2DGAMELIB;

namespace SlaveMatrix
{
    public class 尾_蛇 : 尾
    {
    	public ShapePart X0Y0_尾1_根;

    	public ShapePart X0Y0_尾1_鱗;

    	public ShapePart X0Y0_尾1_鱗左;

    	public ShapePart X0Y0_尾1_鱗右;

    	public ShapePart X0Y0_尾1_尾;

    	public ShapePart X0Y0_尾2_鱗;

    	public ShapePart X0Y0_尾2_鱗左;

    	public ShapePart X0Y0_尾2_鱗右;

    	public ShapePart X0Y0_尾2_尾;

    	public ShapePart X0Y0_尾3_鱗;

    	public ShapePart X0Y0_尾3_鱗左;

    	public ShapePart X0Y0_尾3_鱗右;

    	public ShapePart X0Y0_尾3_尾;

    	public ShapePart X0Y0_尾4_鱗;

    	public ShapePart X0Y0_尾4_鱗左;

    	public ShapePart X0Y0_尾4_鱗右;

    	public ShapePart X0Y0_尾4_尾;

    	public ShapePart X0Y0_尾5_鱗;

    	public ShapePart X0Y0_尾5_鱗左;

    	public ShapePart X0Y0_尾5_鱗右;

    	public ShapePart X0Y0_尾5_尾;

    	public ShapePart X0Y0_尾6_鱗;

    	public ShapePart X0Y0_尾6_鱗左;

    	public ShapePart X0Y0_尾6_鱗右;

    	public ShapePart X0Y0_尾6_尾;

    	public ShapePart X0Y0_尾7_鱗;

    	public ShapePart X0Y0_尾7_鱗左;

    	public ShapePart X0Y0_尾7_鱗右;

    	public ShapePart X0Y0_尾7_尾;

    	public ShapePart X0Y0_尾8_鱗;

    	public ShapePart X0Y0_尾8_鱗左;

    	public ShapePart X0Y0_尾8_鱗右;

    	public ShapePart X0Y0_尾8_尾;

    	public ShapePart X0Y0_尾9_鱗;

    	public ShapePart X0Y0_尾9_鱗左;

    	public ShapePart X0Y0_尾9_鱗右;

    	public ShapePart X0Y0_尾9_尾;

    	public ShapePart X0Y0_尾10_鱗;

    	public ShapePart X0Y0_尾10_鱗左;

    	public ShapePart X0Y0_尾10_鱗右;

    	public ShapePart X0Y0_尾10_尾;

    	public ShapePart X0Y0_尾11_鱗;

    	public ShapePart X0Y0_尾11_鱗左;

    	public ShapePart X0Y0_尾11_鱗右;

    	public ShapePart X0Y0_尾11_尾;

    	public ShapePart X0Y0_尾12_鱗;

    	public ShapePart X0Y0_尾12_鱗左;

    	public ShapePart X0Y0_尾12_鱗右;

    	public ShapePart X0Y0_尾12_尾;

    	public ShapePart X0Y0_尾13_鱗;

    	public ShapePart X0Y0_尾13_鱗左;

    	public ShapePart X0Y0_尾13_鱗右;

    	public ShapePart X0Y0_尾13_尾;

    	public ShapePart X0Y0_尾14_鱗;

    	public ShapePart X0Y0_尾14_鱗左;

    	public ShapePart X0Y0_尾14_鱗右;

    	public ShapePart X0Y0_尾14_尾;

    	public ShapePart X0Y0_尾15_鱗;

    	public ShapePart X0Y0_尾15_鱗左;

    	public ShapePart X0Y0_尾15_鱗右;

    	public ShapePart X0Y0_尾15_尾;

    	public ShapePart X0Y0_尾16_鱗;

    	public ShapePart X0Y0_尾16_鱗左;

    	public ShapePart X0Y0_尾16_鱗右;

    	public ShapePart X0Y0_尾16_尾;

    	public ShapePart X0Y0_尾17_鱗;

    	public ShapePart X0Y0_尾17_鱗左;

    	public ShapePart X0Y0_尾17_鱗右;

    	public ShapePart X0Y0_尾17_尾;

    	public ShapePart X0Y0_尾18_鱗;

    	public ShapePart X0Y0_尾18_鱗左;

    	public ShapePart X0Y0_尾18_鱗右;

    	public ShapePart X0Y0_尾18_尾;

    	public ShapePart X0Y0_尾19_鱗;

    	public ShapePart X0Y0_尾19_鱗左;

    	public ShapePart X0Y0_尾19_鱗右;

    	public ShapePart X0Y0_尾19_尾;

    	public ShapePart X0Y0_尾20_鱗;

    	public ShapePart X0Y0_尾20_鱗左;

    	public ShapePart X0Y0_尾20_鱗右;

    	public ShapePart X0Y0_尾20_尾;

    	public ShapePart X0Y0_尾21_鱗;

    	public ShapePart X0Y0_尾21_鱗左;

    	public ShapePart X0Y0_尾21_鱗右;

    	public ShapePart X0Y0_尾21_尾;

    	public ShapePart X0Y0_尾22_鱗;

    	public ShapePart X0Y0_尾22_鱗左;

    	public ShapePart X0Y0_尾22_鱗右;

    	public ShapePart X0Y0_尾22_尾;

    	public ShapePart X0Y0_尾23_鱗;

    	public ShapePart X0Y0_尾23_鱗左;

    	public ShapePart X0Y0_尾23_鱗右;

    	public ShapePart X0Y0_尾23_尾;

    	public ShapePart X0Y0_尾24_鱗;

    	public ShapePart X0Y0_尾24_鱗左;

    	public ShapePart X0Y0_尾24_鱗右;

    	public ShapePart X0Y0_尾24_尾;

    	public ShapePart X0Y0_尾25_鱗;

    	public ShapePart X0Y0_尾25_鱗左;

    	public ShapePart X0Y0_尾25_鱗右;

    	public ShapePart X0Y0_尾25_尾;

    	public ShapePart X0Y0_尾26_鱗;

    	public ShapePart X0Y0_尾26_鱗左;

    	public ShapePart X0Y0_尾26_鱗右;

    	public ShapePart X0Y0_尾26_尾;

    	public ShapePart X0Y0_輪1_革;

    	public ShapePart X0Y0_輪1_金具1;

    	public ShapePart X0Y0_輪1_金具2;

    	public ShapePart X0Y0_輪1_金具3;

    	public ShapePart X0Y0_輪1_金具左;

    	public ShapePart X0Y0_輪1_金具右;

    	public ShapePart X0Y0_尾27_鱗;

    	public ShapePart X0Y0_尾27_鱗左;

    	public ShapePart X0Y0_尾27_鱗右;

    	public ShapePart X0Y0_尾27_尾;

    	public ShapePart X0Y0_尾28_鱗;

    	public ShapePart X0Y0_尾28_鱗左;

    	public ShapePart X0Y0_尾28_鱗右;

    	public ShapePart X0Y0_尾28_尾;

    	public ShapePart X0Y0_尾29_鱗;

    	public ShapePart X0Y0_尾29_鱗左;

    	public ShapePart X0Y0_尾29_鱗右;

    	public ShapePart X0Y0_尾29_尾;

    	public ShapePart X0Y0_尾30_鱗;

    	public ShapePart X0Y0_尾30_鱗左;

    	public ShapePart X0Y0_尾30_鱗右;

    	public ShapePart X0Y0_尾30_尾;

    	public ShapePart X0Y0_尾31_鱗;

    	public ShapePart X0Y0_尾31_鱗左;

    	public ShapePart X0Y0_尾31_鱗右;

    	public ShapePart X0Y0_尾31_尾;

    	public ShapePart X0Y0_尾32_鱗;

    	public ShapePart X0Y0_尾32_鱗左;

    	public ShapePart X0Y0_尾32_鱗右;

    	public ShapePart X0Y0_尾32_尾;

    	public ShapePart X0Y0_尾33_鱗;

    	public ShapePart X0Y0_尾33_鱗左;

    	public ShapePart X0Y0_尾33_鱗右;

    	public ShapePart X0Y0_尾33_尾;

    	public ShapePart X0Y0_尾34_鱗;

    	public ShapePart X0Y0_尾34_鱗左;

    	public ShapePart X0Y0_尾34_鱗右;

    	public ShapePart X0Y0_尾34_尾;

    	public ShapePart X0Y0_尾35_鱗;

    	public ShapePart X0Y0_尾35_鱗左;

    	public ShapePart X0Y0_尾35_鱗右;

    	public ShapePart X0Y0_尾35_尾;

    	public ShapePart X0Y0_尾36_鱗;

    	public ShapePart X0Y0_尾36_鱗左;

    	public ShapePart X0Y0_尾36_鱗右;

    	public ShapePart X0Y0_尾36_尾;

    	public ShapePart X0Y0_尾37_鱗;

    	public ShapePart X0Y0_尾37_鱗左;

    	public ShapePart X0Y0_尾37_鱗右;

    	public ShapePart X0Y0_尾37_尾;

    	public ShapePart X0Y0_尾38_鱗;

    	public ShapePart X0Y0_尾38_鱗左;

    	public ShapePart X0Y0_尾38_鱗右;

    	public ShapePart X0Y0_尾38_尾;

    	public ShapePart X0Y0_尾39_鱗;

    	public ShapePart X0Y0_尾39_鱗左;

    	public ShapePart X0Y0_尾39_鱗右;

    	public ShapePart X0Y0_尾39_尾;

    	public ShapePart X0Y0_尾40_鱗;

    	public ShapePart X0Y0_尾40_鱗左;

    	public ShapePart X0Y0_尾40_鱗右;

    	public ShapePart X0Y0_尾40_尾;

    	public ShapePart X0Y0_尾41_鱗;

    	public ShapePart X0Y0_尾41_鱗左;

    	public ShapePart X0Y0_尾41_鱗右;

    	public ShapePart X0Y0_尾41_尾;

    	public ShapePart X0Y0_尾42_鱗;

    	public ShapePart X0Y0_尾42_鱗左;

    	public ShapePart X0Y0_尾42_鱗右;

    	public ShapePart X0Y0_尾42_尾;

    	public ShapePart X0Y0_尾43_鱗;

    	public ShapePart X0Y0_尾43_鱗左;

    	public ShapePart X0Y0_尾43_鱗右;

    	public ShapePart X0Y0_尾43_尾;

    	public ShapePart X0Y0_尾44_鱗;

    	public ShapePart X0Y0_尾44_鱗左;

    	public ShapePart X0Y0_尾44_鱗右;

    	public ShapePart X0Y0_尾44_尾;

    	public ShapePart X0Y0_頭_上顎_顎基;

    	public ShapePart X0Y0_頭_上顎_鱗4;

    	public ShapePart X0Y0_頭_上顎_鱗左1;

    	public ShapePart X0Y0_頭_上顎_鱗右1;

    	public ShapePart X0Y0_頭_上顎_鱗左2;

    	public ShapePart X0Y0_頭_上顎_鱗右2;

    	public ShapePart X0Y0_頭_上顎_鱗左3;

    	public ShapePart X0Y0_頭_上顎_鱗右3;

    	public ShapePart X0Y0_頭_上顎_鱗左4;

    	public ShapePart X0Y0_頭_上顎_鱗右4;

    	public ShapePart X0Y0_頭_上顎_鱗左5;

    	public ShapePart X0Y0_頭_上顎_鱗右5;

    	public ShapePart X0Y0_頭_上顎_鱗左8;

    	public ShapePart X0Y0_頭_上顎_鱗右8;

    	public ShapePart X0Y0_頭_上顎_鱗左9;

    	public ShapePart X0Y0_頭_上顎_鱗右9;

    	public ShapePart X0Y0_頭_上顎_鱗左10;

    	public ShapePart X0Y0_頭_上顎_鱗右10;

    	public ShapePart X0Y0_頭_上顎_鱗左11;

    	public ShapePart X0Y0_頭_上顎_鱗右11;

    	public ShapePart X0Y0_頭_下顎_顎基;

    	public ShapePart X0Y0_頭_下顎_鱗1;

    	public ShapePart X0Y0_頭_下顎_鱗2;

    	public ShapePart X0Y0_頭_下顎_鱗3;

    	public ShapePart X0Y0_頭_下顎_鱗4;

    	public ShapePart X0Y0_頭_下顎_鱗左1;

    	public ShapePart X0Y0_頭_下顎_鱗右1;

    	public ShapePart X0Y0_頭_下顎_鱗左2;

    	public ShapePart X0Y0_頭_下顎_鱗右2;

    	public ShapePart X0Y0_頭_下顎_鱗左3;

    	public ShapePart X0Y0_頭_下顎_鱗右3;

    	public ShapePart X0Y0_頭_下顎_鱗左4;

    	public ShapePart X0Y0_頭_下顎_鱗右4;

    	public ShapePart X0Y0_頭_下顎_鱗左5;

    	public ShapePart X0Y0_頭_下顎_鱗右5;

    	public ShapePart X0Y0_頭_下顎_鱗左6;

    	public ShapePart X0Y0_頭_下顎_鱗右6;

    	public ShapePart X0Y0_頭_下顎_鱗左7;

    	public ShapePart X0Y0_頭_下顎_鱗右7;

    	public ShapePart X0Y0_頭_下顎_鱗左8;

    	public ShapePart X0Y0_頭_下顎_鱗右8;

    	public ShapePart X0Y0_頭_下顎_鱗左9;

    	public ShapePart X0Y0_頭_下顎_鱗右9;

    	public ShapePart X0Y0_頭_下顎_鱗左10;

    	public ShapePart X0Y0_頭_下顎_鱗右10;

    	public ShapePart X0Y0_頭_下顎_鱗左11;

    	public ShapePart X0Y0_頭_下顎_鱗右11;

    	public ShapePart X0Y0_輪2_革;

    	public ShapePart X0Y0_輪2_金具1;

    	public ShapePart X0Y0_輪2_金具2;

    	public ShapePart X0Y0_輪2_金具3;

    	public ShapePart X0Y0_輪2_金具左;

    	public ShapePart X0Y0_輪2_金具右;

    	public ColorD 尾1_根CD;

    	public ColorD 尾1_鱗CD;

    	public ColorD 尾1_鱗左CD;

    	public ColorD 尾1_鱗右CD;

    	public ColorD 尾1_尾CD;

    	public ColorD 尾2_鱗CD;

    	public ColorD 尾2_鱗左CD;

    	public ColorD 尾2_鱗右CD;

    	public ColorD 尾2_尾CD;

    	public ColorD 尾3_鱗CD;

    	public ColorD 尾3_鱗左CD;

    	public ColorD 尾3_鱗右CD;

    	public ColorD 尾3_尾CD;

    	public ColorD 尾4_鱗CD;

    	public ColorD 尾4_鱗左CD;

    	public ColorD 尾4_鱗右CD;

    	public ColorD 尾4_尾CD;

    	public ColorD 尾5_鱗CD;

    	public ColorD 尾5_鱗左CD;

    	public ColorD 尾5_鱗右CD;

    	public ColorD 尾5_尾CD;

    	public ColorD 尾6_鱗CD;

    	public ColorD 尾6_鱗左CD;

    	public ColorD 尾6_鱗右CD;

    	public ColorD 尾6_尾CD;

    	public ColorD 尾7_鱗CD;

    	public ColorD 尾7_鱗左CD;

    	public ColorD 尾7_鱗右CD;

    	public ColorD 尾7_尾CD;

    	public ColorD 尾8_鱗CD;

    	public ColorD 尾8_鱗左CD;

    	public ColorD 尾8_鱗右CD;

    	public ColorD 尾8_尾CD;

    	public ColorD 尾9_鱗CD;

    	public ColorD 尾9_鱗左CD;

    	public ColorD 尾9_鱗右CD;

    	public ColorD 尾9_尾CD;

    	public ColorD 尾10_鱗CD;

    	public ColorD 尾10_鱗左CD;

    	public ColorD 尾10_鱗右CD;

    	public ColorD 尾10_尾CD;

    	public ColorD 尾11_鱗CD;

    	public ColorD 尾11_鱗左CD;

    	public ColorD 尾11_鱗右CD;

    	public ColorD 尾11_尾CD;

    	public ColorD 尾12_鱗CD;

    	public ColorD 尾12_鱗左CD;

    	public ColorD 尾12_鱗右CD;

    	public ColorD 尾12_尾CD;

    	public ColorD 尾13_鱗CD;

    	public ColorD 尾13_鱗左CD;

    	public ColorD 尾13_鱗右CD;

    	public ColorD 尾13_尾CD;

    	public ColorD 尾14_鱗CD;

    	public ColorD 尾14_鱗左CD;

    	public ColorD 尾14_鱗右CD;

    	public ColorD 尾14_尾CD;

    	public ColorD 尾15_鱗CD;

    	public ColorD 尾15_鱗左CD;

    	public ColorD 尾15_鱗右CD;

    	public ColorD 尾15_尾CD;

    	public ColorD 尾16_鱗CD;

    	public ColorD 尾16_鱗左CD;

    	public ColorD 尾16_鱗右CD;

    	public ColorD 尾16_尾CD;

    	public ColorD 尾17_鱗CD;

    	public ColorD 尾17_鱗左CD;

    	public ColorD 尾17_鱗右CD;

    	public ColorD 尾17_尾CD;

    	public ColorD 尾18_鱗CD;

    	public ColorD 尾18_鱗左CD;

    	public ColorD 尾18_鱗右CD;

    	public ColorD 尾18_尾CD;

    	public ColorD 尾19_鱗CD;

    	public ColorD 尾19_鱗左CD;

    	public ColorD 尾19_鱗右CD;

    	public ColorD 尾19_尾CD;

    	public ColorD 尾20_鱗CD;

    	public ColorD 尾20_鱗左CD;

    	public ColorD 尾20_鱗右CD;

    	public ColorD 尾20_尾CD;

    	public ColorD 尾21_鱗CD;

    	public ColorD 尾21_鱗左CD;

    	public ColorD 尾21_鱗右CD;

    	public ColorD 尾21_尾CD;

    	public ColorD 尾22_鱗CD;

    	public ColorD 尾22_鱗左CD;

    	public ColorD 尾22_鱗右CD;

    	public ColorD 尾22_尾CD;

    	public ColorD 尾23_鱗CD;

    	public ColorD 尾23_鱗左CD;

    	public ColorD 尾23_鱗右CD;

    	public ColorD 尾23_尾CD;

    	public ColorD 尾24_鱗CD;

    	public ColorD 尾24_鱗左CD;

    	public ColorD 尾24_鱗右CD;

    	public ColorD 尾24_尾CD;

    	public ColorD 尾25_鱗CD;

    	public ColorD 尾25_鱗左CD;

    	public ColorD 尾25_鱗右CD;

    	public ColorD 尾25_尾CD;

    	public ColorD 尾26_鱗CD;

    	public ColorD 尾26_鱗左CD;

    	public ColorD 尾26_鱗右CD;

    	public ColorD 尾26_尾CD;

    	public ColorD 尾27_鱗CD;

    	public ColorD 尾27_鱗左CD;

    	public ColorD 尾27_鱗右CD;

    	public ColorD 尾27_尾CD;

    	public ColorD 尾28_鱗CD;

    	public ColorD 尾28_鱗左CD;

    	public ColorD 尾28_鱗右CD;

    	public ColorD 尾28_尾CD;

    	public ColorD 尾29_鱗CD;

    	public ColorD 尾29_鱗左CD;

    	public ColorD 尾29_鱗右CD;

    	public ColorD 尾29_尾CD;

    	public ColorD 尾30_鱗CD;

    	public ColorD 尾30_鱗左CD;

    	public ColorD 尾30_鱗右CD;

    	public ColorD 尾30_尾CD;

    	public ColorD 尾31_鱗CD;

    	public ColorD 尾31_鱗左CD;

    	public ColorD 尾31_鱗右CD;

    	public ColorD 尾31_尾CD;

    	public ColorD 尾32_鱗CD;

    	public ColorD 尾32_鱗左CD;

    	public ColorD 尾32_鱗右CD;

    	public ColorD 尾32_尾CD;

    	public ColorD 尾33_鱗CD;

    	public ColorD 尾33_鱗左CD;

    	public ColorD 尾33_鱗右CD;

    	public ColorD 尾33_尾CD;

    	public ColorD 尾34_鱗CD;

    	public ColorD 尾34_鱗左CD;

    	public ColorD 尾34_鱗右CD;

    	public ColorD 尾34_尾CD;

    	public ColorD 尾35_鱗CD;

    	public ColorD 尾35_鱗左CD;

    	public ColorD 尾35_鱗右CD;

    	public ColorD 尾35_尾CD;

    	public ColorD 尾36_鱗CD;

    	public ColorD 尾36_鱗左CD;

    	public ColorD 尾36_鱗右CD;

    	public ColorD 尾36_尾CD;

    	public ColorD 尾37_鱗CD;

    	public ColorD 尾37_鱗左CD;

    	public ColorD 尾37_鱗右CD;

    	public ColorD 尾37_尾CD;

    	public ColorD 尾38_鱗CD;

    	public ColorD 尾38_鱗左CD;

    	public ColorD 尾38_鱗右CD;

    	public ColorD 尾38_尾CD;

    	public ColorD 尾39_鱗CD;

    	public ColorD 尾39_鱗左CD;

    	public ColorD 尾39_鱗右CD;

    	public ColorD 尾39_尾CD;

    	public ColorD 尾40_鱗CD;

    	public ColorD 尾40_鱗左CD;

    	public ColorD 尾40_鱗右CD;

    	public ColorD 尾40_尾CD;

    	public ColorD 尾41_鱗CD;

    	public ColorD 尾41_鱗左CD;

    	public ColorD 尾41_鱗右CD;

    	public ColorD 尾41_尾CD;

    	public ColorD 尾42_鱗CD;

    	public ColorD 尾42_鱗左CD;

    	public ColorD 尾42_鱗右CD;

    	public ColorD 尾42_尾CD;

    	public ColorD 尾43_鱗CD;

    	public ColorD 尾43_鱗左CD;

    	public ColorD 尾43_鱗右CD;

    	public ColorD 尾43_尾CD;

    	public ColorD 尾44_鱗CD;

    	public ColorD 尾44_鱗左CD;

    	public ColorD 尾44_鱗右CD;

    	public ColorD 尾44_尾CD;

    	public ColorD 頭_上顎_顎基CD;

    	public ColorD 頭_上顎_鱗4CD;

    	public ColorD 頭_上顎_鱗左1CD;

    	public ColorD 頭_上顎_鱗右1CD;

    	public ColorD 頭_上顎_鱗左2CD;

    	public ColorD 頭_上顎_鱗右2CD;

    	public ColorD 頭_上顎_鱗左3CD;

    	public ColorD 頭_上顎_鱗右3CD;

    	public ColorD 頭_上顎_鱗左4CD;

    	public ColorD 頭_上顎_鱗右4CD;

    	public ColorD 頭_上顎_鱗左5CD;

    	public ColorD 頭_上顎_鱗右5CD;

    	public ColorD 頭_上顎_鱗左8CD;

    	public ColorD 頭_上顎_鱗右8CD;

    	public ColorD 頭_上顎_鱗左9CD;

    	public ColorD 頭_上顎_鱗右9CD;

    	public ColorD 頭_上顎_鱗左10CD;

    	public ColorD 頭_上顎_鱗右10CD;

    	public ColorD 頭_上顎_鱗左11CD;

    	public ColorD 頭_上顎_鱗右11CD;

    	public ColorD 頭_下顎_顎基CD;

    	public ColorD 頭_下顎_鱗1CD;

    	public ColorD 頭_下顎_鱗2CD;

    	public ColorD 頭_下顎_鱗3CD;

    	public ColorD 頭_下顎_鱗4CD;

    	public ColorD 頭_下顎_鱗左1CD;

    	public ColorD 頭_下顎_鱗右1CD;

    	public ColorD 頭_下顎_鱗左2CD;

    	public ColorD 頭_下顎_鱗右2CD;

    	public ColorD 頭_下顎_鱗左3CD;

    	public ColorD 頭_下顎_鱗右3CD;

    	public ColorD 頭_下顎_鱗左4CD;

    	public ColorD 頭_下顎_鱗右4CD;

    	public ColorD 頭_下顎_鱗左5CD;

    	public ColorD 頭_下顎_鱗右5CD;

    	public ColorD 頭_下顎_鱗左6CD;

    	public ColorD 頭_下顎_鱗右6CD;

    	public ColorD 頭_下顎_鱗左7CD;

    	public ColorD 頭_下顎_鱗右7CD;

    	public ColorD 頭_下顎_鱗左8CD;

    	public ColorD 頭_下顎_鱗右8CD;

    	public ColorD 頭_下顎_鱗左9CD;

    	public ColorD 頭_下顎_鱗右9CD;

    	public ColorD 頭_下顎_鱗左10CD;

    	public ColorD 頭_下顎_鱗右10CD;

    	public ColorD 頭_下顎_鱗左11CD;

    	public ColorD 頭_下顎_鱗右11CD;

    	public ColorD 輪1_革CD;

    	public ColorD 輪1_金具1CD;

    	public ColorD 輪1_金具2CD;

    	public ColorD 輪1_金具3CD;

    	public ColorD 輪1_金具左CD;

    	public ColorD 輪1_金具右CD;

    	public ColorD 輪2_革CD;

    	public ColorD 輪2_金具1CD;

    	public ColorD 輪2_金具2CD;

    	public ColorD 輪2_金具3CD;

    	public ColorD 輪2_金具左CD;

    	public ColorD 輪2_金具右CD;

    	public ColorP X0Y0_尾1_根CP;

    	public ColorP X0Y0_尾1_鱗CP;

    	public ColorP X0Y0_尾1_鱗左CP;

    	public ColorP X0Y0_尾1_鱗右CP;

    	public ColorP X0Y0_尾1_尾CP;

    	public ColorP X0Y0_尾2_鱗CP;

    	public ColorP X0Y0_尾2_鱗左CP;

    	public ColorP X0Y0_尾2_鱗右CP;

    	public ColorP X0Y0_尾2_尾CP;

    	public ColorP X0Y0_尾3_鱗CP;

    	public ColorP X0Y0_尾3_鱗左CP;

    	public ColorP X0Y0_尾3_鱗右CP;

    	public ColorP X0Y0_尾3_尾CP;

    	public ColorP X0Y0_尾4_鱗CP;

    	public ColorP X0Y0_尾4_鱗左CP;

    	public ColorP X0Y0_尾4_鱗右CP;

    	public ColorP X0Y0_尾4_尾CP;

    	public ColorP X0Y0_尾5_鱗CP;

    	public ColorP X0Y0_尾5_鱗左CP;

    	public ColorP X0Y0_尾5_鱗右CP;

    	public ColorP X0Y0_尾5_尾CP;

    	public ColorP X0Y0_尾6_鱗CP;

    	public ColorP X0Y0_尾6_鱗左CP;

    	public ColorP X0Y0_尾6_鱗右CP;

    	public ColorP X0Y0_尾6_尾CP;

    	public ColorP X0Y0_尾7_鱗CP;

    	public ColorP X0Y0_尾7_鱗左CP;

    	public ColorP X0Y0_尾7_鱗右CP;

    	public ColorP X0Y0_尾7_尾CP;

    	public ColorP X0Y0_尾8_鱗CP;

    	public ColorP X0Y0_尾8_鱗左CP;

    	public ColorP X0Y0_尾8_鱗右CP;

    	public ColorP X0Y0_尾8_尾CP;

    	public ColorP X0Y0_尾9_鱗CP;

    	public ColorP X0Y0_尾9_鱗左CP;

    	public ColorP X0Y0_尾9_鱗右CP;

    	public ColorP X0Y0_尾9_尾CP;

    	public ColorP X0Y0_尾10_鱗CP;

    	public ColorP X0Y0_尾10_鱗左CP;

    	public ColorP X0Y0_尾10_鱗右CP;

    	public ColorP X0Y0_尾10_尾CP;

    	public ColorP X0Y0_尾11_鱗CP;

    	public ColorP X0Y0_尾11_鱗左CP;

    	public ColorP X0Y0_尾11_鱗右CP;

    	public ColorP X0Y0_尾11_尾CP;

    	public ColorP X0Y0_尾12_鱗CP;

    	public ColorP X0Y0_尾12_鱗左CP;

    	public ColorP X0Y0_尾12_鱗右CP;

    	public ColorP X0Y0_尾12_尾CP;

    	public ColorP X0Y0_尾13_鱗CP;

    	public ColorP X0Y0_尾13_鱗左CP;

    	public ColorP X0Y0_尾13_鱗右CP;

    	public ColorP X0Y0_尾13_尾CP;

    	public ColorP X0Y0_尾14_鱗CP;

    	public ColorP X0Y0_尾14_鱗左CP;

    	public ColorP X0Y0_尾14_鱗右CP;

    	public ColorP X0Y0_尾14_尾CP;

    	public ColorP X0Y0_尾15_鱗CP;

    	public ColorP X0Y0_尾15_鱗左CP;

    	public ColorP X0Y0_尾15_鱗右CP;

    	public ColorP X0Y0_尾15_尾CP;

    	public ColorP X0Y0_尾16_鱗CP;

    	public ColorP X0Y0_尾16_鱗左CP;

    	public ColorP X0Y0_尾16_鱗右CP;

    	public ColorP X0Y0_尾16_尾CP;

    	public ColorP X0Y0_尾17_鱗CP;

    	public ColorP X0Y0_尾17_鱗左CP;

    	public ColorP X0Y0_尾17_鱗右CP;

    	public ColorP X0Y0_尾17_尾CP;

    	public ColorP X0Y0_尾18_鱗CP;

    	public ColorP X0Y0_尾18_鱗左CP;

    	public ColorP X0Y0_尾18_鱗右CP;

    	public ColorP X0Y0_尾18_尾CP;

    	public ColorP X0Y0_尾19_鱗CP;

    	public ColorP X0Y0_尾19_鱗左CP;

    	public ColorP X0Y0_尾19_鱗右CP;

    	public ColorP X0Y0_尾19_尾CP;

    	public ColorP X0Y0_尾20_鱗CP;

    	public ColorP X0Y0_尾20_鱗左CP;

    	public ColorP X0Y0_尾20_鱗右CP;

    	public ColorP X0Y0_尾20_尾CP;

    	public ColorP X0Y0_尾21_鱗CP;

    	public ColorP X0Y0_尾21_鱗左CP;

    	public ColorP X0Y0_尾21_鱗右CP;

    	public ColorP X0Y0_尾21_尾CP;

    	public ColorP X0Y0_尾22_鱗CP;

    	public ColorP X0Y0_尾22_鱗左CP;

    	public ColorP X0Y0_尾22_鱗右CP;

    	public ColorP X0Y0_尾22_尾CP;

    	public ColorP X0Y0_尾23_鱗CP;

    	public ColorP X0Y0_尾23_鱗左CP;

    	public ColorP X0Y0_尾23_鱗右CP;

    	public ColorP X0Y0_尾23_尾CP;

    	public ColorP X0Y0_尾24_鱗CP;

    	public ColorP X0Y0_尾24_鱗左CP;

    	public ColorP X0Y0_尾24_鱗右CP;

    	public ColorP X0Y0_尾24_尾CP;

    	public ColorP X0Y0_尾25_鱗CP;

    	public ColorP X0Y0_尾25_鱗左CP;

    	public ColorP X0Y0_尾25_鱗右CP;

    	public ColorP X0Y0_尾25_尾CP;

    	public ColorP X0Y0_尾26_鱗CP;

    	public ColorP X0Y0_尾26_鱗左CP;

    	public ColorP X0Y0_尾26_鱗右CP;

    	public ColorP X0Y0_尾26_尾CP;

    	public ColorP X0Y0_輪1_革CP;

    	public ColorP X0Y0_輪1_金具1CP;

    	public ColorP X0Y0_輪1_金具2CP;

    	public ColorP X0Y0_輪1_金具3CP;

    	public ColorP X0Y0_輪1_金具左CP;

    	public ColorP X0Y0_輪1_金具右CP;

    	public ColorP X0Y0_尾27_鱗CP;

    	public ColorP X0Y0_尾27_鱗左CP;

    	public ColorP X0Y0_尾27_鱗右CP;

    	public ColorP X0Y0_尾27_尾CP;

    	public ColorP X0Y0_尾28_鱗CP;

    	public ColorP X0Y0_尾28_鱗左CP;

    	public ColorP X0Y0_尾28_鱗右CP;

    	public ColorP X0Y0_尾28_尾CP;

    	public ColorP X0Y0_尾29_鱗CP;

    	public ColorP X0Y0_尾29_鱗左CP;

    	public ColorP X0Y0_尾29_鱗右CP;

    	public ColorP X0Y0_尾29_尾CP;

    	public ColorP X0Y0_尾30_鱗CP;

    	public ColorP X0Y0_尾30_鱗左CP;

    	public ColorP X0Y0_尾30_鱗右CP;

    	public ColorP X0Y0_尾30_尾CP;

    	public ColorP X0Y0_尾31_鱗CP;

    	public ColorP X0Y0_尾31_鱗左CP;

    	public ColorP X0Y0_尾31_鱗右CP;

    	public ColorP X0Y0_尾31_尾CP;

    	public ColorP X0Y0_尾32_鱗CP;

    	public ColorP X0Y0_尾32_鱗左CP;

    	public ColorP X0Y0_尾32_鱗右CP;

    	public ColorP X0Y0_尾32_尾CP;

    	public ColorP X0Y0_尾33_鱗CP;

    	public ColorP X0Y0_尾33_鱗左CP;

    	public ColorP X0Y0_尾33_鱗右CP;

    	public ColorP X0Y0_尾33_尾CP;

    	public ColorP X0Y0_尾34_鱗CP;

    	public ColorP X0Y0_尾34_鱗左CP;

    	public ColorP X0Y0_尾34_鱗右CP;

    	public ColorP X0Y0_尾34_尾CP;

    	public ColorP X0Y0_尾35_鱗CP;

    	public ColorP X0Y0_尾35_鱗左CP;

    	public ColorP X0Y0_尾35_鱗右CP;

    	public ColorP X0Y0_尾35_尾CP;

    	public ColorP X0Y0_尾36_鱗CP;

    	public ColorP X0Y0_尾36_鱗左CP;

    	public ColorP X0Y0_尾36_鱗右CP;

    	public ColorP X0Y0_尾36_尾CP;

    	public ColorP X0Y0_尾37_鱗CP;

    	public ColorP X0Y0_尾37_鱗左CP;

    	public ColorP X0Y0_尾37_鱗右CP;

    	public ColorP X0Y0_尾37_尾CP;

    	public ColorP X0Y0_尾38_鱗CP;

    	public ColorP X0Y0_尾38_鱗左CP;

    	public ColorP X0Y0_尾38_鱗右CP;

    	public ColorP X0Y0_尾38_尾CP;

    	public ColorP X0Y0_尾39_鱗CP;

    	public ColorP X0Y0_尾39_鱗左CP;

    	public ColorP X0Y0_尾39_鱗右CP;

    	public ColorP X0Y0_尾39_尾CP;

    	public ColorP X0Y0_尾40_鱗CP;

    	public ColorP X0Y0_尾40_鱗左CP;

    	public ColorP X0Y0_尾40_鱗右CP;

    	public ColorP X0Y0_尾40_尾CP;

    	public ColorP X0Y0_尾41_鱗CP;

    	public ColorP X0Y0_尾41_鱗左CP;

    	public ColorP X0Y0_尾41_鱗右CP;

    	public ColorP X0Y0_尾41_尾CP;

    	public ColorP X0Y0_尾42_鱗CP;

    	public ColorP X0Y0_尾42_鱗左CP;

    	public ColorP X0Y0_尾42_鱗右CP;

    	public ColorP X0Y0_尾42_尾CP;

    	public ColorP X0Y0_尾43_鱗CP;

    	public ColorP X0Y0_尾43_鱗左CP;

    	public ColorP X0Y0_尾43_鱗右CP;

    	public ColorP X0Y0_尾43_尾CP;

    	public ColorP X0Y0_尾44_鱗CP;

    	public ColorP X0Y0_尾44_鱗左CP;

    	public ColorP X0Y0_尾44_鱗右CP;

    	public ColorP X0Y0_尾44_尾CP;

    	public ColorP X0Y0_頭_上顎_顎基CP;

    	public ColorP X0Y0_頭_上顎_鱗4CP;

    	public ColorP X0Y0_頭_上顎_鱗左1CP;

    	public ColorP X0Y0_頭_上顎_鱗右1CP;

    	public ColorP X0Y0_頭_上顎_鱗左2CP;

    	public ColorP X0Y0_頭_上顎_鱗右2CP;

    	public ColorP X0Y0_頭_上顎_鱗左3CP;

    	public ColorP X0Y0_頭_上顎_鱗右3CP;

    	public ColorP X0Y0_頭_上顎_鱗左4CP;

    	public ColorP X0Y0_頭_上顎_鱗右4CP;

    	public ColorP X0Y0_頭_上顎_鱗左5CP;

    	public ColorP X0Y0_頭_上顎_鱗右5CP;

    	public ColorP X0Y0_頭_上顎_鱗左8CP;

    	public ColorP X0Y0_頭_上顎_鱗右8CP;

    	public ColorP X0Y0_頭_上顎_鱗左9CP;

    	public ColorP X0Y0_頭_上顎_鱗右9CP;

    	public ColorP X0Y0_頭_上顎_鱗左10CP;

    	public ColorP X0Y0_頭_上顎_鱗右10CP;

    	public ColorP X0Y0_頭_上顎_鱗左11CP;

    	public ColorP X0Y0_頭_上顎_鱗右11CP;

    	public ColorP X0Y0_頭_下顎_顎基CP;

    	public ColorP X0Y0_頭_下顎_鱗1CP;

    	public ColorP X0Y0_頭_下顎_鱗2CP;

    	public ColorP X0Y0_頭_下顎_鱗3CP;

    	public ColorP X0Y0_頭_下顎_鱗4CP;

    	public ColorP X0Y0_頭_下顎_鱗左1CP;

    	public ColorP X0Y0_頭_下顎_鱗右1CP;

    	public ColorP X0Y0_頭_下顎_鱗左2CP;

    	public ColorP X0Y0_頭_下顎_鱗右2CP;

    	public ColorP X0Y0_頭_下顎_鱗左3CP;

    	public ColorP X0Y0_頭_下顎_鱗右3CP;

    	public ColorP X0Y0_頭_下顎_鱗左4CP;

    	public ColorP X0Y0_頭_下顎_鱗右4CP;

    	public ColorP X0Y0_頭_下顎_鱗左5CP;

    	public ColorP X0Y0_頭_下顎_鱗右5CP;

    	public ColorP X0Y0_頭_下顎_鱗左6CP;

    	public ColorP X0Y0_頭_下顎_鱗右6CP;

    	public ColorP X0Y0_頭_下顎_鱗左7CP;

    	public ColorP X0Y0_頭_下顎_鱗右7CP;

    	public ColorP X0Y0_頭_下顎_鱗左8CP;

    	public ColorP X0Y0_頭_下顎_鱗右8CP;

    	public ColorP X0Y0_頭_下顎_鱗左9CP;

    	public ColorP X0Y0_頭_下顎_鱗右9CP;

    	public ColorP X0Y0_頭_下顎_鱗左10CP;

    	public ColorP X0Y0_頭_下顎_鱗右10CP;

    	public ColorP X0Y0_頭_下顎_鱗左11CP;

    	public ColorP X0Y0_頭_下顎_鱗右11CP;

    	public ColorP X0Y0_輪2_革CP;

    	public ColorP X0Y0_輪2_金具1CP;

    	public ColorP X0Y0_輪2_金具2CP;

    	public ColorP X0Y0_輪2_金具3CP;

    	public ColorP X0Y0_輪2_金具左CP;

    	public ColorP X0Y0_輪2_金具右CP;

    	public 拘束鎖 鎖1;

    	public 拘束鎖 鎖2;

    	public 拘束鎖 鎖3;

    	public 拘束鎖 鎖4;

    	public override bool 欠損
    	{
    		get
    		{
    			return 欠損_;
    		}
    		set
    		{
    			欠損_ = value;
    		}
    	}

    	public override bool 筋肉
    	{
    		get
    		{
    			return 筋肉_;
    		}
    		set
    		{
    			筋肉_ = value;
    		}
    	}

    	public override bool 拘束
    	{
    		get
    		{
    			return 拘束_;
    		}
    		set
    		{
    			拘束_ = value;
    			輪1表示 = 拘束_;
    			輪2表示 = 拘束_;
    		}
    	}

    	public bool 尾1_根_表示
    	{
    		get
    		{
    			return X0Y0_尾1_根.Dra;
    		}
    		set
    		{
    			X0Y0_尾1_根.Dra = value;
    			X0Y0_尾1_根.Hit = value;
    		}
    	}

    	public bool 尾1_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾1_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾1_鱗.Dra = value;
    			X0Y0_尾1_鱗.Hit = value;
    		}
    	}

    	public bool 尾1_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾1_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾1_鱗左.Dra = value;
    			X0Y0_尾1_鱗左.Hit = value;
    		}
    	}

    	public bool 尾1_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾1_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾1_鱗右.Dra = value;
    			X0Y0_尾1_鱗右.Hit = value;
    		}
    	}

    	public bool 尾1_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾1_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾1_尾.Dra = value;
    			X0Y0_尾1_尾.Hit = value;
    		}
    	}

    	public bool 尾2_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾2_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾2_鱗.Dra = value;
    			X0Y0_尾2_鱗.Hit = value;
    		}
    	}

    	public bool 尾2_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾2_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾2_鱗左.Dra = value;
    			X0Y0_尾2_鱗左.Hit = value;
    		}
    	}

    	public bool 尾2_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾2_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾2_鱗右.Dra = value;
    			X0Y0_尾2_鱗右.Hit = value;
    		}
    	}

    	public bool 尾2_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾2_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾2_尾.Dra = value;
    			X0Y0_尾2_尾.Hit = value;
    		}
    	}

    	public bool 尾3_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾3_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾3_鱗.Dra = value;
    			X0Y0_尾3_鱗.Hit = value;
    		}
    	}

    	public bool 尾3_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾3_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾3_鱗左.Dra = value;
    			X0Y0_尾3_鱗左.Hit = value;
    		}
    	}

    	public bool 尾3_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾3_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾3_鱗右.Dra = value;
    			X0Y0_尾3_鱗右.Hit = value;
    		}
    	}

    	public bool 尾3_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾3_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾3_尾.Dra = value;
    			X0Y0_尾3_尾.Hit = value;
    		}
    	}

    	public bool 尾4_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾4_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾4_鱗.Dra = value;
    			X0Y0_尾4_鱗.Hit = value;
    		}
    	}

    	public bool 尾4_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾4_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾4_鱗左.Dra = value;
    			X0Y0_尾4_鱗左.Hit = value;
    		}
    	}

    	public bool 尾4_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾4_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾4_鱗右.Dra = value;
    			X0Y0_尾4_鱗右.Hit = value;
    		}
    	}

    	public bool 尾4_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾4_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾4_尾.Dra = value;
    			X0Y0_尾4_尾.Hit = value;
    		}
    	}

    	public bool 尾5_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾5_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾5_鱗.Dra = value;
    			X0Y0_尾5_鱗.Hit = value;
    		}
    	}

    	public bool 尾5_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾5_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾5_鱗左.Dra = value;
    			X0Y0_尾5_鱗左.Hit = value;
    		}
    	}

    	public bool 尾5_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾5_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾5_鱗右.Dra = value;
    			X0Y0_尾5_鱗右.Hit = value;
    		}
    	}

    	public bool 尾5_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾5_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾5_尾.Dra = value;
    			X0Y0_尾5_尾.Hit = value;
    		}
    	}

    	public bool 尾6_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾6_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾6_鱗.Dra = value;
    			X0Y0_尾6_鱗.Hit = value;
    		}
    	}

    	public bool 尾6_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾6_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾6_鱗左.Dra = value;
    			X0Y0_尾6_鱗左.Hit = value;
    		}
    	}

    	public bool 尾6_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾6_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾6_鱗右.Dra = value;
    			X0Y0_尾6_鱗右.Hit = value;
    		}
    	}

    	public bool 尾6_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾6_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾6_尾.Dra = value;
    			X0Y0_尾6_尾.Hit = value;
    		}
    	}

    	public bool 尾7_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾7_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾7_鱗.Dra = value;
    			X0Y0_尾7_鱗.Hit = value;
    		}
    	}

    	public bool 尾7_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾7_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾7_鱗左.Dra = value;
    			X0Y0_尾7_鱗左.Hit = value;
    		}
    	}

    	public bool 尾7_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾7_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾7_鱗右.Dra = value;
    			X0Y0_尾7_鱗右.Hit = value;
    		}
    	}

    	public bool 尾7_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾7_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾7_尾.Dra = value;
    			X0Y0_尾7_尾.Hit = value;
    		}
    	}

    	public bool 尾8_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾8_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾8_鱗.Dra = value;
    			X0Y0_尾8_鱗.Hit = value;
    		}
    	}

    	public bool 尾8_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾8_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾8_鱗左.Dra = value;
    			X0Y0_尾8_鱗左.Hit = value;
    		}
    	}

    	public bool 尾8_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾8_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾8_鱗右.Dra = value;
    			X0Y0_尾8_鱗右.Hit = value;
    		}
    	}

    	public bool 尾8_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾8_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾8_尾.Dra = value;
    			X0Y0_尾8_尾.Hit = value;
    		}
    	}

    	public bool 尾9_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾9_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾9_鱗.Dra = value;
    			X0Y0_尾9_鱗.Hit = value;
    		}
    	}

    	public bool 尾9_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾9_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾9_鱗左.Dra = value;
    			X0Y0_尾9_鱗左.Hit = value;
    		}
    	}

    	public bool 尾9_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾9_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾9_鱗右.Dra = value;
    			X0Y0_尾9_鱗右.Hit = value;
    		}
    	}

    	public bool 尾9_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾9_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾9_尾.Dra = value;
    			X0Y0_尾9_尾.Hit = value;
    		}
    	}

    	public bool 尾10_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾10_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾10_鱗.Dra = value;
    			X0Y0_尾10_鱗.Hit = value;
    		}
    	}

    	public bool 尾10_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾10_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾10_鱗左.Dra = value;
    			X0Y0_尾10_鱗左.Hit = value;
    		}
    	}

    	public bool 尾10_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾10_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾10_鱗右.Dra = value;
    			X0Y0_尾10_鱗右.Hit = value;
    		}
    	}

    	public bool 尾10_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾10_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾10_尾.Dra = value;
    			X0Y0_尾10_尾.Hit = value;
    		}
    	}

    	public bool 尾11_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾11_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾11_鱗.Dra = value;
    			X0Y0_尾11_鱗.Hit = value;
    		}
    	}

    	public bool 尾11_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾11_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾11_鱗左.Dra = value;
    			X0Y0_尾11_鱗左.Hit = value;
    		}
    	}

    	public bool 尾11_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾11_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾11_鱗右.Dra = value;
    			X0Y0_尾11_鱗右.Hit = value;
    		}
    	}

    	public bool 尾11_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾11_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾11_尾.Dra = value;
    			X0Y0_尾11_尾.Hit = value;
    		}
    	}

    	public bool 尾12_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾12_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾12_鱗.Dra = value;
    			X0Y0_尾12_鱗.Hit = value;
    		}
    	}

    	public bool 尾12_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾12_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾12_鱗左.Dra = value;
    			X0Y0_尾12_鱗左.Hit = value;
    		}
    	}

    	public bool 尾12_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾12_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾12_鱗右.Dra = value;
    			X0Y0_尾12_鱗右.Hit = value;
    		}
    	}

    	public bool 尾12_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾12_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾12_尾.Dra = value;
    			X0Y0_尾12_尾.Hit = value;
    		}
    	}

    	public bool 尾13_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾13_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾13_鱗.Dra = value;
    			X0Y0_尾13_鱗.Hit = value;
    		}
    	}

    	public bool 尾13_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾13_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾13_鱗左.Dra = value;
    			X0Y0_尾13_鱗左.Hit = value;
    		}
    	}

    	public bool 尾13_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾13_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾13_鱗右.Dra = value;
    			X0Y0_尾13_鱗右.Hit = value;
    		}
    	}

    	public bool 尾13_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾13_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾13_尾.Dra = value;
    			X0Y0_尾13_尾.Hit = value;
    		}
    	}

    	public bool 尾14_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾14_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾14_鱗.Dra = value;
    			X0Y0_尾14_鱗.Hit = value;
    		}
    	}

    	public bool 尾14_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾14_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾14_鱗左.Dra = value;
    			X0Y0_尾14_鱗左.Hit = value;
    		}
    	}

    	public bool 尾14_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾14_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾14_鱗右.Dra = value;
    			X0Y0_尾14_鱗右.Hit = value;
    		}
    	}

    	public bool 尾14_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾14_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾14_尾.Dra = value;
    			X0Y0_尾14_尾.Hit = value;
    		}
    	}

    	public bool 尾15_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾15_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾15_鱗.Dra = value;
    			X0Y0_尾15_鱗.Hit = value;
    		}
    	}

    	public bool 尾15_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾15_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾15_鱗左.Dra = value;
    			X0Y0_尾15_鱗左.Hit = value;
    		}
    	}

    	public bool 尾15_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾15_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾15_鱗右.Dra = value;
    			X0Y0_尾15_鱗右.Hit = value;
    		}
    	}

    	public bool 尾15_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾15_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾15_尾.Dra = value;
    			X0Y0_尾15_尾.Hit = value;
    		}
    	}

    	public bool 尾16_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾16_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾16_鱗.Dra = value;
    			X0Y0_尾16_鱗.Hit = value;
    		}
    	}

    	public bool 尾16_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾16_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾16_鱗左.Dra = value;
    			X0Y0_尾16_鱗左.Hit = value;
    		}
    	}

    	public bool 尾16_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾16_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾16_鱗右.Dra = value;
    			X0Y0_尾16_鱗右.Hit = value;
    		}
    	}

    	public bool 尾16_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾16_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾16_尾.Dra = value;
    			X0Y0_尾16_尾.Hit = value;
    		}
    	}

    	public bool 尾17_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾17_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾17_鱗.Dra = value;
    			X0Y0_尾17_鱗.Hit = value;
    		}
    	}

    	public bool 尾17_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾17_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾17_鱗左.Dra = value;
    			X0Y0_尾17_鱗左.Hit = value;
    		}
    	}

    	public bool 尾17_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾17_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾17_鱗右.Dra = value;
    			X0Y0_尾17_鱗右.Hit = value;
    		}
    	}

    	public bool 尾17_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾17_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾17_尾.Dra = value;
    			X0Y0_尾17_尾.Hit = value;
    		}
    	}

    	public bool 尾18_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾18_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾18_鱗.Dra = value;
    			X0Y0_尾18_鱗.Hit = value;
    		}
    	}

    	public bool 尾18_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾18_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾18_鱗左.Dra = value;
    			X0Y0_尾18_鱗左.Hit = value;
    		}
    	}

    	public bool 尾18_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾18_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾18_鱗右.Dra = value;
    			X0Y0_尾18_鱗右.Hit = value;
    		}
    	}

    	public bool 尾18_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾18_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾18_尾.Dra = value;
    			X0Y0_尾18_尾.Hit = value;
    		}
    	}

    	public bool 尾19_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾19_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾19_鱗.Dra = value;
    			X0Y0_尾19_鱗.Hit = value;
    		}
    	}

    	public bool 尾19_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾19_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾19_鱗左.Dra = value;
    			X0Y0_尾19_鱗左.Hit = value;
    		}
    	}

    	public bool 尾19_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾19_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾19_鱗右.Dra = value;
    			X0Y0_尾19_鱗右.Hit = value;
    		}
    	}

    	public bool 尾19_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾19_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾19_尾.Dra = value;
    			X0Y0_尾19_尾.Hit = value;
    		}
    	}

    	public bool 尾20_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾20_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾20_鱗.Dra = value;
    			X0Y0_尾20_鱗.Hit = value;
    		}
    	}

    	public bool 尾20_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾20_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾20_鱗左.Dra = value;
    			X0Y0_尾20_鱗左.Hit = value;
    		}
    	}

    	public bool 尾20_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾20_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾20_鱗右.Dra = value;
    			X0Y0_尾20_鱗右.Hit = value;
    		}
    	}

    	public bool 尾20_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾20_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾20_尾.Dra = value;
    			X0Y0_尾20_尾.Hit = value;
    		}
    	}

    	public bool 尾21_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾21_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾21_鱗.Dra = value;
    			X0Y0_尾21_鱗.Hit = value;
    		}
    	}

    	public bool 尾21_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾21_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾21_鱗左.Dra = value;
    			X0Y0_尾21_鱗左.Hit = value;
    		}
    	}

    	public bool 尾21_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾21_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾21_鱗右.Dra = value;
    			X0Y0_尾21_鱗右.Hit = value;
    		}
    	}

    	public bool 尾21_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾21_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾21_尾.Dra = value;
    			X0Y0_尾21_尾.Hit = value;
    		}
    	}

    	public bool 尾22_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾22_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾22_鱗.Dra = value;
    			X0Y0_尾22_鱗.Hit = value;
    		}
    	}

    	public bool 尾22_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾22_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾22_鱗左.Dra = value;
    			X0Y0_尾22_鱗左.Hit = value;
    		}
    	}

    	public bool 尾22_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾22_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾22_鱗右.Dra = value;
    			X0Y0_尾22_鱗右.Hit = value;
    		}
    	}

    	public bool 尾22_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾22_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾22_尾.Dra = value;
    			X0Y0_尾22_尾.Hit = value;
    		}
    	}

    	public bool 尾23_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾23_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾23_鱗.Dra = value;
    			X0Y0_尾23_鱗.Hit = value;
    		}
    	}

    	public bool 尾23_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾23_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾23_鱗左.Dra = value;
    			X0Y0_尾23_鱗左.Hit = value;
    		}
    	}

    	public bool 尾23_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾23_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾23_鱗右.Dra = value;
    			X0Y0_尾23_鱗右.Hit = value;
    		}
    	}

    	public bool 尾23_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾23_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾23_尾.Dra = value;
    			X0Y0_尾23_尾.Hit = value;
    		}
    	}

    	public bool 尾24_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾24_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾24_鱗.Dra = value;
    			X0Y0_尾24_鱗.Hit = value;
    		}
    	}

    	public bool 尾24_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾24_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾24_鱗左.Dra = value;
    			X0Y0_尾24_鱗左.Hit = value;
    		}
    	}

    	public bool 尾24_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾24_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾24_鱗右.Dra = value;
    			X0Y0_尾24_鱗右.Hit = value;
    		}
    	}

    	public bool 尾24_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾24_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾24_尾.Dra = value;
    			X0Y0_尾24_尾.Hit = value;
    		}
    	}

    	public bool 尾25_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾25_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾25_鱗.Dra = value;
    			X0Y0_尾25_鱗.Hit = value;
    		}
    	}

    	public bool 尾25_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾25_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾25_鱗左.Dra = value;
    			X0Y0_尾25_鱗左.Hit = value;
    		}
    	}

    	public bool 尾25_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾25_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾25_鱗右.Dra = value;
    			X0Y0_尾25_鱗右.Hit = value;
    		}
    	}

    	public bool 尾25_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾25_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾25_尾.Dra = value;
    			X0Y0_尾25_尾.Hit = value;
    		}
    	}

    	public bool 尾26_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾26_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾26_鱗.Dra = value;
    			X0Y0_尾26_鱗.Hit = value;
    		}
    	}

    	public bool 尾26_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾26_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾26_鱗左.Dra = value;
    			X0Y0_尾26_鱗左.Hit = value;
    		}
    	}

    	public bool 尾26_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾26_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾26_鱗右.Dra = value;
    			X0Y0_尾26_鱗右.Hit = value;
    		}
    	}

    	public bool 尾26_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾26_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾26_尾.Dra = value;
    			X0Y0_尾26_尾.Hit = value;
    		}
    	}

    	public bool 輪1_革_表示
    	{
    		get
    		{
    			return X0Y0_輪1_革.Dra;
    		}
    		set
    		{
    			X0Y0_輪1_革.Dra = value;
    			X0Y0_輪1_革.Hit = value;
    		}
    	}

    	public bool 輪1_金具1_表示
    	{
    		get
    		{
    			return X0Y0_輪1_金具1.Dra;
    		}
    		set
    		{
    			X0Y0_輪1_金具1.Dra = value;
    			X0Y0_輪1_金具1.Hit = value;
    		}
    	}

    	public bool 輪1_金具2_表示
    	{
    		get
    		{
    			return X0Y0_輪1_金具2.Dra;
    		}
    		set
    		{
    			X0Y0_輪1_金具2.Dra = value;
    			X0Y0_輪1_金具2.Hit = value;
    		}
    	}

    	public bool 輪1_金具3_表示
    	{
    		get
    		{
    			return X0Y0_輪1_金具3.Dra;
    		}
    		set
    		{
    			X0Y0_輪1_金具3.Dra = value;
    			X0Y0_輪1_金具3.Hit = value;
    		}
    	}

    	public bool 輪1_金具左_表示
    	{
    		get
    		{
    			return X0Y0_輪1_金具左.Dra;
    		}
    		set
    		{
    			X0Y0_輪1_金具左.Dra = value;
    			X0Y0_輪1_金具左.Hit = value;
    		}
    	}

    	public bool 輪1_金具右_表示
    	{
    		get
    		{
    			return X0Y0_輪1_金具右.Dra;
    		}
    		set
    		{
    			X0Y0_輪1_金具右.Dra = value;
    			X0Y0_輪1_金具右.Hit = value;
    		}
    	}

    	public bool 尾27_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾27_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾27_鱗.Dra = value;
    			X0Y0_尾27_鱗.Hit = value;
    		}
    	}

    	public bool 尾27_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾27_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾27_鱗左.Dra = value;
    			X0Y0_尾27_鱗左.Hit = value;
    		}
    	}

    	public bool 尾27_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾27_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾27_鱗右.Dra = value;
    			X0Y0_尾27_鱗右.Hit = value;
    		}
    	}

    	public bool 尾27_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾27_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾27_尾.Dra = value;
    			X0Y0_尾27_尾.Hit = value;
    		}
    	}

    	public bool 尾28_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾28_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾28_鱗.Dra = value;
    			X0Y0_尾28_鱗.Hit = value;
    		}
    	}

    	public bool 尾28_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾28_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾28_鱗左.Dra = value;
    			X0Y0_尾28_鱗左.Hit = value;
    		}
    	}

    	public bool 尾28_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾28_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾28_鱗右.Dra = value;
    			X0Y0_尾28_鱗右.Hit = value;
    		}
    	}

    	public bool 尾28_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾28_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾28_尾.Dra = value;
    			X0Y0_尾28_尾.Hit = value;
    		}
    	}

    	public bool 尾29_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾29_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾29_鱗.Dra = value;
    			X0Y0_尾29_鱗.Hit = value;
    		}
    	}

    	public bool 尾29_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾29_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾29_鱗左.Dra = value;
    			X0Y0_尾29_鱗左.Hit = value;
    		}
    	}

    	public bool 尾29_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾29_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾29_鱗右.Dra = value;
    			X0Y0_尾29_鱗右.Hit = value;
    		}
    	}

    	public bool 尾29_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾29_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾29_尾.Dra = value;
    			X0Y0_尾29_尾.Hit = value;
    		}
    	}

    	public bool 尾30_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾30_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾30_鱗.Dra = value;
    			X0Y0_尾30_鱗.Hit = value;
    		}
    	}

    	public bool 尾30_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾30_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾30_鱗左.Dra = value;
    			X0Y0_尾30_鱗左.Hit = value;
    		}
    	}

    	public bool 尾30_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾30_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾30_鱗右.Dra = value;
    			X0Y0_尾30_鱗右.Hit = value;
    		}
    	}

    	public bool 尾30_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾30_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾30_尾.Dra = value;
    			X0Y0_尾30_尾.Hit = value;
    		}
    	}

    	public bool 尾31_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾31_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾31_鱗.Dra = value;
    			X0Y0_尾31_鱗.Hit = value;
    		}
    	}

    	public bool 尾31_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾31_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾31_鱗左.Dra = value;
    			X0Y0_尾31_鱗左.Hit = value;
    		}
    	}

    	public bool 尾31_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾31_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾31_鱗右.Dra = value;
    			X0Y0_尾31_鱗右.Hit = value;
    		}
    	}

    	public bool 尾31_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾31_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾31_尾.Dra = value;
    			X0Y0_尾31_尾.Hit = value;
    		}
    	}

    	public bool 尾32_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾32_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾32_鱗.Dra = value;
    			X0Y0_尾32_鱗.Hit = value;
    		}
    	}

    	public bool 尾32_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾32_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾32_鱗左.Dra = value;
    			X0Y0_尾32_鱗左.Hit = value;
    		}
    	}

    	public bool 尾32_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾32_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾32_鱗右.Dra = value;
    			X0Y0_尾32_鱗右.Hit = value;
    		}
    	}

    	public bool 尾32_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾32_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾32_尾.Dra = value;
    			X0Y0_尾32_尾.Hit = value;
    		}
    	}

    	public bool 尾33_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾33_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾33_鱗.Dra = value;
    			X0Y0_尾33_鱗.Hit = value;
    		}
    	}

    	public bool 尾33_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾33_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾33_鱗左.Dra = value;
    			X0Y0_尾33_鱗左.Hit = value;
    		}
    	}

    	public bool 尾33_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾33_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾33_鱗右.Dra = value;
    			X0Y0_尾33_鱗右.Hit = value;
    		}
    	}

    	public bool 尾33_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾33_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾33_尾.Dra = value;
    			X0Y0_尾33_尾.Hit = value;
    		}
    	}

    	public bool 尾34_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾34_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾34_鱗.Dra = value;
    			X0Y0_尾34_鱗.Hit = value;
    		}
    	}

    	public bool 尾34_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾34_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾34_鱗左.Dra = value;
    			X0Y0_尾34_鱗左.Hit = value;
    		}
    	}

    	public bool 尾34_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾34_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾34_鱗右.Dra = value;
    			X0Y0_尾34_鱗右.Hit = value;
    		}
    	}

    	public bool 尾34_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾34_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾34_尾.Dra = value;
    			X0Y0_尾34_尾.Hit = value;
    		}
    	}

    	public bool 尾35_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾35_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾35_鱗.Dra = value;
    			X0Y0_尾35_鱗.Hit = value;
    		}
    	}

    	public bool 尾35_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾35_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾35_鱗左.Dra = value;
    			X0Y0_尾35_鱗左.Hit = value;
    		}
    	}

    	public bool 尾35_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾35_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾35_鱗右.Dra = value;
    			X0Y0_尾35_鱗右.Hit = value;
    		}
    	}

    	public bool 尾35_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾35_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾35_尾.Dra = value;
    			X0Y0_尾35_尾.Hit = value;
    		}
    	}

    	public bool 尾36_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾36_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾36_鱗.Dra = value;
    			X0Y0_尾36_鱗.Hit = value;
    		}
    	}

    	public bool 尾36_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾36_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾36_鱗左.Dra = value;
    			X0Y0_尾36_鱗左.Hit = value;
    		}
    	}

    	public bool 尾36_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾36_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾36_鱗右.Dra = value;
    			X0Y0_尾36_鱗右.Hit = value;
    		}
    	}

    	public bool 尾36_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾36_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾36_尾.Dra = value;
    			X0Y0_尾36_尾.Hit = value;
    		}
    	}

    	public bool 尾37_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾37_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾37_鱗.Dra = value;
    			X0Y0_尾37_鱗.Hit = value;
    		}
    	}

    	public bool 尾37_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾37_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾37_鱗左.Dra = value;
    			X0Y0_尾37_鱗左.Hit = value;
    		}
    	}

    	public bool 尾37_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾37_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾37_鱗右.Dra = value;
    			X0Y0_尾37_鱗右.Hit = value;
    		}
    	}

    	public bool 尾37_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾37_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾37_尾.Dra = value;
    			X0Y0_尾37_尾.Hit = value;
    		}
    	}

    	public bool 尾38_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾38_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾38_鱗.Dra = value;
    			X0Y0_尾38_鱗.Hit = value;
    		}
    	}

    	public bool 尾38_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾38_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾38_鱗左.Dra = value;
    			X0Y0_尾38_鱗左.Hit = value;
    		}
    	}

    	public bool 尾38_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾38_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾38_鱗右.Dra = value;
    			X0Y0_尾38_鱗右.Hit = value;
    		}
    	}

    	public bool 尾38_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾38_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾38_尾.Dra = value;
    			X0Y0_尾38_尾.Hit = value;
    		}
    	}

    	public bool 尾39_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾39_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾39_鱗.Dra = value;
    			X0Y0_尾39_鱗.Hit = value;
    		}
    	}

    	public bool 尾39_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾39_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾39_鱗左.Dra = value;
    			X0Y0_尾39_鱗左.Hit = value;
    		}
    	}

    	public bool 尾39_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾39_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾39_鱗右.Dra = value;
    			X0Y0_尾39_鱗右.Hit = value;
    		}
    	}

    	public bool 尾39_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾39_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾39_尾.Dra = value;
    			X0Y0_尾39_尾.Hit = value;
    		}
    	}

    	public bool 尾40_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾40_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾40_鱗.Dra = value;
    			X0Y0_尾40_鱗.Hit = value;
    		}
    	}

    	public bool 尾40_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾40_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾40_鱗左.Dra = value;
    			X0Y0_尾40_鱗左.Hit = value;
    		}
    	}

    	public bool 尾40_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾40_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾40_鱗右.Dra = value;
    			X0Y0_尾40_鱗右.Hit = value;
    		}
    	}

    	public bool 尾40_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾40_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾40_尾.Dra = value;
    			X0Y0_尾40_尾.Hit = value;
    		}
    	}

    	public bool 尾41_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾41_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾41_鱗.Dra = value;
    			X0Y0_尾41_鱗.Hit = value;
    		}
    	}

    	public bool 尾41_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾41_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾41_鱗左.Dra = value;
    			X0Y0_尾41_鱗左.Hit = value;
    		}
    	}

    	public bool 尾41_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾41_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾41_鱗右.Dra = value;
    			X0Y0_尾41_鱗右.Hit = value;
    		}
    	}

    	public bool 尾41_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾41_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾41_尾.Dra = value;
    			X0Y0_尾41_尾.Hit = value;
    		}
    	}

    	public bool 尾42_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾42_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾42_鱗.Dra = value;
    			X0Y0_尾42_鱗.Hit = value;
    		}
    	}

    	public bool 尾42_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾42_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾42_鱗左.Dra = value;
    			X0Y0_尾42_鱗左.Hit = value;
    		}
    	}

    	public bool 尾42_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾42_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾42_鱗右.Dra = value;
    			X0Y0_尾42_鱗右.Hit = value;
    		}
    	}

    	public bool 尾42_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾42_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾42_尾.Dra = value;
    			X0Y0_尾42_尾.Hit = value;
    		}
    	}

    	public bool 尾43_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾43_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾43_鱗.Dra = value;
    			X0Y0_尾43_鱗.Hit = value;
    		}
    	}

    	public bool 尾43_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾43_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾43_鱗左.Dra = value;
    			X0Y0_尾43_鱗左.Hit = value;
    		}
    	}

    	public bool 尾43_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾43_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾43_鱗右.Dra = value;
    			X0Y0_尾43_鱗右.Hit = value;
    		}
    	}

    	public bool 尾43_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾43_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾43_尾.Dra = value;
    			X0Y0_尾43_尾.Hit = value;
    		}
    	}

    	public bool 尾44_鱗_表示
    	{
    		get
    		{
    			return X0Y0_尾44_鱗.Dra;
    		}
    		set
    		{
    			X0Y0_尾44_鱗.Dra = value;
    			X0Y0_尾44_鱗.Hit = value;
    		}
    	}

    	public bool 尾44_鱗左_表示
    	{
    		get
    		{
    			return X0Y0_尾44_鱗左.Dra;
    		}
    		set
    		{
    			X0Y0_尾44_鱗左.Dra = value;
    			X0Y0_尾44_鱗左.Hit = value;
    		}
    	}

    	public bool 尾44_鱗右_表示
    	{
    		get
    		{
    			return X0Y0_尾44_鱗右.Dra;
    		}
    		set
    		{
    			X0Y0_尾44_鱗右.Dra = value;
    			X0Y0_尾44_鱗右.Hit = value;
    		}
    	}

    	public bool 尾44_尾_表示
    	{
    		get
    		{
    			return X0Y0_尾44_尾.Dra;
    		}
    		set
    		{
    			X0Y0_尾44_尾.Dra = value;
    			X0Y0_尾44_尾.Hit = value;
    		}
    	}

    	public bool 頭_上顎_顎基_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_顎基.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_顎基.Dra = value;
    			X0Y0_頭_上顎_顎基.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗4_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗4.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗4.Dra = value;
    			X0Y0_頭_上顎_鱗4.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗左1_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗左1.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗左1.Dra = value;
    			X0Y0_頭_上顎_鱗左1.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗右1_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗右1.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗右1.Dra = value;
    			X0Y0_頭_上顎_鱗右1.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗左2_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗左2.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗左2.Dra = value;
    			X0Y0_頭_上顎_鱗左2.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗右2_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗右2.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗右2.Dra = value;
    			X0Y0_頭_上顎_鱗右2.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗左3_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗左3.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗左3.Dra = value;
    			X0Y0_頭_上顎_鱗左3.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗右3_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗右3.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗右3.Dra = value;
    			X0Y0_頭_上顎_鱗右3.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗左4_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗左4.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗左4.Dra = value;
    			X0Y0_頭_上顎_鱗左4.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗右4_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗右4.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗右4.Dra = value;
    			X0Y0_頭_上顎_鱗右4.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗左5_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗左5.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗左5.Dra = value;
    			X0Y0_頭_上顎_鱗左5.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗右5_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗右5.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗右5.Dra = value;
    			X0Y0_頭_上顎_鱗右5.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗左8_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗左8.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗左8.Dra = value;
    			X0Y0_頭_上顎_鱗左8.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗右8_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗右8.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗右8.Dra = value;
    			X0Y0_頭_上顎_鱗右8.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗左9_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗左9.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗左9.Dra = value;
    			X0Y0_頭_上顎_鱗左9.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗右9_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗右9.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗右9.Dra = value;
    			X0Y0_頭_上顎_鱗右9.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗左10_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗左10.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗左10.Dra = value;
    			X0Y0_頭_上顎_鱗左10.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗右10_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗右10.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗右10.Dra = value;
    			X0Y0_頭_上顎_鱗右10.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗左11_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗左11.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗左11.Dra = value;
    			X0Y0_頭_上顎_鱗左11.Hit = value;
    		}
    	}

    	public bool 頭_上顎_鱗右11_表示
    	{
    		get
    		{
    			return X0Y0_頭_上顎_鱗右11.Dra;
    		}
    		set
    		{
    			X0Y0_頭_上顎_鱗右11.Dra = value;
    			X0Y0_頭_上顎_鱗右11.Hit = value;
    		}
    	}

    	public bool 頭_下顎_顎基_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_顎基.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_顎基.Dra = value;
    			X0Y0_頭_下顎_顎基.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗1_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗1.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗1.Dra = value;
    			X0Y0_頭_下顎_鱗1.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗2_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗2.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗2.Dra = value;
    			X0Y0_頭_下顎_鱗2.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗3_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗3.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗3.Dra = value;
    			X0Y0_頭_下顎_鱗3.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗4_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗4.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗4.Dra = value;
    			X0Y0_頭_下顎_鱗4.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗左1_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗左1.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗左1.Dra = value;
    			X0Y0_頭_下顎_鱗左1.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗右1_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗右1.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗右1.Dra = value;
    			X0Y0_頭_下顎_鱗右1.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗左2_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗左2.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗左2.Dra = value;
    			X0Y0_頭_下顎_鱗左2.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗右2_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗右2.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗右2.Dra = value;
    			X0Y0_頭_下顎_鱗右2.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗左3_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗左3.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗左3.Dra = value;
    			X0Y0_頭_下顎_鱗左3.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗右3_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗右3.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗右3.Dra = value;
    			X0Y0_頭_下顎_鱗右3.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗左4_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗左4.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗左4.Dra = value;
    			X0Y0_頭_下顎_鱗左4.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗右4_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗右4.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗右4.Dra = value;
    			X0Y0_頭_下顎_鱗右4.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗左5_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗左5.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗左5.Dra = value;
    			X0Y0_頭_下顎_鱗左5.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗右5_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗右5.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗右5.Dra = value;
    			X0Y0_頭_下顎_鱗右5.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗左6_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗左6.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗左6.Dra = value;
    			X0Y0_頭_下顎_鱗左6.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗右6_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗右6.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗右6.Dra = value;
    			X0Y0_頭_下顎_鱗右6.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗左7_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗左7.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗左7.Dra = value;
    			X0Y0_頭_下顎_鱗左7.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗右7_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗右7.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗右7.Dra = value;
    			X0Y0_頭_下顎_鱗右7.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗左8_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗左8.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗左8.Dra = value;
    			X0Y0_頭_下顎_鱗左8.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗右8_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗右8.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗右8.Dra = value;
    			X0Y0_頭_下顎_鱗右8.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗左9_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗左9.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗左9.Dra = value;
    			X0Y0_頭_下顎_鱗左9.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗右9_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗右9.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗右9.Dra = value;
    			X0Y0_頭_下顎_鱗右9.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗左10_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗左10.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗左10.Dra = value;
    			X0Y0_頭_下顎_鱗左10.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗右10_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗右10.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗右10.Dra = value;
    			X0Y0_頭_下顎_鱗右10.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗左11_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗左11.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗左11.Dra = value;
    			X0Y0_頭_下顎_鱗左11.Hit = value;
    		}
    	}

    	public bool 頭_下顎_鱗右11_表示
    	{
    		get
    		{
    			return X0Y0_頭_下顎_鱗右11.Dra;
    		}
    		set
    		{
    			X0Y0_頭_下顎_鱗右11.Dra = value;
    			X0Y0_頭_下顎_鱗右11.Hit = value;
    		}
    	}

    	public bool 輪2_革_表示
    	{
    		get
    		{
    			return X0Y0_輪2_革.Dra;
    		}
    		set
    		{
    			X0Y0_輪2_革.Dra = value;
    			X0Y0_輪2_革.Hit = value;
    		}
    	}

    	public bool 輪2_金具1_表示
    	{
    		get
    		{
    			return X0Y0_輪2_金具1.Dra;
    		}
    		set
    		{
    			X0Y0_輪2_金具1.Dra = value;
    			X0Y0_輪2_金具1.Hit = value;
    		}
    	}

    	public bool 輪2_金具2_表示
    	{
    		get
    		{
    			return X0Y0_輪2_金具2.Dra;
    		}
    		set
    		{
    			X0Y0_輪2_金具2.Dra = value;
    			X0Y0_輪2_金具2.Hit = value;
    		}
    	}

    	public bool 輪2_金具3_表示
    	{
    		get
    		{
    			return X0Y0_輪2_金具3.Dra;
    		}
    		set
    		{
    			X0Y0_輪2_金具3.Dra = value;
    			X0Y0_輪2_金具3.Hit = value;
    		}
    	}

    	public bool 輪2_金具左_表示
    	{
    		get
    		{
    			return X0Y0_輪2_金具左.Dra;
    		}
    		set
    		{
    			X0Y0_輪2_金具左.Dra = value;
    			X0Y0_輪2_金具左.Hit = value;
    		}
    	}

    	public bool 輪2_金具右_表示
    	{
    		get
    		{
    			return X0Y0_輪2_金具右.Dra;
    		}
    		set
    		{
    			X0Y0_輪2_金具右.Dra = value;
    			X0Y0_輪2_金具右.Hit = value;
    		}
    	}

    	public bool 輪1表示
    	{
    		get
    		{
    			return 輪1_革_表示;
    		}
    		set
    		{
    			輪1_革_表示 = value;
    			輪1_金具1_表示 = value;
    			輪1_金具2_表示 = value;
    			輪1_金具3_表示 = value;
    			輪1_金具左_表示 = value;
    			輪1_金具右_表示 = value;
    		}
    	}

    	public bool 輪2表示
    	{
    		get
    		{
    			return 輪2_革_表示;
    		}
    		set
    		{
    			輪2_革_表示 = value;
    			輪2_金具1_表示 = value;
    			輪2_金具2_表示 = value;
    			輪2_金具3_表示 = value;
    			輪2_金具左_表示 = value;
    			輪2_金具右_表示 = value;
    		}
    	}

    	public bool 鎖表示
    	{
    		get
    		{
    			return 鎖1.表示;
    		}
    		set
    		{
    			鎖1.表示 = value;
    			鎖2.表示 = value;
    			鎖3.表示 = value;
    			鎖4.表示 = value;
    		}
    	}

    	public override bool 表示
    	{
    		get
    		{
    			return 尾1_根_表示;
    		}
    		set
    		{
    			尾1_根_表示 = value;
    			尾1_鱗_表示 = value;
    			尾1_鱗左_表示 = value;
    			尾1_鱗右_表示 = value;
    			尾1_尾_表示 = value;
    			尾2_鱗_表示 = value;
    			尾2_鱗左_表示 = value;
    			尾2_鱗右_表示 = value;
    			尾2_尾_表示 = value;
    			尾3_鱗_表示 = value;
    			尾3_鱗左_表示 = value;
    			尾3_鱗右_表示 = value;
    			尾3_尾_表示 = value;
    			尾4_鱗_表示 = value;
    			尾4_鱗左_表示 = value;
    			尾4_鱗右_表示 = value;
    			尾4_尾_表示 = value;
    			尾5_鱗_表示 = value;
    			尾5_鱗左_表示 = value;
    			尾5_鱗右_表示 = value;
    			尾5_尾_表示 = value;
    			尾6_鱗_表示 = value;
    			尾6_鱗左_表示 = value;
    			尾6_鱗右_表示 = value;
    			尾6_尾_表示 = value;
    			尾7_鱗_表示 = value;
    			尾7_鱗左_表示 = value;
    			尾7_鱗右_表示 = value;
    			尾7_尾_表示 = value;
    			尾8_鱗_表示 = value;
    			尾8_鱗左_表示 = value;
    			尾8_鱗右_表示 = value;
    			尾8_尾_表示 = value;
    			尾9_鱗_表示 = value;
    			尾9_鱗左_表示 = value;
    			尾9_鱗右_表示 = value;
    			尾9_尾_表示 = value;
    			尾10_鱗_表示 = value;
    			尾10_鱗左_表示 = value;
    			尾10_鱗右_表示 = value;
    			尾10_尾_表示 = value;
    			尾11_鱗_表示 = value;
    			尾11_鱗左_表示 = value;
    			尾11_鱗右_表示 = value;
    			尾11_尾_表示 = value;
    			尾12_鱗_表示 = value;
    			尾12_鱗左_表示 = value;
    			尾12_鱗右_表示 = value;
    			尾12_尾_表示 = value;
    			尾13_鱗_表示 = value;
    			尾13_鱗左_表示 = value;
    			尾13_鱗右_表示 = value;
    			尾13_尾_表示 = value;
    			尾14_鱗_表示 = value;
    			尾14_鱗左_表示 = value;
    			尾14_鱗右_表示 = value;
    			尾14_尾_表示 = value;
    			尾15_鱗_表示 = value;
    			尾15_鱗左_表示 = value;
    			尾15_鱗右_表示 = value;
    			尾15_尾_表示 = value;
    			尾16_鱗_表示 = value;
    			尾16_鱗左_表示 = value;
    			尾16_鱗右_表示 = value;
    			尾16_尾_表示 = value;
    			尾17_鱗_表示 = value;
    			尾17_鱗左_表示 = value;
    			尾17_鱗右_表示 = value;
    			尾17_尾_表示 = value;
    			尾18_鱗_表示 = value;
    			尾18_鱗左_表示 = value;
    			尾18_鱗右_表示 = value;
    			尾18_尾_表示 = value;
    			尾19_鱗_表示 = value;
    			尾19_鱗左_表示 = value;
    			尾19_鱗右_表示 = value;
    			尾19_尾_表示 = value;
    			尾20_鱗_表示 = value;
    			尾20_鱗左_表示 = value;
    			尾20_鱗右_表示 = value;
    			尾20_尾_表示 = value;
    			尾21_鱗_表示 = value;
    			尾21_鱗左_表示 = value;
    			尾21_鱗右_表示 = value;
    			尾21_尾_表示 = value;
    			尾22_鱗_表示 = value;
    			尾22_鱗左_表示 = value;
    			尾22_鱗右_表示 = value;
    			尾22_尾_表示 = value;
    			尾23_鱗_表示 = value;
    			尾23_鱗左_表示 = value;
    			尾23_鱗右_表示 = value;
    			尾23_尾_表示 = value;
    			尾24_鱗_表示 = value;
    			尾24_鱗左_表示 = value;
    			尾24_鱗右_表示 = value;
    			尾24_尾_表示 = value;
    			尾25_鱗_表示 = value;
    			尾25_鱗左_表示 = value;
    			尾25_鱗右_表示 = value;
    			尾25_尾_表示 = value;
    			尾26_鱗_表示 = value;
    			尾26_鱗左_表示 = value;
    			尾26_鱗右_表示 = value;
    			尾26_尾_表示 = value;
    			輪1_革_表示 = value;
    			輪1_金具1_表示 = value;
    			輪1_金具2_表示 = value;
    			輪1_金具3_表示 = value;
    			輪1_金具左_表示 = value;
    			輪1_金具右_表示 = value;
    			尾27_鱗_表示 = value;
    			尾27_鱗左_表示 = value;
    			尾27_鱗右_表示 = value;
    			尾27_尾_表示 = value;
    			尾28_鱗_表示 = value;
    			尾28_鱗左_表示 = value;
    			尾28_鱗右_表示 = value;
    			尾28_尾_表示 = value;
    			尾29_鱗_表示 = value;
    			尾29_鱗左_表示 = value;
    			尾29_鱗右_表示 = value;
    			尾29_尾_表示 = value;
    			尾30_鱗_表示 = value;
    			尾30_鱗左_表示 = value;
    			尾30_鱗右_表示 = value;
    			尾30_尾_表示 = value;
    			尾31_鱗_表示 = value;
    			尾31_鱗左_表示 = value;
    			尾31_鱗右_表示 = value;
    			尾31_尾_表示 = value;
    			尾32_鱗_表示 = value;
    			尾32_鱗左_表示 = value;
    			尾32_鱗右_表示 = value;
    			尾32_尾_表示 = value;
    			尾33_鱗_表示 = value;
    			尾33_鱗左_表示 = value;
    			尾33_鱗右_表示 = value;
    			尾33_尾_表示 = value;
    			尾34_鱗_表示 = value;
    			尾34_鱗左_表示 = value;
    			尾34_鱗右_表示 = value;
    			尾34_尾_表示 = value;
    			尾35_鱗_表示 = value;
    			尾35_鱗左_表示 = value;
    			尾35_鱗右_表示 = value;
    			尾35_尾_表示 = value;
    			尾36_鱗_表示 = value;
    			尾36_鱗左_表示 = value;
    			尾36_鱗右_表示 = value;
    			尾36_尾_表示 = value;
    			尾37_鱗_表示 = value;
    			尾37_鱗左_表示 = value;
    			尾37_鱗右_表示 = value;
    			尾37_尾_表示 = value;
    			尾38_鱗_表示 = value;
    			尾38_鱗左_表示 = value;
    			尾38_鱗右_表示 = value;
    			尾38_尾_表示 = value;
    			尾39_鱗_表示 = value;
    			尾39_鱗左_表示 = value;
    			尾39_鱗右_表示 = value;
    			尾39_尾_表示 = value;
    			尾40_鱗_表示 = value;
    			尾40_鱗左_表示 = value;
    			尾40_鱗右_表示 = value;
    			尾40_尾_表示 = value;
    			尾41_鱗_表示 = value;
    			尾41_鱗左_表示 = value;
    			尾41_鱗右_表示 = value;
    			尾41_尾_表示 = value;
    			尾42_鱗_表示 = value;
    			尾42_鱗左_表示 = value;
    			尾42_鱗右_表示 = value;
    			尾42_尾_表示 = value;
    			尾43_鱗_表示 = value;
    			尾43_鱗左_表示 = value;
    			尾43_鱗右_表示 = value;
    			尾43_尾_表示 = value;
    			尾44_鱗_表示 = value;
    			尾44_鱗左_表示 = value;
    			尾44_鱗右_表示 = value;
    			尾44_尾_表示 = value;
    			頭_上顎_顎基_表示 = value;
    			頭_上顎_鱗4_表示 = value;
    			頭_上顎_鱗左1_表示 = value;
    			頭_上顎_鱗右1_表示 = value;
    			頭_上顎_鱗左2_表示 = value;
    			頭_上顎_鱗右2_表示 = value;
    			頭_上顎_鱗左3_表示 = value;
    			頭_上顎_鱗右3_表示 = value;
    			頭_上顎_鱗左4_表示 = value;
    			頭_上顎_鱗右4_表示 = value;
    			頭_上顎_鱗左5_表示 = value;
    			頭_上顎_鱗右5_表示 = value;
    			頭_上顎_鱗左8_表示 = value;
    			頭_上顎_鱗右8_表示 = value;
    			頭_上顎_鱗左9_表示 = value;
    			頭_上顎_鱗右9_表示 = value;
    			頭_上顎_鱗左10_表示 = value;
    			頭_上顎_鱗右10_表示 = value;
    			頭_上顎_鱗左11_表示 = value;
    			頭_上顎_鱗右11_表示 = value;
    			頭_下顎_顎基_表示 = value;
    			頭_下顎_鱗1_表示 = value;
    			頭_下顎_鱗2_表示 = value;
    			頭_下顎_鱗3_表示 = value;
    			頭_下顎_鱗4_表示 = value;
    			頭_下顎_鱗左1_表示 = value;
    			頭_下顎_鱗右1_表示 = value;
    			頭_下顎_鱗左2_表示 = value;
    			頭_下顎_鱗右2_表示 = value;
    			頭_下顎_鱗左3_表示 = value;
    			頭_下顎_鱗右3_表示 = value;
    			頭_下顎_鱗左4_表示 = value;
    			頭_下顎_鱗右4_表示 = value;
    			頭_下顎_鱗左5_表示 = value;
    			頭_下顎_鱗右5_表示 = value;
    			頭_下顎_鱗左6_表示 = value;
    			頭_下顎_鱗右6_表示 = value;
    			頭_下顎_鱗左7_表示 = value;
    			頭_下顎_鱗右7_表示 = value;
    			頭_下顎_鱗左8_表示 = value;
    			頭_下顎_鱗右8_表示 = value;
    			頭_下顎_鱗左9_表示 = value;
    			頭_下顎_鱗右9_表示 = value;
    			頭_下顎_鱗左10_表示 = value;
    			頭_下顎_鱗右10_表示 = value;
    			頭_下顎_鱗左11_表示 = value;
    			頭_下顎_鱗右11_表示 = value;
    			輪2_革_表示 = value;
    			輪2_金具1_表示 = value;
    			輪2_金具2_表示 = value;
    			輪2_金具3_表示 = value;
    			輪2_金具左_表示 = value;
    			輪2_金具右_表示 = value;
    			鎖1.表示 = value;
    			鎖2.表示 = value;
    			鎖3.表示 = value;
    			鎖4.表示 = value;
    		}
    	}

    	public override double Intensity
    	{
    		get
    		{
    			return 尾1_根CD.不透明度;
    		}
    		set
    		{
    			尾1_根CD.不透明度 = value;
    			尾1_鱗CD.不透明度 = value;
    			尾1_鱗左CD.不透明度 = value;
    			尾1_鱗右CD.不透明度 = value;
    			尾1_尾CD.不透明度 = value;
    			尾2_鱗CD.不透明度 = value;
    			尾2_鱗左CD.不透明度 = value;
    			尾2_鱗右CD.不透明度 = value;
    			尾2_尾CD.不透明度 = value;
    			尾3_鱗CD.不透明度 = value;
    			尾3_鱗左CD.不透明度 = value;
    			尾3_鱗右CD.不透明度 = value;
    			尾3_尾CD.不透明度 = value;
    			尾4_鱗CD.不透明度 = value;
    			尾4_鱗左CD.不透明度 = value;
    			尾4_鱗右CD.不透明度 = value;
    			尾4_尾CD.不透明度 = value;
    			尾5_鱗CD.不透明度 = value;
    			尾5_鱗左CD.不透明度 = value;
    			尾5_鱗右CD.不透明度 = value;
    			尾5_尾CD.不透明度 = value;
    			尾6_鱗CD.不透明度 = value;
    			尾6_鱗左CD.不透明度 = value;
    			尾6_鱗右CD.不透明度 = value;
    			尾6_尾CD.不透明度 = value;
    			尾7_鱗CD.不透明度 = value;
    			尾7_鱗左CD.不透明度 = value;
    			尾7_鱗右CD.不透明度 = value;
    			尾7_尾CD.不透明度 = value;
    			尾8_鱗CD.不透明度 = value;
    			尾8_鱗左CD.不透明度 = value;
    			尾8_鱗右CD.不透明度 = value;
    			尾8_尾CD.不透明度 = value;
    			尾9_鱗CD.不透明度 = value;
    			尾9_鱗左CD.不透明度 = value;
    			尾9_鱗右CD.不透明度 = value;
    			尾9_尾CD.不透明度 = value;
    			尾10_鱗CD.不透明度 = value;
    			尾10_鱗左CD.不透明度 = value;
    			尾10_鱗右CD.不透明度 = value;
    			尾10_尾CD.不透明度 = value;
    			尾11_鱗CD.不透明度 = value;
    			尾11_鱗左CD.不透明度 = value;
    			尾11_鱗右CD.不透明度 = value;
    			尾11_尾CD.不透明度 = value;
    			尾12_鱗CD.不透明度 = value;
    			尾12_鱗左CD.不透明度 = value;
    			尾12_鱗右CD.不透明度 = value;
    			尾12_尾CD.不透明度 = value;
    			尾13_鱗CD.不透明度 = value;
    			尾13_鱗左CD.不透明度 = value;
    			尾13_鱗右CD.不透明度 = value;
    			尾13_尾CD.不透明度 = value;
    			尾14_鱗CD.不透明度 = value;
    			尾14_鱗左CD.不透明度 = value;
    			尾14_鱗右CD.不透明度 = value;
    			尾14_尾CD.不透明度 = value;
    			尾15_鱗CD.不透明度 = value;
    			尾15_鱗左CD.不透明度 = value;
    			尾15_鱗右CD.不透明度 = value;
    			尾15_尾CD.不透明度 = value;
    			尾16_鱗CD.不透明度 = value;
    			尾16_鱗左CD.不透明度 = value;
    			尾16_鱗右CD.不透明度 = value;
    			尾16_尾CD.不透明度 = value;
    			尾17_鱗CD.不透明度 = value;
    			尾17_鱗左CD.不透明度 = value;
    			尾17_鱗右CD.不透明度 = value;
    			尾17_尾CD.不透明度 = value;
    			尾18_鱗CD.不透明度 = value;
    			尾18_鱗左CD.不透明度 = value;
    			尾18_鱗右CD.不透明度 = value;
    			尾18_尾CD.不透明度 = value;
    			尾19_鱗CD.不透明度 = value;
    			尾19_鱗左CD.不透明度 = value;
    			尾19_鱗右CD.不透明度 = value;
    			尾19_尾CD.不透明度 = value;
    			尾20_鱗CD.不透明度 = value;
    			尾20_鱗左CD.不透明度 = value;
    			尾20_鱗右CD.不透明度 = value;
    			尾20_尾CD.不透明度 = value;
    			尾21_鱗CD.不透明度 = value;
    			尾21_鱗左CD.不透明度 = value;
    			尾21_鱗右CD.不透明度 = value;
    			尾21_尾CD.不透明度 = value;
    			尾22_鱗CD.不透明度 = value;
    			尾22_鱗左CD.不透明度 = value;
    			尾22_鱗右CD.不透明度 = value;
    			尾22_尾CD.不透明度 = value;
    			尾23_鱗CD.不透明度 = value;
    			尾23_鱗左CD.不透明度 = value;
    			尾23_鱗右CD.不透明度 = value;
    			尾23_尾CD.不透明度 = value;
    			尾24_鱗CD.不透明度 = value;
    			尾24_鱗左CD.不透明度 = value;
    			尾24_鱗右CD.不透明度 = value;
    			尾24_尾CD.不透明度 = value;
    			尾25_鱗CD.不透明度 = value;
    			尾25_鱗左CD.不透明度 = value;
    			尾25_鱗右CD.不透明度 = value;
    			尾25_尾CD.不透明度 = value;
    			尾26_鱗CD.不透明度 = value;
    			尾26_鱗左CD.不透明度 = value;
    			尾26_鱗右CD.不透明度 = value;
    			尾26_尾CD.不透明度 = value;
    			尾27_鱗CD.不透明度 = value;
    			尾27_鱗左CD.不透明度 = value;
    			尾27_鱗右CD.不透明度 = value;
    			尾27_尾CD.不透明度 = value;
    			尾28_鱗CD.不透明度 = value;
    			尾28_鱗左CD.不透明度 = value;
    			尾28_鱗右CD.不透明度 = value;
    			尾28_尾CD.不透明度 = value;
    			尾29_鱗CD.不透明度 = value;
    			尾29_鱗左CD.不透明度 = value;
    			尾29_鱗右CD.不透明度 = value;
    			尾29_尾CD.不透明度 = value;
    			尾30_鱗CD.不透明度 = value;
    			尾30_鱗左CD.不透明度 = value;
    			尾30_鱗右CD.不透明度 = value;
    			尾30_尾CD.不透明度 = value;
    			尾31_鱗CD.不透明度 = value;
    			尾31_鱗左CD.不透明度 = value;
    			尾31_鱗右CD.不透明度 = value;
    			尾31_尾CD.不透明度 = value;
    			尾32_鱗CD.不透明度 = value;
    			尾32_鱗左CD.不透明度 = value;
    			尾32_鱗右CD.不透明度 = value;
    			尾32_尾CD.不透明度 = value;
    			尾33_鱗CD.不透明度 = value;
    			尾33_鱗左CD.不透明度 = value;
    			尾33_鱗右CD.不透明度 = value;
    			尾33_尾CD.不透明度 = value;
    			尾34_鱗CD.不透明度 = value;
    			尾34_鱗左CD.不透明度 = value;
    			尾34_鱗右CD.不透明度 = value;
    			尾34_尾CD.不透明度 = value;
    			尾35_鱗CD.不透明度 = value;
    			尾35_鱗左CD.不透明度 = value;
    			尾35_鱗右CD.不透明度 = value;
    			尾35_尾CD.不透明度 = value;
    			尾36_鱗CD.不透明度 = value;
    			尾36_鱗左CD.不透明度 = value;
    			尾36_鱗右CD.不透明度 = value;
    			尾36_尾CD.不透明度 = value;
    			尾37_鱗CD.不透明度 = value;
    			尾37_鱗左CD.不透明度 = value;
    			尾37_鱗右CD.不透明度 = value;
    			尾37_尾CD.不透明度 = value;
    			尾38_鱗CD.不透明度 = value;
    			尾38_鱗左CD.不透明度 = value;
    			尾38_鱗右CD.不透明度 = value;
    			尾38_尾CD.不透明度 = value;
    			尾39_鱗CD.不透明度 = value;
    			尾39_鱗左CD.不透明度 = value;
    			尾39_鱗右CD.不透明度 = value;
    			尾39_尾CD.不透明度 = value;
    			尾40_鱗CD.不透明度 = value;
    			尾40_鱗左CD.不透明度 = value;
    			尾40_鱗右CD.不透明度 = value;
    			尾40_尾CD.不透明度 = value;
    			尾41_鱗CD.不透明度 = value;
    			尾41_鱗左CD.不透明度 = value;
    			尾41_鱗右CD.不透明度 = value;
    			尾41_尾CD.不透明度 = value;
    			尾42_鱗CD.不透明度 = value;
    			尾42_鱗左CD.不透明度 = value;
    			尾42_鱗右CD.不透明度 = value;
    			尾42_尾CD.不透明度 = value;
    			尾43_鱗CD.不透明度 = value;
    			尾43_鱗左CD.不透明度 = value;
    			尾43_鱗右CD.不透明度 = value;
    			尾43_尾CD.不透明度 = value;
    			尾44_鱗CD.不透明度 = value;
    			尾44_鱗左CD.不透明度 = value;
    			尾44_鱗右CD.不透明度 = value;
    			尾44_尾CD.不透明度 = value;
    			頭_上顎_顎基CD.不透明度 = value;
    			頭_上顎_鱗4CD.不透明度 = value;
    			頭_上顎_鱗左1CD.不透明度 = value;
    			頭_上顎_鱗右1CD.不透明度 = value;
    			頭_上顎_鱗左2CD.不透明度 = value;
    			頭_上顎_鱗右2CD.不透明度 = value;
    			頭_上顎_鱗左3CD.不透明度 = value;
    			頭_上顎_鱗右3CD.不透明度 = value;
    			頭_上顎_鱗左4CD.不透明度 = value;
    			頭_上顎_鱗右4CD.不透明度 = value;
    			頭_上顎_鱗左5CD.不透明度 = value;
    			頭_上顎_鱗右5CD.不透明度 = value;
    			頭_上顎_鱗左8CD.不透明度 = value;
    			頭_上顎_鱗右8CD.不透明度 = value;
    			頭_上顎_鱗左9CD.不透明度 = value;
    			頭_上顎_鱗右9CD.不透明度 = value;
    			頭_上顎_鱗左10CD.不透明度 = value;
    			頭_上顎_鱗右10CD.不透明度 = value;
    			頭_上顎_鱗左11CD.不透明度 = value;
    			頭_上顎_鱗右11CD.不透明度 = value;
    			頭_下顎_顎基CD.不透明度 = value;
    			頭_下顎_鱗1CD.不透明度 = value;
    			頭_下顎_鱗2CD.不透明度 = value;
    			頭_下顎_鱗3CD.不透明度 = value;
    			頭_下顎_鱗4CD.不透明度 = value;
    			頭_下顎_鱗左1CD.不透明度 = value;
    			頭_下顎_鱗右1CD.不透明度 = value;
    			頭_下顎_鱗左2CD.不透明度 = value;
    			頭_下顎_鱗右2CD.不透明度 = value;
    			頭_下顎_鱗左3CD.不透明度 = value;
    			頭_下顎_鱗右3CD.不透明度 = value;
    			頭_下顎_鱗左4CD.不透明度 = value;
    			頭_下顎_鱗右4CD.不透明度 = value;
    			頭_下顎_鱗左5CD.不透明度 = value;
    			頭_下顎_鱗右5CD.不透明度 = value;
    			頭_下顎_鱗左6CD.不透明度 = value;
    			頭_下顎_鱗右6CD.不透明度 = value;
    			頭_下顎_鱗左7CD.不透明度 = value;
    			頭_下顎_鱗右7CD.不透明度 = value;
    			頭_下顎_鱗左8CD.不透明度 = value;
    			頭_下顎_鱗右8CD.不透明度 = value;
    			頭_下顎_鱗左9CD.不透明度 = value;
    			頭_下顎_鱗右9CD.不透明度 = value;
    			頭_下顎_鱗左10CD.不透明度 = value;
    			頭_下顎_鱗右10CD.不透明度 = value;
    			頭_下顎_鱗左11CD.不透明度 = value;
    			頭_下顎_鱗右11CD.不透明度 = value;
    			輪1_革CD.不透明度 = value;
    			輪1_金具1CD.不透明度 = value;
    			輪1_金具2CD.不透明度 = value;
    			輪1_金具3CD.不透明度 = value;
    			輪1_金具左CD.不透明度 = value;
    			輪1_金具右CD.不透明度 = value;
    			輪2_革CD.不透明度 = value;
    			輪2_金具1CD.不透明度 = value;
    			輪2_金具2CD.不透明度 = value;
    			輪2_金具3CD.不透明度 = value;
    			輪2_金具左CD.不透明度 = value;
    			輪2_金具右CD.不透明度 = value;
    		}
    	}

    	public JointS 鎖1_接続点 => new JointS(Body, X0Y0_輪1_金具左, 0);

    	public JointS 鎖2_接続点 => new JointS(Body, X0Y0_輪1_金具右, 0);

    	public JointS 鎖3_接続点 => new JointS(Body, X0Y0_輪2_金具左, 0);

    	public JointS 鎖4_接続点 => new JointS(Body, X0Y0_輪2_金具右, 0);

    	public 尾_蛇(double DisUnit, 配色指定 配色指定, BodyColorSet 体配色, ModeEventDispatcher Med, 尾_蛇D e)
    	{
    		ThisType = GetType();
    		MorphVariant morphVariant = new MorphVariant();
    		morphVariant.Tag = "蛇尾";
    		morphVariant.Add(new PartGroup(GlobalState.尻尾["尾"][0][13]));
    		Body = new VariantGrid();
    		Body.Tag = morphVariant.Tag;
    		Body.Add(morphVariant);
    		PartGroup partGroup = Body[0][0];
    		PartGroup pars2 = partGroup["尾1"].ToPars();
    		X0Y0_尾1_根 = pars2["根"].ToPar();
    		X0Y0_尾1_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾1_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾1_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾1_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾2"].ToPars();
    		X0Y0_尾2_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾2_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾2_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾2_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾3"].ToPars();
    		X0Y0_尾3_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾3_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾3_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾3_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾4"].ToPars();
    		X0Y0_尾4_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾4_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾4_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾4_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾5"].ToPars();
    		X0Y0_尾5_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾5_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾5_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾5_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾6"].ToPars();
    		X0Y0_尾6_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾6_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾6_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾6_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾7"].ToPars();
    		X0Y0_尾7_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾7_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾7_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾7_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾8"].ToPars();
    		X0Y0_尾8_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾8_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾8_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾8_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾9"].ToPars();
    		X0Y0_尾9_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾9_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾9_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾9_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾10"].ToPars();
    		X0Y0_尾10_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾10_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾10_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾10_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾11"].ToPars();
    		X0Y0_尾11_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾11_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾11_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾11_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾12"].ToPars();
    		X0Y0_尾12_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾12_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾12_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾12_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾13"].ToPars();
    		X0Y0_尾13_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾13_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾13_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾13_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾14"].ToPars();
    		X0Y0_尾14_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾14_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾14_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾14_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾15"].ToPars();
    		X0Y0_尾15_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾15_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾15_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾15_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾16"].ToPars();
    		X0Y0_尾16_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾16_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾16_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾16_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾17"].ToPars();
    		X0Y0_尾17_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾17_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾17_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾17_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾18"].ToPars();
    		X0Y0_尾18_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾18_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾18_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾18_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾19"].ToPars();
    		X0Y0_尾19_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾19_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾19_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾19_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾20"].ToPars();
    		X0Y0_尾20_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾20_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾20_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾20_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾21"].ToPars();
    		X0Y0_尾21_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾21_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾21_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾21_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾22"].ToPars();
    		X0Y0_尾22_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾22_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾22_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾22_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾23"].ToPars();
    		X0Y0_尾23_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾23_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾23_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾23_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾24"].ToPars();
    		X0Y0_尾24_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾24_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾24_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾24_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾25"].ToPars();
    		X0Y0_尾25_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾25_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾25_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾25_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾26"].ToPars();
    		X0Y0_尾26_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾26_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾26_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾26_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["輪1"].ToPars();
    		X0Y0_輪1_革 = pars2["革"].ToPar();
    		X0Y0_輪1_金具1 = pars2["金具1"].ToPar();
    		X0Y0_輪1_金具2 = pars2["金具2"].ToPar();
    		X0Y0_輪1_金具3 = pars2["金具3"].ToPar();
    		X0Y0_輪1_金具左 = pars2["金具左"].ToPar();
    		X0Y0_輪1_金具右 = pars2["金具右"].ToPar();
    		pars2 = partGroup["尾27"].ToPars();
    		X0Y0_尾27_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾27_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾27_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾27_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾28"].ToPars();
    		X0Y0_尾28_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾28_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾28_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾28_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾29"].ToPars();
    		X0Y0_尾29_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾29_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾29_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾29_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾30"].ToPars();
    		X0Y0_尾30_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾30_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾30_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾30_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾31"].ToPars();
    		X0Y0_尾31_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾31_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾31_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾31_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾32"].ToPars();
    		X0Y0_尾32_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾32_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾32_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾32_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾33"].ToPars();
    		X0Y0_尾33_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾33_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾33_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾33_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾34"].ToPars();
    		X0Y0_尾34_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾34_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾34_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾34_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾35"].ToPars();
    		X0Y0_尾35_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾35_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾35_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾35_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾36"].ToPars();
    		X0Y0_尾36_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾36_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾36_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾36_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾37"].ToPars();
    		X0Y0_尾37_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾37_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾37_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾37_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾38"].ToPars();
    		X0Y0_尾38_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾38_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾38_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾38_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾39"].ToPars();
    		X0Y0_尾39_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾39_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾39_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾39_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾40"].ToPars();
    		X0Y0_尾40_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾40_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾40_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾40_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾41"].ToPars();
    		X0Y0_尾41_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾41_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾41_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾41_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾42"].ToPars();
    		X0Y0_尾42_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾42_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾42_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾42_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾43"].ToPars();
    		X0Y0_尾43_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾43_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾43_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾43_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["尾44"].ToPars();
    		X0Y0_尾44_鱗 = pars2["鱗"].ToPar();
    		X0Y0_尾44_鱗左 = pars2["鱗左"].ToPar();
    		X0Y0_尾44_鱗右 = pars2["鱗右"].ToPar();
    		X0Y0_尾44_尾 = pars2["尾"].ToPar();
    		pars2 = partGroup["頭"].ToPars();
    		PartGroup pars3 = pars2["上顎"].ToPars();
    		X0Y0_頭_上顎_顎基 = pars3["顎基"].ToPar();
    		X0Y0_頭_上顎_鱗4 = pars3["鱗4"].ToPar();
    		X0Y0_頭_上顎_鱗左1 = pars3["鱗左1"].ToPar();
    		X0Y0_頭_上顎_鱗右1 = pars3["鱗右1"].ToPar();
    		X0Y0_頭_上顎_鱗左2 = pars3["鱗左2"].ToPar();
    		X0Y0_頭_上顎_鱗右2 = pars3["鱗右2"].ToPar();
    		X0Y0_頭_上顎_鱗左3 = pars3["鱗左3"].ToPar();
    		X0Y0_頭_上顎_鱗右3 = pars3["鱗右3"].ToPar();
    		X0Y0_頭_上顎_鱗左4 = pars3["鱗左4"].ToPar();
    		X0Y0_頭_上顎_鱗右4 = pars3["鱗右4"].ToPar();
    		X0Y0_頭_上顎_鱗左5 = pars3["鱗左5"].ToPar();
    		X0Y0_頭_上顎_鱗右5 = pars3["鱗右5"].ToPar();
    		X0Y0_頭_上顎_鱗左8 = pars3["鱗左8"].ToPar();
    		X0Y0_頭_上顎_鱗右8 = pars3["鱗右8"].ToPar();
    		X0Y0_頭_上顎_鱗左9 = pars3["鱗左9"].ToPar();
    		X0Y0_頭_上顎_鱗右9 = pars3["鱗右9"].ToPar();
    		X0Y0_頭_上顎_鱗左10 = pars3["鱗左10"].ToPar();
    		X0Y0_頭_上顎_鱗右10 = pars3["鱗右10"].ToPar();
    		X0Y0_頭_上顎_鱗左11 = pars3["鱗左11"].ToPar();
    		X0Y0_頭_上顎_鱗右11 = pars3["鱗右11"].ToPar();
    		pars3 = pars2["下顎"].ToPars();
    		X0Y0_頭_下顎_顎基 = pars3["顎基"].ToPar();
    		X0Y0_頭_下顎_鱗1 = pars3["鱗1"].ToPar();
    		X0Y0_頭_下顎_鱗2 = pars3["鱗2"].ToPar();
    		X0Y0_頭_下顎_鱗3 = pars3["鱗3"].ToPar();
    		X0Y0_頭_下顎_鱗4 = pars3["鱗4"].ToPar();
    		X0Y0_頭_下顎_鱗左1 = pars3["鱗左1"].ToPar();
    		X0Y0_頭_下顎_鱗右1 = pars3["鱗右1"].ToPar();
    		X0Y0_頭_下顎_鱗左2 = pars3["鱗左2"].ToPar();
    		X0Y0_頭_下顎_鱗右2 = pars3["鱗右2"].ToPar();
    		X0Y0_頭_下顎_鱗左3 = pars3["鱗左3"].ToPar();
    		X0Y0_頭_下顎_鱗右3 = pars3["鱗右3"].ToPar();
    		X0Y0_頭_下顎_鱗左4 = pars3["鱗左4"].ToPar();
    		X0Y0_頭_下顎_鱗右4 = pars3["鱗右4"].ToPar();
    		X0Y0_頭_下顎_鱗左5 = pars3["鱗左5"].ToPar();
    		X0Y0_頭_下顎_鱗右5 = pars3["鱗右5"].ToPar();
    		X0Y0_頭_下顎_鱗左6 = pars3["鱗左6"].ToPar();
    		X0Y0_頭_下顎_鱗右6 = pars3["鱗右6"].ToPar();
    		X0Y0_頭_下顎_鱗左7 = pars3["鱗左7"].ToPar();
    		X0Y0_頭_下顎_鱗右7 = pars3["鱗右7"].ToPar();
    		X0Y0_頭_下顎_鱗左8 = pars3["鱗左8"].ToPar();
    		X0Y0_頭_下顎_鱗右8 = pars3["鱗右8"].ToPar();
    		X0Y0_頭_下顎_鱗左9 = pars3["鱗左9"].ToPar();
    		X0Y0_頭_下顎_鱗右9 = pars3["鱗右9"].ToPar();
    		X0Y0_頭_下顎_鱗左10 = pars3["鱗左10"].ToPar();
    		X0Y0_頭_下顎_鱗右10 = pars3["鱗右10"].ToPar();
    		X0Y0_頭_下顎_鱗左11 = pars3["鱗左11"].ToPar();
    		X0Y0_頭_下顎_鱗右11 = pars3["鱗右11"].ToPar();
    		pars2 = partGroup["輪2"].ToPars();
    		X0Y0_輪2_革 = pars2["革"].ToPar();
    		X0Y0_輪2_金具1 = pars2["金具1"].ToPar();
    		X0Y0_輪2_金具2 = pars2["金具2"].ToPar();
    		X0Y0_輪2_金具3 = pars2["金具3"].ToPar();
    		X0Y0_輪2_金具左 = pars2["金具左"].ToPar();
    		X0Y0_輪2_金具右 = pars2["金具右"].ToPar();
    		Body.SetJoints();
    		接続根 = new JointD(Body);
    		右 = e.右;
    		反転X = e.反転X;
    		反転Y = e.反転Y;
    		基準C = e.基準C;
    		位置C = e.位置C;
    		角度B = e.角度B;
    		角度C = e.角度C;
    		尺度B = e.尺度B;
    		尺度C = e.尺度C;
    		尺度XB = e.尺度XB;
    		尺度XC = e.尺度XC;
    		尺度YB = e.尺度YB;
    		尺度YC = e.尺度YC;
    		肥大 = e.肥大;
    		身長 = e.身長;
    		Xv = e.Xv;
    		Yv = e.Yv;
    		Xi = e.Xi;
    		Yi = e.Yi;
    		サイズ = e.サイズ;
    		サイズX = e.サイズX;
    		サイズY = e.サイズY;
    		尾1_根_表示 = e.尾1_根_表示;
    		尾1_鱗_表示 = e.尾1_鱗_表示;
    		尾1_鱗左_表示 = e.尾1_鱗左_表示;
    		尾1_鱗右_表示 = e.尾1_鱗右_表示;
    		尾1_尾_表示 = e.尾1_尾_表示;
    		尾2_鱗_表示 = e.尾2_鱗_表示;
    		尾2_鱗左_表示 = e.尾2_鱗左_表示;
    		尾2_鱗右_表示 = e.尾2_鱗右_表示;
    		尾2_尾_表示 = e.尾2_尾_表示;
    		尾3_鱗_表示 = e.尾3_鱗_表示;
    		尾3_鱗左_表示 = e.尾3_鱗左_表示;
    		尾3_鱗右_表示 = e.尾3_鱗右_表示;
    		尾3_尾_表示 = e.尾3_尾_表示;
    		尾4_鱗_表示 = e.尾4_鱗_表示;
    		尾4_鱗左_表示 = e.尾4_鱗左_表示;
    		尾4_鱗右_表示 = e.尾4_鱗右_表示;
    		尾4_尾_表示 = e.尾4_尾_表示;
    		尾5_鱗_表示 = e.尾5_鱗_表示;
    		尾5_鱗左_表示 = e.尾5_鱗左_表示;
    		尾5_鱗右_表示 = e.尾5_鱗右_表示;
    		尾5_尾_表示 = e.尾5_尾_表示;
    		尾6_鱗_表示 = e.尾6_鱗_表示;
    		尾6_鱗左_表示 = e.尾6_鱗左_表示;
    		尾6_鱗右_表示 = e.尾6_鱗右_表示;
    		尾6_尾_表示 = e.尾6_尾_表示;
    		尾7_鱗_表示 = e.尾7_鱗_表示;
    		尾7_鱗左_表示 = e.尾7_鱗左_表示;
    		尾7_鱗右_表示 = e.尾7_鱗右_表示;
    		尾7_尾_表示 = e.尾7_尾_表示;
    		尾8_鱗_表示 = e.尾8_鱗_表示;
    		尾8_鱗左_表示 = e.尾8_鱗左_表示;
    		尾8_鱗右_表示 = e.尾8_鱗右_表示;
    		尾8_尾_表示 = e.尾8_尾_表示;
    		尾9_鱗_表示 = e.尾9_鱗_表示;
    		尾9_鱗左_表示 = e.尾9_鱗左_表示;
    		尾9_鱗右_表示 = e.尾9_鱗右_表示;
    		尾9_尾_表示 = e.尾9_尾_表示;
    		尾10_鱗_表示 = e.尾10_鱗_表示;
    		尾10_鱗左_表示 = e.尾10_鱗左_表示;
    		尾10_鱗右_表示 = e.尾10_鱗右_表示;
    		尾10_尾_表示 = e.尾10_尾_表示;
    		尾11_鱗_表示 = e.尾11_鱗_表示;
    		尾11_鱗左_表示 = e.尾11_鱗左_表示;
    		尾11_鱗右_表示 = e.尾11_鱗右_表示;
    		尾11_尾_表示 = e.尾11_尾_表示;
    		尾12_鱗_表示 = e.尾12_鱗_表示;
    		尾12_鱗左_表示 = e.尾12_鱗左_表示;
    		尾12_鱗右_表示 = e.尾12_鱗右_表示;
    		尾12_尾_表示 = e.尾12_尾_表示;
    		尾13_鱗_表示 = e.尾13_鱗_表示;
    		尾13_鱗左_表示 = e.尾13_鱗左_表示;
    		尾13_鱗右_表示 = e.尾13_鱗右_表示;
    		尾13_尾_表示 = e.尾13_尾_表示;
    		尾14_鱗_表示 = e.尾14_鱗_表示;
    		尾14_鱗左_表示 = e.尾14_鱗左_表示;
    		尾14_鱗右_表示 = e.尾14_鱗右_表示;
    		尾14_尾_表示 = e.尾14_尾_表示;
    		尾15_鱗_表示 = e.尾15_鱗_表示;
    		尾15_鱗左_表示 = e.尾15_鱗左_表示;
    		尾15_鱗右_表示 = e.尾15_鱗右_表示;
    		尾15_尾_表示 = e.尾15_尾_表示;
    		尾16_鱗_表示 = e.尾16_鱗_表示;
    		尾16_鱗左_表示 = e.尾16_鱗左_表示;
    		尾16_鱗右_表示 = e.尾16_鱗右_表示;
    		尾16_尾_表示 = e.尾16_尾_表示;
    		尾17_鱗_表示 = e.尾17_鱗_表示;
    		尾17_鱗左_表示 = e.尾17_鱗左_表示;
    		尾17_鱗右_表示 = e.尾17_鱗右_表示;
    		尾17_尾_表示 = e.尾17_尾_表示;
    		尾18_鱗_表示 = e.尾18_鱗_表示;
    		尾18_鱗左_表示 = e.尾18_鱗左_表示;
    		尾18_鱗右_表示 = e.尾18_鱗右_表示;
    		尾18_尾_表示 = e.尾18_尾_表示;
    		尾19_鱗_表示 = e.尾19_鱗_表示;
    		尾19_鱗左_表示 = e.尾19_鱗左_表示;
    		尾19_鱗右_表示 = e.尾19_鱗右_表示;
    		尾19_尾_表示 = e.尾19_尾_表示;
    		尾20_鱗_表示 = e.尾20_鱗_表示;
    		尾20_鱗左_表示 = e.尾20_鱗左_表示;
    		尾20_鱗右_表示 = e.尾20_鱗右_表示;
    		尾20_尾_表示 = e.尾20_尾_表示;
    		尾21_鱗_表示 = e.尾21_鱗_表示;
    		尾21_鱗左_表示 = e.尾21_鱗左_表示;
    		尾21_鱗右_表示 = e.尾21_鱗右_表示;
    		尾21_尾_表示 = e.尾21_尾_表示;
    		尾22_鱗_表示 = e.尾22_鱗_表示;
    		尾22_鱗左_表示 = e.尾22_鱗左_表示;
    		尾22_鱗右_表示 = e.尾22_鱗右_表示;
    		尾22_尾_表示 = e.尾22_尾_表示;
    		尾23_鱗_表示 = e.尾23_鱗_表示;
    		尾23_鱗左_表示 = e.尾23_鱗左_表示;
    		尾23_鱗右_表示 = e.尾23_鱗右_表示;
    		尾23_尾_表示 = e.尾23_尾_表示;
    		尾24_鱗_表示 = e.尾24_鱗_表示;
    		尾24_鱗左_表示 = e.尾24_鱗左_表示;
    		尾24_鱗右_表示 = e.尾24_鱗右_表示;
    		尾24_尾_表示 = e.尾24_尾_表示;
    		尾25_鱗_表示 = e.尾25_鱗_表示;
    		尾25_鱗左_表示 = e.尾25_鱗左_表示;
    		尾25_鱗右_表示 = e.尾25_鱗右_表示;
    		尾25_尾_表示 = e.尾25_尾_表示;
    		尾26_鱗_表示 = e.尾26_鱗_表示;
    		尾26_鱗左_表示 = e.尾26_鱗左_表示;
    		尾26_鱗右_表示 = e.尾26_鱗右_表示;
    		尾26_尾_表示 = e.尾26_尾_表示;
    		輪1_革_表示 = e.輪1_革_表示;
    		輪1_金具1_表示 = e.輪1_金具1_表示;
    		輪1_金具2_表示 = e.輪1_金具2_表示;
    		輪1_金具3_表示 = e.輪1_金具3_表示;
    		輪1_金具左_表示 = e.輪1_金具左_表示;
    		輪1_金具右_表示 = e.輪1_金具右_表示;
    		尾27_鱗_表示 = e.尾27_鱗_表示;
    		尾27_鱗左_表示 = e.尾27_鱗左_表示;
    		尾27_鱗右_表示 = e.尾27_鱗右_表示;
    		尾27_尾_表示 = e.尾27_尾_表示;
    		尾28_鱗_表示 = e.尾28_鱗_表示;
    		尾28_鱗左_表示 = e.尾28_鱗左_表示;
    		尾28_鱗右_表示 = e.尾28_鱗右_表示;
    		尾28_尾_表示 = e.尾28_尾_表示;
    		尾29_鱗_表示 = e.尾29_鱗_表示;
    		尾29_鱗左_表示 = e.尾29_鱗左_表示;
    		尾29_鱗右_表示 = e.尾29_鱗右_表示;
    		尾29_尾_表示 = e.尾29_尾_表示;
    		尾30_鱗_表示 = e.尾30_鱗_表示;
    		尾30_鱗左_表示 = e.尾30_鱗左_表示;
    		尾30_鱗右_表示 = e.尾30_鱗右_表示;
    		尾30_尾_表示 = e.尾30_尾_表示;
    		尾31_鱗_表示 = e.尾31_鱗_表示;
    		尾31_鱗左_表示 = e.尾31_鱗左_表示;
    		尾31_鱗右_表示 = e.尾31_鱗右_表示;
    		尾31_尾_表示 = e.尾31_尾_表示;
    		尾32_鱗_表示 = e.尾32_鱗_表示;
    		尾32_鱗左_表示 = e.尾32_鱗左_表示;
    		尾32_鱗右_表示 = e.尾32_鱗右_表示;
    		尾32_尾_表示 = e.尾32_尾_表示;
    		尾33_鱗_表示 = e.尾33_鱗_表示;
    		尾33_鱗左_表示 = e.尾33_鱗左_表示;
    		尾33_鱗右_表示 = e.尾33_鱗右_表示;
    		尾33_尾_表示 = e.尾33_尾_表示;
    		尾34_鱗_表示 = e.尾34_鱗_表示;
    		尾34_鱗左_表示 = e.尾34_鱗左_表示;
    		尾34_鱗右_表示 = e.尾34_鱗右_表示;
    		尾34_尾_表示 = e.尾34_尾_表示;
    		尾35_鱗_表示 = e.尾35_鱗_表示;
    		尾35_鱗左_表示 = e.尾35_鱗左_表示;
    		尾35_鱗右_表示 = e.尾35_鱗右_表示;
    		尾35_尾_表示 = e.尾35_尾_表示;
    		尾36_鱗_表示 = e.尾36_鱗_表示;
    		尾36_鱗左_表示 = e.尾36_鱗左_表示;
    		尾36_鱗右_表示 = e.尾36_鱗右_表示;
    		尾36_尾_表示 = e.尾36_尾_表示;
    		尾37_鱗_表示 = e.尾37_鱗_表示;
    		尾37_鱗左_表示 = e.尾37_鱗左_表示;
    		尾37_鱗右_表示 = e.尾37_鱗右_表示;
    		尾37_尾_表示 = e.尾37_尾_表示;
    		尾38_鱗_表示 = e.尾38_鱗_表示;
    		尾38_鱗左_表示 = e.尾38_鱗左_表示;
    		尾38_鱗右_表示 = e.尾38_鱗右_表示;
    		尾38_尾_表示 = e.尾38_尾_表示;
    		尾39_鱗_表示 = e.尾39_鱗_表示;
    		尾39_鱗左_表示 = e.尾39_鱗左_表示;
    		尾39_鱗右_表示 = e.尾39_鱗右_表示;
    		尾39_尾_表示 = e.尾39_尾_表示;
    		尾40_鱗_表示 = e.尾40_鱗_表示;
    		尾40_鱗左_表示 = e.尾40_鱗左_表示;
    		尾40_鱗右_表示 = e.尾40_鱗右_表示;
    		尾40_尾_表示 = e.尾40_尾_表示;
    		尾41_鱗_表示 = e.尾41_鱗_表示;
    		尾41_鱗左_表示 = e.尾41_鱗左_表示;
    		尾41_鱗右_表示 = e.尾41_鱗右_表示;
    		尾41_尾_表示 = e.尾41_尾_表示;
    		尾42_鱗_表示 = e.尾42_鱗_表示;
    		尾42_鱗左_表示 = e.尾42_鱗左_表示;
    		尾42_鱗右_表示 = e.尾42_鱗右_表示;
    		尾42_尾_表示 = e.尾42_尾_表示;
    		尾43_鱗_表示 = e.尾43_鱗_表示;
    		尾43_鱗左_表示 = e.尾43_鱗左_表示;
    		尾43_鱗右_表示 = e.尾43_鱗右_表示;
    		尾43_尾_表示 = e.尾43_尾_表示;
    		尾44_鱗_表示 = e.尾44_鱗_表示;
    		尾44_鱗左_表示 = e.尾44_鱗左_表示;
    		尾44_鱗右_表示 = e.尾44_鱗右_表示;
    		尾44_尾_表示 = e.尾44_尾_表示;
    		頭_上顎_顎基_表示 = e.頭_上顎_顎基_表示;
    		頭_上顎_鱗4_表示 = e.頭_上顎_鱗4_表示;
    		頭_上顎_鱗左1_表示 = e.頭_上顎_鱗左1_表示;
    		頭_上顎_鱗右1_表示 = e.頭_上顎_鱗右1_表示;
    		頭_上顎_鱗左2_表示 = e.頭_上顎_鱗左2_表示;
    		頭_上顎_鱗右2_表示 = e.頭_上顎_鱗右2_表示;
    		頭_上顎_鱗左3_表示 = e.頭_上顎_鱗左3_表示;
    		頭_上顎_鱗右3_表示 = e.頭_上顎_鱗右3_表示;
    		頭_上顎_鱗左4_表示 = e.頭_上顎_鱗左4_表示;
    		頭_上顎_鱗右4_表示 = e.頭_上顎_鱗右4_表示;
    		頭_上顎_鱗左5_表示 = e.頭_上顎_鱗左5_表示;
    		頭_上顎_鱗右5_表示 = e.頭_上顎_鱗右5_表示;
    		頭_上顎_鱗左8_表示 = e.頭_上顎_鱗左8_表示;
    		頭_上顎_鱗右8_表示 = e.頭_上顎_鱗右8_表示;
    		頭_上顎_鱗左9_表示 = e.頭_上顎_鱗左9_表示;
    		頭_上顎_鱗右9_表示 = e.頭_上顎_鱗右9_表示;
    		頭_上顎_鱗左10_表示 = e.頭_上顎_鱗左10_表示;
    		頭_上顎_鱗右10_表示 = e.頭_上顎_鱗右10_表示;
    		頭_上顎_鱗左11_表示 = e.頭_上顎_鱗左11_表示;
    		頭_上顎_鱗右11_表示 = e.頭_上顎_鱗右11_表示;
    		頭_下顎_顎基_表示 = e.頭_下顎_顎基_表示;
    		頭_下顎_鱗1_表示 = e.頭_下顎_鱗1_表示;
    		頭_下顎_鱗2_表示 = e.頭_下顎_鱗2_表示;
    		頭_下顎_鱗3_表示 = e.頭_下顎_鱗3_表示;
    		頭_下顎_鱗4_表示 = e.頭_下顎_鱗4_表示;
    		頭_下顎_鱗左1_表示 = e.頭_下顎_鱗左1_表示;
    		頭_下顎_鱗右1_表示 = e.頭_下顎_鱗右1_表示;
    		頭_下顎_鱗左2_表示 = e.頭_下顎_鱗左2_表示;
    		頭_下顎_鱗右2_表示 = e.頭_下顎_鱗右2_表示;
    		頭_下顎_鱗左3_表示 = e.頭_下顎_鱗左3_表示;
    		頭_下顎_鱗右3_表示 = e.頭_下顎_鱗右3_表示;
    		頭_下顎_鱗左4_表示 = e.頭_下顎_鱗左4_表示;
    		頭_下顎_鱗右4_表示 = e.頭_下顎_鱗右4_表示;
    		頭_下顎_鱗左5_表示 = e.頭_下顎_鱗左5_表示;
    		頭_下顎_鱗右5_表示 = e.頭_下顎_鱗右5_表示;
    		頭_下顎_鱗左6_表示 = e.頭_下顎_鱗左6_表示;
    		頭_下顎_鱗右6_表示 = e.頭_下顎_鱗右6_表示;
    		頭_下顎_鱗左7_表示 = e.頭_下顎_鱗左7_表示;
    		頭_下顎_鱗右7_表示 = e.頭_下顎_鱗右7_表示;
    		頭_下顎_鱗左8_表示 = e.頭_下顎_鱗左8_表示;
    		頭_下顎_鱗右8_表示 = e.頭_下顎_鱗右8_表示;
    		頭_下顎_鱗左9_表示 = e.頭_下顎_鱗左9_表示;
    		頭_下顎_鱗右9_表示 = e.頭_下顎_鱗右9_表示;
    		頭_下顎_鱗左10_表示 = e.頭_下顎_鱗左10_表示;
    		頭_下顎_鱗右10_表示 = e.頭_下顎_鱗右10_表示;
    		頭_下顎_鱗左11_表示 = e.頭_下顎_鱗左11_表示;
    		頭_下顎_鱗右11_表示 = e.頭_下顎_鱗右11_表示;
    		輪2_革_表示 = e.輪2_革_表示;
    		輪2_金具1_表示 = e.輪2_金具1_表示;
    		輪2_金具2_表示 = e.輪2_金具2_表示;
    		輪2_金具3_表示 = e.輪2_金具3_表示;
    		輪2_金具左_表示 = e.輪2_金具左_表示;
    		輪2_金具右_表示 = e.輪2_金具右_表示;
    		輪1表示 = e.輪1表示;
    		輪2表示 = e.輪2表示;
    		欠損 = e.欠損;
    		筋肉 = e.筋肉;
    		拘束 = e.拘束;
    		if (!e.表示)
    		{
    			表示 = false;
    		}
    		base.配色指定 = 配色指定;
    		配色(体配色);
    		X0Y0_尾1_根CP = new ColorP(X0Y0_尾1_根, 尾1_根CD, DisUnit, abj: true);
    		X0Y0_尾1_鱗CP = new ColorP(X0Y0_尾1_鱗, 尾1_鱗CD, DisUnit, abj: true);
    		X0Y0_尾1_鱗左CP = new ColorP(X0Y0_尾1_鱗左, 尾1_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾1_鱗右CP = new ColorP(X0Y0_尾1_鱗右, 尾1_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾1_尾CP = new ColorP(X0Y0_尾1_尾, 尾1_尾CD, DisUnit, abj: true);
    		X0Y0_尾2_鱗CP = new ColorP(X0Y0_尾2_鱗, 尾2_鱗CD, DisUnit, abj: true);
    		X0Y0_尾2_鱗左CP = new ColorP(X0Y0_尾2_鱗左, 尾2_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾2_鱗右CP = new ColorP(X0Y0_尾2_鱗右, 尾2_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾2_尾CP = new ColorP(X0Y0_尾2_尾, 尾2_尾CD, DisUnit, abj: true);
    		X0Y0_尾3_鱗CP = new ColorP(X0Y0_尾3_鱗, 尾3_鱗CD, DisUnit, abj: true);
    		X0Y0_尾3_鱗左CP = new ColorP(X0Y0_尾3_鱗左, 尾3_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾3_鱗右CP = new ColorP(X0Y0_尾3_鱗右, 尾3_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾3_尾CP = new ColorP(X0Y0_尾3_尾, 尾3_尾CD, DisUnit, abj: true);
    		X0Y0_尾4_鱗CP = new ColorP(X0Y0_尾4_鱗, 尾4_鱗CD, DisUnit, abj: true);
    		X0Y0_尾4_鱗左CP = new ColorP(X0Y0_尾4_鱗左, 尾4_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾4_鱗右CP = new ColorP(X0Y0_尾4_鱗右, 尾4_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾4_尾CP = new ColorP(X0Y0_尾4_尾, 尾4_尾CD, DisUnit, abj: true);
    		X0Y0_尾5_鱗CP = new ColorP(X0Y0_尾5_鱗, 尾5_鱗CD, DisUnit, abj: true);
    		X0Y0_尾5_鱗左CP = new ColorP(X0Y0_尾5_鱗左, 尾5_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾5_鱗右CP = new ColorP(X0Y0_尾5_鱗右, 尾5_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾5_尾CP = new ColorP(X0Y0_尾5_尾, 尾5_尾CD, DisUnit, abj: true);
    		X0Y0_尾6_鱗CP = new ColorP(X0Y0_尾6_鱗, 尾6_鱗CD, DisUnit, abj: true);
    		X0Y0_尾6_鱗左CP = new ColorP(X0Y0_尾6_鱗左, 尾6_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾6_鱗右CP = new ColorP(X0Y0_尾6_鱗右, 尾6_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾6_尾CP = new ColorP(X0Y0_尾6_尾, 尾6_尾CD, DisUnit, abj: true);
    		X0Y0_尾7_鱗CP = new ColorP(X0Y0_尾7_鱗, 尾7_鱗CD, DisUnit, abj: true);
    		X0Y0_尾7_鱗左CP = new ColorP(X0Y0_尾7_鱗左, 尾7_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾7_鱗右CP = new ColorP(X0Y0_尾7_鱗右, 尾7_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾7_尾CP = new ColorP(X0Y0_尾7_尾, 尾7_尾CD, DisUnit, abj: true);
    		X0Y0_尾8_鱗CP = new ColorP(X0Y0_尾8_鱗, 尾8_鱗CD, DisUnit, abj: true);
    		X0Y0_尾8_鱗左CP = new ColorP(X0Y0_尾8_鱗左, 尾8_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾8_鱗右CP = new ColorP(X0Y0_尾8_鱗右, 尾8_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾8_尾CP = new ColorP(X0Y0_尾8_尾, 尾8_尾CD, DisUnit, abj: true);
    		X0Y0_尾9_鱗CP = new ColorP(X0Y0_尾9_鱗, 尾9_鱗CD, DisUnit, abj: true);
    		X0Y0_尾9_鱗左CP = new ColorP(X0Y0_尾9_鱗左, 尾9_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾9_鱗右CP = new ColorP(X0Y0_尾9_鱗右, 尾9_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾9_尾CP = new ColorP(X0Y0_尾9_尾, 尾9_尾CD, DisUnit, abj: true);
    		X0Y0_尾10_鱗CP = new ColorP(X0Y0_尾10_鱗, 尾10_鱗CD, DisUnit, abj: true);
    		X0Y0_尾10_鱗左CP = new ColorP(X0Y0_尾10_鱗左, 尾10_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾10_鱗右CP = new ColorP(X0Y0_尾10_鱗右, 尾10_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾10_尾CP = new ColorP(X0Y0_尾10_尾, 尾10_尾CD, DisUnit, abj: true);
    		X0Y0_尾11_鱗CP = new ColorP(X0Y0_尾11_鱗, 尾11_鱗CD, DisUnit, abj: true);
    		X0Y0_尾11_鱗左CP = new ColorP(X0Y0_尾11_鱗左, 尾11_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾11_鱗右CP = new ColorP(X0Y0_尾11_鱗右, 尾11_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾11_尾CP = new ColorP(X0Y0_尾11_尾, 尾11_尾CD, DisUnit, abj: true);
    		X0Y0_尾12_鱗CP = new ColorP(X0Y0_尾12_鱗, 尾12_鱗CD, DisUnit, abj: true);
    		X0Y0_尾12_鱗左CP = new ColorP(X0Y0_尾12_鱗左, 尾12_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾12_鱗右CP = new ColorP(X0Y0_尾12_鱗右, 尾12_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾12_尾CP = new ColorP(X0Y0_尾12_尾, 尾12_尾CD, DisUnit, abj: true);
    		X0Y0_尾13_鱗CP = new ColorP(X0Y0_尾13_鱗, 尾13_鱗CD, DisUnit, abj: true);
    		X0Y0_尾13_鱗左CP = new ColorP(X0Y0_尾13_鱗左, 尾13_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾13_鱗右CP = new ColorP(X0Y0_尾13_鱗右, 尾13_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾13_尾CP = new ColorP(X0Y0_尾13_尾, 尾13_尾CD, DisUnit, abj: true);
    		X0Y0_尾14_鱗CP = new ColorP(X0Y0_尾14_鱗, 尾14_鱗CD, DisUnit, abj: true);
    		X0Y0_尾14_鱗左CP = new ColorP(X0Y0_尾14_鱗左, 尾14_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾14_鱗右CP = new ColorP(X0Y0_尾14_鱗右, 尾14_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾14_尾CP = new ColorP(X0Y0_尾14_尾, 尾14_尾CD, DisUnit, abj: true);
    		X0Y0_尾15_鱗CP = new ColorP(X0Y0_尾15_鱗, 尾15_鱗CD, DisUnit, abj: true);
    		X0Y0_尾15_鱗左CP = new ColorP(X0Y0_尾15_鱗左, 尾15_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾15_鱗右CP = new ColorP(X0Y0_尾15_鱗右, 尾15_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾15_尾CP = new ColorP(X0Y0_尾15_尾, 尾15_尾CD, DisUnit, abj: true);
    		X0Y0_尾16_鱗CP = new ColorP(X0Y0_尾16_鱗, 尾16_鱗CD, DisUnit, abj: true);
    		X0Y0_尾16_鱗左CP = new ColorP(X0Y0_尾16_鱗左, 尾16_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾16_鱗右CP = new ColorP(X0Y0_尾16_鱗右, 尾16_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾16_尾CP = new ColorP(X0Y0_尾16_尾, 尾16_尾CD, DisUnit, abj: true);
    		X0Y0_尾17_鱗CP = new ColorP(X0Y0_尾17_鱗, 尾17_鱗CD, DisUnit, abj: true);
    		X0Y0_尾17_鱗左CP = new ColorP(X0Y0_尾17_鱗左, 尾17_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾17_鱗右CP = new ColorP(X0Y0_尾17_鱗右, 尾17_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾17_尾CP = new ColorP(X0Y0_尾17_尾, 尾17_尾CD, DisUnit, abj: true);
    		X0Y0_尾18_鱗CP = new ColorP(X0Y0_尾18_鱗, 尾18_鱗CD, DisUnit, abj: true);
    		X0Y0_尾18_鱗左CP = new ColorP(X0Y0_尾18_鱗左, 尾18_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾18_鱗右CP = new ColorP(X0Y0_尾18_鱗右, 尾18_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾18_尾CP = new ColorP(X0Y0_尾18_尾, 尾18_尾CD, DisUnit, abj: true);
    		X0Y0_尾19_鱗CP = new ColorP(X0Y0_尾19_鱗, 尾19_鱗CD, DisUnit, abj: true);
    		X0Y0_尾19_鱗左CP = new ColorP(X0Y0_尾19_鱗左, 尾19_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾19_鱗右CP = new ColorP(X0Y0_尾19_鱗右, 尾19_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾19_尾CP = new ColorP(X0Y0_尾19_尾, 尾19_尾CD, DisUnit, abj: true);
    		X0Y0_尾20_鱗CP = new ColorP(X0Y0_尾20_鱗, 尾20_鱗CD, DisUnit, abj: true);
    		X0Y0_尾20_鱗左CP = new ColorP(X0Y0_尾20_鱗左, 尾20_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾20_鱗右CP = new ColorP(X0Y0_尾20_鱗右, 尾20_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾20_尾CP = new ColorP(X0Y0_尾20_尾, 尾20_尾CD, DisUnit, abj: true);
    		X0Y0_尾21_鱗CP = new ColorP(X0Y0_尾21_鱗, 尾21_鱗CD, DisUnit, abj: true);
    		X0Y0_尾21_鱗左CP = new ColorP(X0Y0_尾21_鱗左, 尾21_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾21_鱗右CP = new ColorP(X0Y0_尾21_鱗右, 尾21_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾21_尾CP = new ColorP(X0Y0_尾21_尾, 尾21_尾CD, DisUnit, abj: true);
    		X0Y0_尾22_鱗CP = new ColorP(X0Y0_尾22_鱗, 尾22_鱗CD, DisUnit, abj: true);
    		X0Y0_尾22_鱗左CP = new ColorP(X0Y0_尾22_鱗左, 尾22_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾22_鱗右CP = new ColorP(X0Y0_尾22_鱗右, 尾22_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾22_尾CP = new ColorP(X0Y0_尾22_尾, 尾22_尾CD, DisUnit, abj: true);
    		X0Y0_尾23_鱗CP = new ColorP(X0Y0_尾23_鱗, 尾23_鱗CD, DisUnit, abj: true);
    		X0Y0_尾23_鱗左CP = new ColorP(X0Y0_尾23_鱗左, 尾23_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾23_鱗右CP = new ColorP(X0Y0_尾23_鱗右, 尾23_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾23_尾CP = new ColorP(X0Y0_尾23_尾, 尾23_尾CD, DisUnit, abj: true);
    		X0Y0_尾24_鱗CP = new ColorP(X0Y0_尾24_鱗, 尾24_鱗CD, DisUnit, abj: true);
    		X0Y0_尾24_鱗左CP = new ColorP(X0Y0_尾24_鱗左, 尾24_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾24_鱗右CP = new ColorP(X0Y0_尾24_鱗右, 尾24_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾24_尾CP = new ColorP(X0Y0_尾24_尾, 尾24_尾CD, DisUnit, abj: true);
    		X0Y0_尾25_鱗CP = new ColorP(X0Y0_尾25_鱗, 尾25_鱗CD, DisUnit, abj: true);
    		X0Y0_尾25_鱗左CP = new ColorP(X0Y0_尾25_鱗左, 尾25_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾25_鱗右CP = new ColorP(X0Y0_尾25_鱗右, 尾25_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾25_尾CP = new ColorP(X0Y0_尾25_尾, 尾25_尾CD, DisUnit, abj: true);
    		X0Y0_尾26_鱗CP = new ColorP(X0Y0_尾26_鱗, 尾26_鱗CD, DisUnit, abj: true);
    		X0Y0_尾26_鱗左CP = new ColorP(X0Y0_尾26_鱗左, 尾26_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾26_鱗右CP = new ColorP(X0Y0_尾26_鱗右, 尾26_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾26_尾CP = new ColorP(X0Y0_尾26_尾, 尾26_尾CD, DisUnit, abj: true);
    		X0Y0_輪1_革CP = new ColorP(X0Y0_輪1_革, 輪1_革CD, DisUnit, abj: true);
    		X0Y0_輪1_金具1CP = new ColorP(X0Y0_輪1_金具1, 輪1_金具1CD, DisUnit, abj: true);
    		X0Y0_輪1_金具2CP = new ColorP(X0Y0_輪1_金具2, 輪1_金具2CD, DisUnit, abj: true);
    		X0Y0_輪1_金具3CP = new ColorP(X0Y0_輪1_金具3, 輪1_金具3CD, DisUnit, abj: true);
    		X0Y0_輪1_金具左CP = new ColorP(X0Y0_輪1_金具左, 輪1_金具左CD, DisUnit, abj: true);
    		X0Y0_輪1_金具右CP = new ColorP(X0Y0_輪1_金具右, 輪1_金具右CD, DisUnit, abj: true);
    		X0Y0_尾27_鱗CP = new ColorP(X0Y0_尾27_鱗, 尾27_鱗CD, DisUnit, abj: true);
    		X0Y0_尾27_鱗左CP = new ColorP(X0Y0_尾27_鱗左, 尾27_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾27_鱗右CP = new ColorP(X0Y0_尾27_鱗右, 尾27_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾27_尾CP = new ColorP(X0Y0_尾27_尾, 尾27_尾CD, DisUnit, abj: true);
    		X0Y0_尾28_鱗CP = new ColorP(X0Y0_尾28_鱗, 尾28_鱗CD, DisUnit, abj: true);
    		X0Y0_尾28_鱗左CP = new ColorP(X0Y0_尾28_鱗左, 尾28_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾28_鱗右CP = new ColorP(X0Y0_尾28_鱗右, 尾28_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾28_尾CP = new ColorP(X0Y0_尾28_尾, 尾28_尾CD, DisUnit, abj: true);
    		X0Y0_尾29_鱗CP = new ColorP(X0Y0_尾29_鱗, 尾29_鱗CD, DisUnit, abj: true);
    		X0Y0_尾29_鱗左CP = new ColorP(X0Y0_尾29_鱗左, 尾29_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾29_鱗右CP = new ColorP(X0Y0_尾29_鱗右, 尾29_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾29_尾CP = new ColorP(X0Y0_尾29_尾, 尾29_尾CD, DisUnit, abj: true);
    		X0Y0_尾30_鱗CP = new ColorP(X0Y0_尾30_鱗, 尾30_鱗CD, DisUnit, abj: true);
    		X0Y0_尾30_鱗左CP = new ColorP(X0Y0_尾30_鱗左, 尾30_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾30_鱗右CP = new ColorP(X0Y0_尾30_鱗右, 尾30_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾30_尾CP = new ColorP(X0Y0_尾30_尾, 尾30_尾CD, DisUnit, abj: true);
    		X0Y0_尾31_鱗CP = new ColorP(X0Y0_尾31_鱗, 尾31_鱗CD, DisUnit, abj: true);
    		X0Y0_尾31_鱗左CP = new ColorP(X0Y0_尾31_鱗左, 尾31_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾31_鱗右CP = new ColorP(X0Y0_尾31_鱗右, 尾31_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾31_尾CP = new ColorP(X0Y0_尾31_尾, 尾31_尾CD, DisUnit, abj: true);
    		X0Y0_尾32_鱗CP = new ColorP(X0Y0_尾32_鱗, 尾32_鱗CD, DisUnit, abj: true);
    		X0Y0_尾32_鱗左CP = new ColorP(X0Y0_尾32_鱗左, 尾32_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾32_鱗右CP = new ColorP(X0Y0_尾32_鱗右, 尾32_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾32_尾CP = new ColorP(X0Y0_尾32_尾, 尾32_尾CD, DisUnit, abj: true);
    		X0Y0_尾33_鱗CP = new ColorP(X0Y0_尾33_鱗, 尾33_鱗CD, DisUnit, abj: true);
    		X0Y0_尾33_鱗左CP = new ColorP(X0Y0_尾33_鱗左, 尾33_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾33_鱗右CP = new ColorP(X0Y0_尾33_鱗右, 尾33_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾33_尾CP = new ColorP(X0Y0_尾33_尾, 尾33_尾CD, DisUnit, abj: true);
    		X0Y0_尾34_鱗CP = new ColorP(X0Y0_尾34_鱗, 尾34_鱗CD, DisUnit, abj: true);
    		X0Y0_尾34_鱗左CP = new ColorP(X0Y0_尾34_鱗左, 尾34_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾34_鱗右CP = new ColorP(X0Y0_尾34_鱗右, 尾34_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾34_尾CP = new ColorP(X0Y0_尾34_尾, 尾34_尾CD, DisUnit, abj: true);
    		X0Y0_尾35_鱗CP = new ColorP(X0Y0_尾35_鱗, 尾35_鱗CD, DisUnit, abj: true);
    		X0Y0_尾35_鱗左CP = new ColorP(X0Y0_尾35_鱗左, 尾35_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾35_鱗右CP = new ColorP(X0Y0_尾35_鱗右, 尾35_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾35_尾CP = new ColorP(X0Y0_尾35_尾, 尾35_尾CD, DisUnit, abj: true);
    		X0Y0_尾36_鱗CP = new ColorP(X0Y0_尾36_鱗, 尾36_鱗CD, DisUnit, abj: true);
    		X0Y0_尾36_鱗左CP = new ColorP(X0Y0_尾36_鱗左, 尾36_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾36_鱗右CP = new ColorP(X0Y0_尾36_鱗右, 尾36_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾36_尾CP = new ColorP(X0Y0_尾36_尾, 尾36_尾CD, DisUnit, abj: true);
    		X0Y0_尾37_鱗CP = new ColorP(X0Y0_尾37_鱗, 尾37_鱗CD, DisUnit, abj: true);
    		X0Y0_尾37_鱗左CP = new ColorP(X0Y0_尾37_鱗左, 尾37_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾37_鱗右CP = new ColorP(X0Y0_尾37_鱗右, 尾37_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾37_尾CP = new ColorP(X0Y0_尾37_尾, 尾37_尾CD, DisUnit, abj: true);
    		X0Y0_尾38_鱗CP = new ColorP(X0Y0_尾38_鱗, 尾38_鱗CD, DisUnit, abj: true);
    		X0Y0_尾38_鱗左CP = new ColorP(X0Y0_尾38_鱗左, 尾38_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾38_鱗右CP = new ColorP(X0Y0_尾38_鱗右, 尾38_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾38_尾CP = new ColorP(X0Y0_尾38_尾, 尾38_尾CD, DisUnit, abj: true);
    		X0Y0_尾39_鱗CP = new ColorP(X0Y0_尾39_鱗, 尾39_鱗CD, DisUnit, abj: true);
    		X0Y0_尾39_鱗左CP = new ColorP(X0Y0_尾39_鱗左, 尾39_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾39_鱗右CP = new ColorP(X0Y0_尾39_鱗右, 尾39_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾39_尾CP = new ColorP(X0Y0_尾39_尾, 尾39_尾CD, DisUnit, abj: true);
    		X0Y0_尾40_鱗CP = new ColorP(X0Y0_尾40_鱗, 尾40_鱗CD, DisUnit, abj: true);
    		X0Y0_尾40_鱗左CP = new ColorP(X0Y0_尾40_鱗左, 尾40_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾40_鱗右CP = new ColorP(X0Y0_尾40_鱗右, 尾40_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾40_尾CP = new ColorP(X0Y0_尾40_尾, 尾40_尾CD, DisUnit, abj: true);
    		X0Y0_尾41_鱗CP = new ColorP(X0Y0_尾41_鱗, 尾41_鱗CD, DisUnit, abj: true);
    		X0Y0_尾41_鱗左CP = new ColorP(X0Y0_尾41_鱗左, 尾41_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾41_鱗右CP = new ColorP(X0Y0_尾41_鱗右, 尾41_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾41_尾CP = new ColorP(X0Y0_尾41_尾, 尾41_尾CD, DisUnit, abj: true);
    		X0Y0_尾42_鱗CP = new ColorP(X0Y0_尾42_鱗, 尾42_鱗CD, DisUnit, abj: true);
    		X0Y0_尾42_鱗左CP = new ColorP(X0Y0_尾42_鱗左, 尾42_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾42_鱗右CP = new ColorP(X0Y0_尾42_鱗右, 尾42_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾42_尾CP = new ColorP(X0Y0_尾42_尾, 尾42_尾CD, DisUnit, abj: true);
    		X0Y0_尾43_鱗CP = new ColorP(X0Y0_尾43_鱗, 尾43_鱗CD, DisUnit, abj: true);
    		X0Y0_尾43_鱗左CP = new ColorP(X0Y0_尾43_鱗左, 尾43_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾43_鱗右CP = new ColorP(X0Y0_尾43_鱗右, 尾43_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾43_尾CP = new ColorP(X0Y0_尾43_尾, 尾43_尾CD, DisUnit, abj: true);
    		X0Y0_尾44_鱗CP = new ColorP(X0Y0_尾44_鱗, 尾44_鱗CD, DisUnit, abj: true);
    		X0Y0_尾44_鱗左CP = new ColorP(X0Y0_尾44_鱗左, 尾44_鱗左CD, DisUnit, abj: true);
    		X0Y0_尾44_鱗右CP = new ColorP(X0Y0_尾44_鱗右, 尾44_鱗右CD, DisUnit, abj: true);
    		X0Y0_尾44_尾CP = new ColorP(X0Y0_尾44_尾, 尾44_尾CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_顎基CP = new ColorP(X0Y0_頭_上顎_顎基, 頭_上顎_顎基CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗4CP = new ColorP(X0Y0_頭_上顎_鱗4, 頭_上顎_鱗4CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗左1CP = new ColorP(X0Y0_頭_上顎_鱗左1, 頭_上顎_鱗左1CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗右1CP = new ColorP(X0Y0_頭_上顎_鱗右1, 頭_上顎_鱗右1CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗左2CP = new ColorP(X0Y0_頭_上顎_鱗左2, 頭_上顎_鱗左2CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗右2CP = new ColorP(X0Y0_頭_上顎_鱗右2, 頭_上顎_鱗右2CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗左3CP = new ColorP(X0Y0_頭_上顎_鱗左3, 頭_上顎_鱗左3CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗右3CP = new ColorP(X0Y0_頭_上顎_鱗右3, 頭_上顎_鱗右3CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗左4CP = new ColorP(X0Y0_頭_上顎_鱗左4, 頭_上顎_鱗左4CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗右4CP = new ColorP(X0Y0_頭_上顎_鱗右4, 頭_上顎_鱗右4CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗左5CP = new ColorP(X0Y0_頭_上顎_鱗左5, 頭_上顎_鱗左5CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗右5CP = new ColorP(X0Y0_頭_上顎_鱗右5, 頭_上顎_鱗右5CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗左8CP = new ColorP(X0Y0_頭_上顎_鱗左8, 頭_上顎_鱗左8CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗右8CP = new ColorP(X0Y0_頭_上顎_鱗右8, 頭_上顎_鱗右8CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗左9CP = new ColorP(X0Y0_頭_上顎_鱗左9, 頭_上顎_鱗左9CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗右9CP = new ColorP(X0Y0_頭_上顎_鱗右9, 頭_上顎_鱗右9CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗左10CP = new ColorP(X0Y0_頭_上顎_鱗左10, 頭_上顎_鱗左10CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗右10CP = new ColorP(X0Y0_頭_上顎_鱗右10, 頭_上顎_鱗右10CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗左11CP = new ColorP(X0Y0_頭_上顎_鱗左11, 頭_上顎_鱗左11CD, DisUnit, abj: true);
    		X0Y0_頭_上顎_鱗右11CP = new ColorP(X0Y0_頭_上顎_鱗右11, 頭_上顎_鱗右11CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_顎基CP = new ColorP(X0Y0_頭_下顎_顎基, 頭_下顎_顎基CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗1CP = new ColorP(X0Y0_頭_下顎_鱗1, 頭_下顎_鱗1CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗2CP = new ColorP(X0Y0_頭_下顎_鱗2, 頭_下顎_鱗2CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗3CP = new ColorP(X0Y0_頭_下顎_鱗3, 頭_下顎_鱗3CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗4CP = new ColorP(X0Y0_頭_下顎_鱗4, 頭_下顎_鱗4CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗左1CP = new ColorP(X0Y0_頭_下顎_鱗左1, 頭_下顎_鱗左1CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗右1CP = new ColorP(X0Y0_頭_下顎_鱗右1, 頭_下顎_鱗右1CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗左2CP = new ColorP(X0Y0_頭_下顎_鱗左2, 頭_下顎_鱗左2CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗右2CP = new ColorP(X0Y0_頭_下顎_鱗右2, 頭_下顎_鱗右2CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗左3CP = new ColorP(X0Y0_頭_下顎_鱗左3, 頭_下顎_鱗左3CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗右3CP = new ColorP(X0Y0_頭_下顎_鱗右3, 頭_下顎_鱗右3CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗左4CP = new ColorP(X0Y0_頭_下顎_鱗左4, 頭_下顎_鱗左4CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗右4CP = new ColorP(X0Y0_頭_下顎_鱗右4, 頭_下顎_鱗右4CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗左5CP = new ColorP(X0Y0_頭_下顎_鱗左5, 頭_下顎_鱗左5CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗右5CP = new ColorP(X0Y0_頭_下顎_鱗右5, 頭_下顎_鱗右5CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗左6CP = new ColorP(X0Y0_頭_下顎_鱗左6, 頭_下顎_鱗左6CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗右6CP = new ColorP(X0Y0_頭_下顎_鱗右6, 頭_下顎_鱗右6CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗左7CP = new ColorP(X0Y0_頭_下顎_鱗左7, 頭_下顎_鱗左7CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗右7CP = new ColorP(X0Y0_頭_下顎_鱗右7, 頭_下顎_鱗右7CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗左8CP = new ColorP(X0Y0_頭_下顎_鱗左8, 頭_下顎_鱗左8CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗右8CP = new ColorP(X0Y0_頭_下顎_鱗右8, 頭_下顎_鱗右8CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗左9CP = new ColorP(X0Y0_頭_下顎_鱗左9, 頭_下顎_鱗左9CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗右9CP = new ColorP(X0Y0_頭_下顎_鱗右9, 頭_下顎_鱗右9CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗左10CP = new ColorP(X0Y0_頭_下顎_鱗左10, 頭_下顎_鱗左10CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗右10CP = new ColorP(X0Y0_頭_下顎_鱗右10, 頭_下顎_鱗右10CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗左11CP = new ColorP(X0Y0_頭_下顎_鱗左11, 頭_下顎_鱗左11CD, DisUnit, abj: true);
    		X0Y0_頭_下顎_鱗右11CP = new ColorP(X0Y0_頭_下顎_鱗右11, 頭_下顎_鱗右11CD, DisUnit, abj: true);
    		X0Y0_輪2_革CP = new ColorP(X0Y0_輪2_革, 輪2_革CD, DisUnit, abj: true);
    		X0Y0_輪2_金具1CP = new ColorP(X0Y0_輪2_金具1, 輪2_金具1CD, DisUnit, abj: true);
    		X0Y0_輪2_金具2CP = new ColorP(X0Y0_輪2_金具2, 輪2_金具2CD, DisUnit, abj: true);
    		X0Y0_輪2_金具3CP = new ColorP(X0Y0_輪2_金具3, 輪2_金具3CD, DisUnit, abj: true);
    		X0Y0_輪2_金具左CP = new ColorP(X0Y0_輪2_金具左, 輪2_金具左CD, DisUnit, abj: true);
    		X0Y0_輪2_金具右CP = new ColorP(X0Y0_輪2_金具右, 輪2_金具右CD, DisUnit, abj: true);
    		Intensity = e.濃度;
    		鎖1 = new 拘束鎖(DisUnit, 右, 配色指定, 体配色, Xasix);
    		鎖2 = new 拘束鎖(DisUnit, !右, 配色指定, 体配色, Xasix);
    		鎖3 = new 拘束鎖(DisUnit, 右, 配色指定, 体配色, Xasix);
    		鎖4 = new 拘束鎖(DisUnit, !右, 配色指定, 体配色, Xasix);
    		鎖1.接続(鎖1_接続点);
    		鎖2.接続(鎖2_接続点);
    		鎖3.接続(鎖3_接続点);
    		鎖4.接続(鎖4_接続点);
    		int num = (右 ? (-10) : 10);
    		鎖1.角度B -= num;
    		鎖2.角度B += num;
    		鎖3.角度B -= num;
    		鎖4.角度B += num;
    		鎖表示 = e.鎖表示;
    	}

    	public override void 描画0(RenderArea Are)
    	{
    		Are.Draw(X0Y0_尾1_根);
    		Are.Draw(X0Y0_尾1_鱗);
    		Are.Draw(X0Y0_尾1_鱗左);
    		Are.Draw(X0Y0_尾1_鱗右);
    		Are.Draw(X0Y0_尾1_尾);
    		Are.Draw(X0Y0_尾2_鱗);
    		Are.Draw(X0Y0_尾2_鱗左);
    		Are.Draw(X0Y0_尾2_鱗右);
    		Are.Draw(X0Y0_尾2_尾);
    		Are.Draw(X0Y0_尾3_鱗);
    		Are.Draw(X0Y0_尾3_鱗左);
    		Are.Draw(X0Y0_尾3_鱗右);
    		Are.Draw(X0Y0_尾3_尾);
    		Are.Draw(X0Y0_尾4_鱗);
    		Are.Draw(X0Y0_尾4_鱗左);
    		Are.Draw(X0Y0_尾4_鱗右);
    		Are.Draw(X0Y0_尾4_尾);
    		Are.Draw(X0Y0_尾5_鱗);
    		Are.Draw(X0Y0_尾5_鱗左);
    		Are.Draw(X0Y0_尾5_鱗右);
    		Are.Draw(X0Y0_尾5_尾);
    		Are.Draw(X0Y0_尾6_鱗);
    		Are.Draw(X0Y0_尾6_鱗左);
    		Are.Draw(X0Y0_尾6_鱗右);
    		Are.Draw(X0Y0_尾6_尾);
    		Are.Draw(X0Y0_尾7_鱗);
    		Are.Draw(X0Y0_尾7_鱗左);
    		Are.Draw(X0Y0_尾7_鱗右);
    		Are.Draw(X0Y0_尾7_尾);
    		Are.Draw(X0Y0_尾8_鱗);
    		Are.Draw(X0Y0_尾8_鱗左);
    		Are.Draw(X0Y0_尾8_鱗右);
    		Are.Draw(X0Y0_尾8_尾);
    		Are.Draw(X0Y0_尾9_鱗);
    		Are.Draw(X0Y0_尾9_鱗左);
    		Are.Draw(X0Y0_尾9_鱗右);
    		Are.Draw(X0Y0_尾9_尾);
    		Are.Draw(X0Y0_尾10_鱗);
    		Are.Draw(X0Y0_尾10_鱗左);
    		Are.Draw(X0Y0_尾10_鱗右);
    		Are.Draw(X0Y0_尾10_尾);
    		Are.Draw(X0Y0_尾11_鱗);
    		Are.Draw(X0Y0_尾11_鱗左);
    		Are.Draw(X0Y0_尾11_鱗右);
    		Are.Draw(X0Y0_尾11_尾);
    		Are.Draw(X0Y0_尾12_鱗);
    		Are.Draw(X0Y0_尾12_鱗左);
    		Are.Draw(X0Y0_尾12_鱗右);
    		Are.Draw(X0Y0_尾12_尾);
    		Are.Draw(X0Y0_尾13_鱗);
    		Are.Draw(X0Y0_尾13_鱗左);
    		Are.Draw(X0Y0_尾13_鱗右);
    		Are.Draw(X0Y0_尾13_尾);
    		Are.Draw(X0Y0_尾14_鱗);
    		Are.Draw(X0Y0_尾14_鱗左);
    		Are.Draw(X0Y0_尾14_鱗右);
    		Are.Draw(X0Y0_尾14_尾);
    		Are.Draw(X0Y0_尾15_鱗);
    		Are.Draw(X0Y0_尾15_鱗左);
    		Are.Draw(X0Y0_尾15_鱗右);
    		Are.Draw(X0Y0_尾15_尾);
    		Are.Draw(X0Y0_尾16_鱗);
    		Are.Draw(X0Y0_尾16_鱗左);
    		Are.Draw(X0Y0_尾16_鱗右);
    		Are.Draw(X0Y0_尾16_尾);
    		Are.Draw(X0Y0_尾17_鱗);
    		Are.Draw(X0Y0_尾17_鱗左);
    		Are.Draw(X0Y0_尾17_鱗右);
    		Are.Draw(X0Y0_尾17_尾);
    		Are.Draw(X0Y0_尾18_鱗);
    		Are.Draw(X0Y0_尾18_鱗左);
    		Are.Draw(X0Y0_尾18_鱗右);
    		Are.Draw(X0Y0_尾18_尾);
    		Are.Draw(X0Y0_尾19_鱗);
    		Are.Draw(X0Y0_尾19_鱗左);
    		Are.Draw(X0Y0_尾19_鱗右);
    		Are.Draw(X0Y0_尾19_尾);
    		Are.Draw(X0Y0_尾20_鱗);
    		Are.Draw(X0Y0_尾20_鱗左);
    		Are.Draw(X0Y0_尾20_鱗右);
    		Are.Draw(X0Y0_尾20_尾);
    		Are.Draw(X0Y0_尾21_鱗);
    		Are.Draw(X0Y0_尾21_鱗左);
    		Are.Draw(X0Y0_尾21_鱗右);
    		Are.Draw(X0Y0_尾21_尾);
    		Are.Draw(X0Y0_尾22_鱗);
    		Are.Draw(X0Y0_尾22_鱗左);
    		Are.Draw(X0Y0_尾22_鱗右);
    		Are.Draw(X0Y0_尾22_尾);
    		Are.Draw(X0Y0_尾23_鱗);
    		Are.Draw(X0Y0_尾23_鱗左);
    		Are.Draw(X0Y0_尾23_鱗右);
    		Are.Draw(X0Y0_尾23_尾);
    		Are.Draw(X0Y0_尾24_鱗);
    		Are.Draw(X0Y0_尾24_鱗左);
    		Are.Draw(X0Y0_尾24_鱗右);
    		Are.Draw(X0Y0_尾24_尾);
    		Are.Draw(X0Y0_尾25_鱗);
    		Are.Draw(X0Y0_尾25_鱗左);
    		Are.Draw(X0Y0_尾25_鱗右);
    		Are.Draw(X0Y0_尾25_尾);
    		Are.Draw(X0Y0_尾26_鱗);
    		Are.Draw(X0Y0_尾26_鱗左);
    		Are.Draw(X0Y0_尾26_鱗右);
    		Are.Draw(X0Y0_尾26_尾);
    		Are.Draw(X0Y0_輪1_革);
    		Are.Draw(X0Y0_輪1_金具1);
    		Are.Draw(X0Y0_輪1_金具2);
    		Are.Draw(X0Y0_輪1_金具3);
    		Are.Draw(X0Y0_輪1_金具左);
    		Are.Draw(X0Y0_輪1_金具右);
    		鎖1.描画0(Are);
    		鎖2.描画0(Are);
    		Are.Draw(X0Y0_尾27_鱗);
    		Are.Draw(X0Y0_尾27_鱗左);
    		Are.Draw(X0Y0_尾27_鱗右);
    		Are.Draw(X0Y0_尾27_尾);
    		Are.Draw(X0Y0_尾28_鱗);
    		Are.Draw(X0Y0_尾28_鱗左);
    		Are.Draw(X0Y0_尾28_鱗右);
    		Are.Draw(X0Y0_尾28_尾);
    		Are.Draw(X0Y0_尾29_鱗);
    		Are.Draw(X0Y0_尾29_鱗左);
    		Are.Draw(X0Y0_尾29_鱗右);
    		Are.Draw(X0Y0_尾29_尾);
    		Are.Draw(X0Y0_尾30_鱗);
    		Are.Draw(X0Y0_尾30_鱗左);
    		Are.Draw(X0Y0_尾30_鱗右);
    		Are.Draw(X0Y0_尾30_尾);
    		Are.Draw(X0Y0_尾31_鱗);
    		Are.Draw(X0Y0_尾31_鱗左);
    		Are.Draw(X0Y0_尾31_鱗右);
    		Are.Draw(X0Y0_尾31_尾);
    		Are.Draw(X0Y0_尾32_鱗);
    		Are.Draw(X0Y0_尾32_鱗左);
    		Are.Draw(X0Y0_尾32_鱗右);
    		Are.Draw(X0Y0_尾32_尾);
    		Are.Draw(X0Y0_尾33_鱗);
    		Are.Draw(X0Y0_尾33_鱗左);
    		Are.Draw(X0Y0_尾33_鱗右);
    		Are.Draw(X0Y0_尾33_尾);
    		Are.Draw(X0Y0_尾34_鱗);
    		Are.Draw(X0Y0_尾34_鱗左);
    		Are.Draw(X0Y0_尾34_鱗右);
    		Are.Draw(X0Y0_尾34_尾);
    		Are.Draw(X0Y0_尾35_鱗);
    		Are.Draw(X0Y0_尾35_鱗左);
    		Are.Draw(X0Y0_尾35_鱗右);
    		Are.Draw(X0Y0_尾35_尾);
    		Are.Draw(X0Y0_尾36_鱗);
    		Are.Draw(X0Y0_尾36_鱗左);
    		Are.Draw(X0Y0_尾36_鱗右);
    		Are.Draw(X0Y0_尾36_尾);
    		Are.Draw(X0Y0_尾37_鱗);
    		Are.Draw(X0Y0_尾37_鱗左);
    		Are.Draw(X0Y0_尾37_鱗右);
    		Are.Draw(X0Y0_尾37_尾);
    		Are.Draw(X0Y0_尾38_鱗);
    		Are.Draw(X0Y0_尾38_鱗左);
    		Are.Draw(X0Y0_尾38_鱗右);
    		Are.Draw(X0Y0_尾38_尾);
    		Are.Draw(X0Y0_尾39_鱗);
    		Are.Draw(X0Y0_尾39_鱗左);
    		Are.Draw(X0Y0_尾39_鱗右);
    		Are.Draw(X0Y0_尾39_尾);
    		Are.Draw(X0Y0_尾40_鱗);
    		Are.Draw(X0Y0_尾40_鱗左);
    		Are.Draw(X0Y0_尾40_鱗右);
    		Are.Draw(X0Y0_尾40_尾);
    		Are.Draw(X0Y0_尾41_鱗);
    		Are.Draw(X0Y0_尾41_鱗左);
    		Are.Draw(X0Y0_尾41_鱗右);
    		Are.Draw(X0Y0_尾41_尾);
    		Are.Draw(X0Y0_尾42_鱗);
    		Are.Draw(X0Y0_尾42_鱗左);
    		Are.Draw(X0Y0_尾42_鱗右);
    		Are.Draw(X0Y0_尾42_尾);
    		Are.Draw(X0Y0_尾43_鱗);
    		Are.Draw(X0Y0_尾43_鱗左);
    		Are.Draw(X0Y0_尾43_鱗右);
    		Are.Draw(X0Y0_尾43_尾);
    		Are.Draw(X0Y0_尾44_鱗);
    		Are.Draw(X0Y0_尾44_鱗左);
    		Are.Draw(X0Y0_尾44_鱗右);
    		Are.Draw(X0Y0_尾44_尾);
    		Are.Draw(X0Y0_頭_上顎_顎基);
    		Are.Draw(X0Y0_頭_上顎_鱗4);
    		Are.Draw(X0Y0_頭_上顎_鱗左1);
    		Are.Draw(X0Y0_頭_上顎_鱗右1);
    		Are.Draw(X0Y0_頭_上顎_鱗左2);
    		Are.Draw(X0Y0_頭_上顎_鱗右2);
    		Are.Draw(X0Y0_頭_上顎_鱗左3);
    		Are.Draw(X0Y0_頭_上顎_鱗右3);
    		Are.Draw(X0Y0_頭_上顎_鱗左4);
    		Are.Draw(X0Y0_頭_上顎_鱗右4);
    		Are.Draw(X0Y0_頭_上顎_鱗左5);
    		Are.Draw(X0Y0_頭_上顎_鱗右5);
    		Are.Draw(X0Y0_頭_上顎_鱗左8);
    		Are.Draw(X0Y0_頭_上顎_鱗右8);
    		Are.Draw(X0Y0_頭_上顎_鱗左9);
    		Are.Draw(X0Y0_頭_上顎_鱗右9);
    		Are.Draw(X0Y0_頭_上顎_鱗左10);
    		Are.Draw(X0Y0_頭_上顎_鱗右10);
    		Are.Draw(X0Y0_頭_上顎_鱗左11);
    		Are.Draw(X0Y0_頭_上顎_鱗右11);
    		Are.Draw(X0Y0_頭_下顎_顎基);
    		Are.Draw(X0Y0_頭_下顎_鱗1);
    		Are.Draw(X0Y0_頭_下顎_鱗2);
    		Are.Draw(X0Y0_頭_下顎_鱗3);
    		Are.Draw(X0Y0_頭_下顎_鱗4);
    		Are.Draw(X0Y0_頭_下顎_鱗左1);
    		Are.Draw(X0Y0_頭_下顎_鱗右1);
    		Are.Draw(X0Y0_頭_下顎_鱗左2);
    		Are.Draw(X0Y0_頭_下顎_鱗右2);
    		Are.Draw(X0Y0_頭_下顎_鱗左3);
    		Are.Draw(X0Y0_頭_下顎_鱗右3);
    		Are.Draw(X0Y0_頭_下顎_鱗左4);
    		Are.Draw(X0Y0_頭_下顎_鱗右4);
    		Are.Draw(X0Y0_頭_下顎_鱗左5);
    		Are.Draw(X0Y0_頭_下顎_鱗右5);
    		Are.Draw(X0Y0_頭_下顎_鱗左6);
    		Are.Draw(X0Y0_頭_下顎_鱗右6);
    		Are.Draw(X0Y0_頭_下顎_鱗左7);
    		Are.Draw(X0Y0_頭_下顎_鱗右7);
    		Are.Draw(X0Y0_頭_下顎_鱗左8);
    		Are.Draw(X0Y0_頭_下顎_鱗右8);
    		Are.Draw(X0Y0_頭_下顎_鱗左9);
    		Are.Draw(X0Y0_頭_下顎_鱗右9);
    		Are.Draw(X0Y0_頭_下顎_鱗左10);
    		Are.Draw(X0Y0_頭_下顎_鱗右10);
    		Are.Draw(X0Y0_頭_下顎_鱗左11);
    		Are.Draw(X0Y0_頭_下顎_鱗右11);
    		Are.Draw(X0Y0_輪2_革);
    		Are.Draw(X0Y0_輪2_金具1);
    		Are.Draw(X0Y0_輪2_金具2);
    		Are.Draw(X0Y0_輪2_金具3);
    		Are.Draw(X0Y0_輪2_金具左);
    		Are.Draw(X0Y0_輪2_金具右);
    		鎖3.描画0(Are);
    		鎖4.描画0(Are);
    	}

    	public override void Dispose()
    	{
    		base.Dispose();
    		鎖1.Dispose();
    		鎖2.Dispose();
    		鎖3.Dispose();
    		鎖4.Dispose();
    	}

    	public override void SetAngle0()
    	{
    		_ = 右;
    		double maxAngle = 20.0;
    		X0Y0_尾1_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾2_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾3_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾4_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾5_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾6_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾7_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾8_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾9_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾10_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾11_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾12_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾13_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾14_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾15_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾16_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾17_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾18_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾19_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾20_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾21_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾22_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾23_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾24_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾25_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾26_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾27_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾28_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾29_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾30_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾31_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾32_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾33_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾34_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾35_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾36_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾37_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾38_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾39_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾40_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾41_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾42_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾43_尾.SetAngleBase(maxAngle.GetRanAngle());
    		X0Y0_尾44_尾.SetAngleBase(maxAngle.GetRanAngle());
    		Body.JoinPAall();
    	}

    	public override bool Is革(ShapePart p)
    	{
    		if (p != X0Y0_輪1_革 && p != X0Y0_輪1_金具1 && p != X0Y0_輪1_金具2 && p != X0Y0_輪1_金具3 && p != X0Y0_輪1_金具左 && p != X0Y0_輪1_金具右 && p != X0Y0_輪2_革 && p != X0Y0_輪2_金具1 && p != X0Y0_輪2_金具2 && p != X0Y0_輪2_金具3 && p != X0Y0_輪2_金具左)
    		{
    			return p == X0Y0_輪2_金具右;
    		}
    		return true;
    	}

    	public override IEnumerable<ShapePart> Enum軸()
    	{
    		yield return X0Y0_尾1_尾;
    		yield return X0Y0_尾2_尾;
    		yield return X0Y0_尾3_尾;
    		yield return X0Y0_尾4_尾;
    		yield return X0Y0_尾5_尾;
    		yield return X0Y0_尾6_尾;
    		yield return X0Y0_尾7_尾;
    		yield return X0Y0_尾8_尾;
    		yield return X0Y0_尾9_尾;
    		yield return X0Y0_尾10_尾;
    		yield return X0Y0_尾11_尾;
    		yield return X0Y0_尾12_尾;
    		yield return X0Y0_尾13_尾;
    		yield return X0Y0_尾14_尾;
    		yield return X0Y0_尾15_尾;
    		yield return X0Y0_尾16_尾;
    		yield return X0Y0_尾17_尾;
    		yield return X0Y0_尾18_尾;
    		yield return X0Y0_尾19_尾;
    		yield return X0Y0_尾20_尾;
    		yield return X0Y0_尾21_尾;
    		yield return X0Y0_尾22_尾;
    		yield return X0Y0_尾23_尾;
    		yield return X0Y0_尾24_尾;
    		yield return X0Y0_尾25_尾;
    		yield return X0Y0_尾26_尾;
    		yield return X0Y0_尾27_尾;
    		yield return X0Y0_尾28_尾;
    		yield return X0Y0_尾29_尾;
    		yield return X0Y0_尾30_尾;
    		yield return X0Y0_尾31_尾;
    		yield return X0Y0_尾32_尾;
    		yield return X0Y0_尾33_尾;
    		yield return X0Y0_尾34_尾;
    		yield return X0Y0_尾35_尾;
    		yield return X0Y0_尾36_尾;
    		yield return X0Y0_尾37_尾;
    		yield return X0Y0_尾38_尾;
    		yield return X0Y0_尾39_尾;
    		yield return X0Y0_尾40_尾;
    		yield return X0Y0_尾41_尾;
    		yield return X0Y0_尾42_尾;
    		yield return X0Y0_尾43_尾;
    		yield return X0Y0_尾44_尾;
    	}

    	public override void 色更新()
    	{
    		X0Y0_尾1_根CP.Update();
    		X0Y0_尾1_鱗CP.Update();
    		X0Y0_尾1_鱗左CP.Update();
    		X0Y0_尾1_鱗右CP.Update();
    		X0Y0_尾1_尾CP.Update();
    		X0Y0_尾2_鱗CP.Update();
    		X0Y0_尾2_鱗左CP.Update();
    		X0Y0_尾2_鱗右CP.Update();
    		X0Y0_尾2_尾CP.Update();
    		X0Y0_尾3_鱗CP.Update();
    		X0Y0_尾3_鱗左CP.Update();
    		X0Y0_尾3_鱗右CP.Update();
    		X0Y0_尾3_尾CP.Update();
    		X0Y0_尾4_鱗CP.Update();
    		X0Y0_尾4_鱗左CP.Update();
    		X0Y0_尾4_鱗右CP.Update();
    		X0Y0_尾4_尾CP.Update();
    		X0Y0_尾5_鱗CP.Update();
    		X0Y0_尾5_鱗左CP.Update();
    		X0Y0_尾5_鱗右CP.Update();
    		X0Y0_尾5_尾CP.Update();
    		X0Y0_尾6_鱗CP.Update();
    		X0Y0_尾6_鱗左CP.Update();
    		X0Y0_尾6_鱗右CP.Update();
    		X0Y0_尾6_尾CP.Update();
    		X0Y0_尾7_鱗CP.Update();
    		X0Y0_尾7_鱗左CP.Update();
    		X0Y0_尾7_鱗右CP.Update();
    		X0Y0_尾7_尾CP.Update();
    		X0Y0_尾8_鱗CP.Update();
    		X0Y0_尾8_鱗左CP.Update();
    		X0Y0_尾8_鱗右CP.Update();
    		X0Y0_尾8_尾CP.Update();
    		X0Y0_尾9_鱗CP.Update();
    		X0Y0_尾9_鱗左CP.Update();
    		X0Y0_尾9_鱗右CP.Update();
    		X0Y0_尾9_尾CP.Update();
    		X0Y0_尾10_鱗CP.Update();
    		X0Y0_尾10_鱗左CP.Update();
    		X0Y0_尾10_鱗右CP.Update();
    		X0Y0_尾10_尾CP.Update();
    		X0Y0_尾11_鱗CP.Update();
    		X0Y0_尾11_鱗左CP.Update();
    		X0Y0_尾11_鱗右CP.Update();
    		X0Y0_尾11_尾CP.Update();
    		X0Y0_尾12_鱗CP.Update();
    		X0Y0_尾12_鱗左CP.Update();
    		X0Y0_尾12_鱗右CP.Update();
    		X0Y0_尾12_尾CP.Update();
    		X0Y0_尾13_鱗CP.Update();
    		X0Y0_尾13_鱗左CP.Update();
    		X0Y0_尾13_鱗右CP.Update();
    		X0Y0_尾13_尾CP.Update();
    		X0Y0_尾14_鱗CP.Update();
    		X0Y0_尾14_鱗左CP.Update();
    		X0Y0_尾14_鱗右CP.Update();
    		X0Y0_尾14_尾CP.Update();
    		X0Y0_尾15_鱗CP.Update();
    		X0Y0_尾15_鱗左CP.Update();
    		X0Y0_尾15_鱗右CP.Update();
    		X0Y0_尾15_尾CP.Update();
    		X0Y0_尾16_鱗CP.Update();
    		X0Y0_尾16_鱗左CP.Update();
    		X0Y0_尾16_鱗右CP.Update();
    		X0Y0_尾16_尾CP.Update();
    		X0Y0_尾17_鱗CP.Update();
    		X0Y0_尾17_鱗左CP.Update();
    		X0Y0_尾17_鱗右CP.Update();
    		X0Y0_尾17_尾CP.Update();
    		X0Y0_尾18_鱗CP.Update();
    		X0Y0_尾18_鱗左CP.Update();
    		X0Y0_尾18_鱗右CP.Update();
    		X0Y0_尾18_尾CP.Update();
    		X0Y0_尾19_鱗CP.Update();
    		X0Y0_尾19_鱗左CP.Update();
    		X0Y0_尾19_鱗右CP.Update();
    		X0Y0_尾19_尾CP.Update();
    		X0Y0_尾20_鱗CP.Update();
    		X0Y0_尾20_鱗左CP.Update();
    		X0Y0_尾20_鱗右CP.Update();
    		X0Y0_尾20_尾CP.Update();
    		X0Y0_尾21_鱗CP.Update();
    		X0Y0_尾21_鱗左CP.Update();
    		X0Y0_尾21_鱗右CP.Update();
    		X0Y0_尾21_尾CP.Update();
    		X0Y0_尾22_鱗CP.Update();
    		X0Y0_尾22_鱗左CP.Update();
    		X0Y0_尾22_鱗右CP.Update();
    		X0Y0_尾22_尾CP.Update();
    		X0Y0_尾23_鱗CP.Update();
    		X0Y0_尾23_鱗左CP.Update();
    		X0Y0_尾23_鱗右CP.Update();
    		X0Y0_尾23_尾CP.Update();
    		X0Y0_尾24_鱗CP.Update();
    		X0Y0_尾24_鱗左CP.Update();
    		X0Y0_尾24_鱗右CP.Update();
    		X0Y0_尾24_尾CP.Update();
    		X0Y0_尾25_鱗CP.Update();
    		X0Y0_尾25_鱗左CP.Update();
    		X0Y0_尾25_鱗右CP.Update();
    		X0Y0_尾25_尾CP.Update();
    		X0Y0_尾26_鱗CP.Update();
    		X0Y0_尾26_鱗左CP.Update();
    		X0Y0_尾26_鱗右CP.Update();
    		X0Y0_尾26_尾CP.Update();
    		X0Y0_輪1_革CP.Update();
    		X0Y0_輪1_金具1CP.Update();
    		X0Y0_輪1_金具2CP.Update();
    		X0Y0_輪1_金具3CP.Update();
    		X0Y0_輪1_金具左CP.Update();
    		X0Y0_輪1_金具右CP.Update();
    		X0Y0_尾27_鱗CP.Update();
    		X0Y0_尾27_鱗左CP.Update();
    		X0Y0_尾27_鱗右CP.Update();
    		X0Y0_尾27_尾CP.Update();
    		X0Y0_尾28_鱗CP.Update();
    		X0Y0_尾28_鱗左CP.Update();
    		X0Y0_尾28_鱗右CP.Update();
    		X0Y0_尾28_尾CP.Update();
    		X0Y0_尾29_鱗CP.Update();
    		X0Y0_尾29_鱗左CP.Update();
    		X0Y0_尾29_鱗右CP.Update();
    		X0Y0_尾29_尾CP.Update();
    		X0Y0_尾30_鱗CP.Update();
    		X0Y0_尾30_鱗左CP.Update();
    		X0Y0_尾30_鱗右CP.Update();
    		X0Y0_尾30_尾CP.Update();
    		X0Y0_尾31_鱗CP.Update();
    		X0Y0_尾31_鱗左CP.Update();
    		X0Y0_尾31_鱗右CP.Update();
    		X0Y0_尾31_尾CP.Update();
    		X0Y0_尾32_鱗CP.Update();
    		X0Y0_尾32_鱗左CP.Update();
    		X0Y0_尾32_鱗右CP.Update();
    		X0Y0_尾32_尾CP.Update();
    		X0Y0_尾33_鱗CP.Update();
    		X0Y0_尾33_鱗左CP.Update();
    		X0Y0_尾33_鱗右CP.Update();
    		X0Y0_尾33_尾CP.Update();
    		X0Y0_尾34_鱗CP.Update();
    		X0Y0_尾34_鱗左CP.Update();
    		X0Y0_尾34_鱗右CP.Update();
    		X0Y0_尾34_尾CP.Update();
    		X0Y0_尾35_鱗CP.Update();
    		X0Y0_尾35_鱗左CP.Update();
    		X0Y0_尾35_鱗右CP.Update();
    		X0Y0_尾35_尾CP.Update();
    		X0Y0_尾36_鱗CP.Update();
    		X0Y0_尾36_鱗左CP.Update();
    		X0Y0_尾36_鱗右CP.Update();
    		X0Y0_尾36_尾CP.Update();
    		X0Y0_尾37_鱗CP.Update();
    		X0Y0_尾37_鱗左CP.Update();
    		X0Y0_尾37_鱗右CP.Update();
    		X0Y0_尾37_尾CP.Update();
    		X0Y0_尾38_鱗CP.Update();
    		X0Y0_尾38_鱗左CP.Update();
    		X0Y0_尾38_鱗右CP.Update();
    		X0Y0_尾38_尾CP.Update();
    		X0Y0_尾39_鱗CP.Update();
    		X0Y0_尾39_鱗左CP.Update();
    		X0Y0_尾39_鱗右CP.Update();
    		X0Y0_尾39_尾CP.Update();
    		X0Y0_尾40_鱗CP.Update();
    		X0Y0_尾40_鱗左CP.Update();
    		X0Y0_尾40_鱗右CP.Update();
    		X0Y0_尾40_尾CP.Update();
    		X0Y0_尾41_鱗CP.Update();
    		X0Y0_尾41_鱗左CP.Update();
    		X0Y0_尾41_鱗右CP.Update();
    		X0Y0_尾41_尾CP.Update();
    		X0Y0_尾42_鱗CP.Update();
    		X0Y0_尾42_鱗左CP.Update();
    		X0Y0_尾42_鱗右CP.Update();
    		X0Y0_尾42_尾CP.Update();
    		X0Y0_尾43_鱗CP.Update();
    		X0Y0_尾43_鱗左CP.Update();
    		X0Y0_尾43_鱗右CP.Update();
    		X0Y0_尾43_尾CP.Update();
    		X0Y0_尾44_鱗CP.Update();
    		X0Y0_尾44_鱗左CP.Update();
    		X0Y0_尾44_鱗右CP.Update();
    		X0Y0_尾44_尾CP.Update();
    		X0Y0_頭_上顎_顎基CP.Update();
    		X0Y0_頭_上顎_鱗4CP.Update();
    		X0Y0_頭_上顎_鱗左1CP.Update();
    		X0Y0_頭_上顎_鱗右1CP.Update();
    		X0Y0_頭_上顎_鱗左2CP.Update();
    		X0Y0_頭_上顎_鱗右2CP.Update();
    		X0Y0_頭_上顎_鱗左3CP.Update();
    		X0Y0_頭_上顎_鱗右3CP.Update();
    		X0Y0_頭_上顎_鱗左4CP.Update();
    		X0Y0_頭_上顎_鱗右4CP.Update();
    		X0Y0_頭_上顎_鱗左5CP.Update();
    		X0Y0_頭_上顎_鱗右5CP.Update();
    		X0Y0_頭_上顎_鱗左8CP.Update();
    		X0Y0_頭_上顎_鱗右8CP.Update();
    		X0Y0_頭_上顎_鱗左9CP.Update();
    		X0Y0_頭_上顎_鱗右9CP.Update();
    		X0Y0_頭_上顎_鱗左10CP.Update();
    		X0Y0_頭_上顎_鱗右10CP.Update();
    		X0Y0_頭_上顎_鱗左11CP.Update();
    		X0Y0_頭_上顎_鱗右11CP.Update();
    		X0Y0_頭_下顎_顎基CP.Update();
    		X0Y0_頭_下顎_鱗1CP.Update();
    		X0Y0_頭_下顎_鱗2CP.Update();
    		X0Y0_頭_下顎_鱗3CP.Update();
    		X0Y0_頭_下顎_鱗4CP.Update();
    		X0Y0_頭_下顎_鱗左1CP.Update();
    		X0Y0_頭_下顎_鱗右1CP.Update();
    		X0Y0_頭_下顎_鱗左2CP.Update();
    		X0Y0_頭_下顎_鱗右2CP.Update();
    		X0Y0_頭_下顎_鱗左3CP.Update();
    		X0Y0_頭_下顎_鱗右3CP.Update();
    		X0Y0_頭_下顎_鱗左4CP.Update();
    		X0Y0_頭_下顎_鱗右4CP.Update();
    		X0Y0_頭_下顎_鱗左5CP.Update();
    		X0Y0_頭_下顎_鱗右5CP.Update();
    		X0Y0_頭_下顎_鱗左6CP.Update();
    		X0Y0_頭_下顎_鱗右6CP.Update();
    		X0Y0_頭_下顎_鱗左7CP.Update();
    		X0Y0_頭_下顎_鱗右7CP.Update();
    		X0Y0_頭_下顎_鱗左8CP.Update();
    		X0Y0_頭_下顎_鱗右8CP.Update();
    		X0Y0_頭_下顎_鱗左9CP.Update();
    		X0Y0_頭_下顎_鱗右9CP.Update();
    		X0Y0_頭_下顎_鱗左10CP.Update();
    		X0Y0_頭_下顎_鱗右10CP.Update();
    		X0Y0_頭_下顎_鱗左11CP.Update();
    		X0Y0_頭_下顎_鱗右11CP.Update();
    		X0Y0_輪2_革CP.Update();
    		X0Y0_輪2_金具1CP.Update();
    		X0Y0_輪2_金具2CP.Update();
    		X0Y0_輪2_金具3CP.Update();
    		X0Y0_輪2_金具左CP.Update();
    		X0Y0_輪2_金具右CP.Update();
    		鎖1.接続PA();
    		鎖2.接続PA();
    		鎖3.接続PA();
    		鎖4.接続PA();
    		鎖1.色更新();
    		鎖2.色更新();
    		鎖3.色更新();
    		鎖4.色更新();
    	}

    	private void 配色(BodyColorSet 体配色)
    	{
    		switch (配色指定)
    		{
    		case 配色指定.N0:
    			配色N0(体配色);
    			break;
    		case 配色指定.T0:
    			配色T0(体配色);
    			break;
    		case 配色指定.T1:
    			配色T1(体配色);
    			break;
    		default:
    			配色N0(体配色);
    			break;
    		}
    	}

    	private void 配色N0(BodyColorSet 体配色)
    	{
    		尾1_根CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0O);
    		尾1_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾1_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾1_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾1_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾2_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾2_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾2_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾2_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾3_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾3_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾3_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾3_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾4_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾4_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾4_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾4_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾5_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾5_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾5_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾5_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾6_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾6_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾6_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾6_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾7_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾7_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾7_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾7_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾8_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾8_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾8_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾8_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾9_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾9_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾9_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾9_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾10_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾10_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾10_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾10_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾11_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾11_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾11_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾11_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾12_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾12_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾12_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾12_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾13_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾13_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾13_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾13_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾14_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾14_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾14_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾14_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾15_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾15_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾15_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾15_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾16_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾16_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾16_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾16_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾17_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾17_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾17_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾17_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾18_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾18_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾18_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾18_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾19_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾19_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾19_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾19_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾20_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾20_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾20_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾20_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾21_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾21_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾21_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾21_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾22_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾22_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾22_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾22_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾23_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾23_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾23_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾23_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾24_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾24_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾24_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾24_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾25_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾25_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾25_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾25_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾26_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾26_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾26_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾26_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾27_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾27_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾27_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾27_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾28_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾28_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾28_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾28_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾29_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾29_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾29_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾29_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾30_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾30_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾30_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾30_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾31_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾31_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾31_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾31_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾32_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾32_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾32_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾32_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾33_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾33_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾33_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾33_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾34_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾34_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾34_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾34_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾35_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾35_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾35_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾35_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾36_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾36_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾36_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾36_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾37_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾37_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾37_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾37_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾38_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾38_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾38_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾38_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾39_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾39_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾39_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾39_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾40_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾40_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾40_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾40_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾41_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾41_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾41_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾41_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾42_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾42_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾42_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾42_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾43_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾43_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾43_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾43_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾44_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾44_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾44_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾44_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_顎基CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗4CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左1CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右1CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左2CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右2CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左3CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右3CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左4CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右4CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左5CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右5CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左8CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右8CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左9CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右9CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左10CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右10CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左11CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右11CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_下顎_顎基CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗1CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗2CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗3CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗4CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左1CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_下顎_鱗右1CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_下顎_鱗左2CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右2CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左3CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右3CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左4CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右4CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左5CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右5CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左6CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右6CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左7CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右7CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左8CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右8CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左9CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右9CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左10CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右10CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左11CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右11CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		輪1_革CD = new ColorD();
    		輪1_金具1CD = new ColorD();
    		輪1_金具2CD = new ColorD();
    		輪1_金具3CD = new ColorD();
    		輪1_金具左CD = new ColorD();
    		輪1_金具右CD = new ColorD();
    		輪2_革CD = new ColorD();
    		輪2_金具1CD = new ColorD();
    		輪2_金具2CD = new ColorD();
    		輪2_金具3CD = new ColorD();
    		輪2_金具左CD = new ColorD();
    		輪2_金具右CD = new ColorD();
    	}

    	private void 配色T0(BodyColorSet 体配色)
    	{
    		尾1_根CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0O);
    		尾1_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾1_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾1_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾1_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾2_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾2_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾2_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾2_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾3_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾3_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾3_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾3_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾4_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾4_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾4_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾4_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾5_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾5_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾5_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾5_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾6_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾6_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾6_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾6_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾7_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾7_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾7_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾7_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾8_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾8_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾8_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾8_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾9_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾9_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾9_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾9_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾10_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾10_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾10_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾10_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾11_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾11_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾11_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾11_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾12_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾12_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾12_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾12_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾13_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾13_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾13_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾13_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾14_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾14_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾14_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾14_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾15_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾15_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾15_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾15_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾16_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾16_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾16_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾16_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾17_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾17_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾17_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾17_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾18_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾18_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾18_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾18_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾19_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾19_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾19_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾19_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾20_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾20_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾20_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾20_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾21_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾21_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾21_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾21_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾22_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾22_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾22_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾22_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾23_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾23_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾23_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾23_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾24_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾24_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾24_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾24_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾25_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾25_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾25_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾25_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾26_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾26_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾26_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾26_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾27_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾27_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾27_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾27_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾28_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾28_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾28_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾28_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾29_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾29_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾29_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾29_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾30_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾30_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾30_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾30_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾31_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾31_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾31_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾31_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾32_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾32_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾32_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾32_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾33_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾33_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾33_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾33_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾34_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾34_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾34_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾34_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾35_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾35_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾35_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾35_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾36_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾36_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾36_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾36_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾37_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾37_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾37_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾37_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾38_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾38_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾38_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾38_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾39_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾39_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾39_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾39_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾40_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾40_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾40_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾40_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾41_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾41_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾41_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾41_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾42_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾42_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾42_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾42_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾43_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾43_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾43_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾43_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾44_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾44_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾44_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾44_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_顎基CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗4CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_上顎_鱗左1CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_上顎_鱗右1CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_上顎_鱗左2CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右2CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左3CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右3CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左4CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_上顎_鱗右4CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_上顎_鱗左5CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右5CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左8CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右8CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左9CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_上顎_鱗右9CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_上顎_鱗左10CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右10CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左11CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_上顎_鱗右11CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_下顎_顎基CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗1CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗2CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗3CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗4CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左1CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_下顎_鱗右1CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_下顎_鱗左2CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右2CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左3CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右3CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左4CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右4CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左5CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右5CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左6CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右6CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左7CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右7CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左8CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右8CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左9CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右9CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左10CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右10CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左11CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右11CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		輪1_革CD = new ColorD();
    		輪1_金具1CD = new ColorD();
    		輪1_金具2CD = new ColorD();
    		輪1_金具3CD = new ColorD();
    		輪1_金具左CD = new ColorD();
    		輪1_金具右CD = new ColorD();
    		輪2_革CD = new ColorD();
    		輪2_金具1CD = new ColorD();
    		輪2_金具2CD = new ColorD();
    		輪2_金具3CD = new ColorD();
    		輪2_金具左CD = new ColorD();
    		輪2_金具右CD = new ColorD();
    	}

    	private void 配色T1(BodyColorSet 体配色)
    	{
    		尾1_根CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0O);
    		尾1_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾1_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾1_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾1_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾2_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾2_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾2_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾2_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾3_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾3_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾3_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾3_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾4_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾4_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾4_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾4_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾5_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾5_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾5_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾5_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾6_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾6_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾6_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾6_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾7_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾7_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾7_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾7_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾8_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾8_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾8_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾8_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾9_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾9_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾9_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾9_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾10_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾10_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾10_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾10_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾11_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾11_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾11_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾11_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾12_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾12_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾12_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾12_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾13_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾13_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾13_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾13_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾14_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾14_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾14_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾14_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾15_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾15_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾15_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾15_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾16_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾16_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾16_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾16_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾17_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾17_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾17_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾17_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾18_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾18_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾18_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾18_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾19_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾19_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾19_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾19_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾20_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾20_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾20_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾20_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾21_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾21_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾21_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾21_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾22_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾22_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾22_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾22_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾23_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾23_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾23_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾23_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾24_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾24_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾24_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾24_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾25_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾25_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾25_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾25_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾26_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾26_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾26_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾26_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾27_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾27_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾27_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾27_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾28_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾28_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾28_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾28_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾29_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾29_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾29_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾29_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾30_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾30_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾30_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾30_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾31_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾31_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾31_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾31_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾32_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾32_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾32_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾32_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾33_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾33_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾33_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾33_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾34_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾34_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾34_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾34_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾35_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾35_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾35_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾35_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾36_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾36_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾36_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾36_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾37_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾37_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾37_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾37_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾38_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾38_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾38_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾38_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾39_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾39_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾39_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾39_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾40_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾40_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾40_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾40_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾41_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾41_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾41_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾41_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾42_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾42_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾42_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾42_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾43_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾43_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾43_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾43_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		尾44_鱗CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		尾44_鱗左CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾44_鱗右CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		尾44_尾CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_顎基CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗4CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左1CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右1CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左2CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右2CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左3CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_上顎_鱗右3CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_上顎_鱗左4CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右4CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左5CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_上顎_鱗右5CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_上顎_鱗左8CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_上顎_鱗右8CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_上顎_鱗左9CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右9CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗左10CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_上顎_鱗右10CD = new ColorD(ref ColorHelper.Black, ref 体配色.刺青O);
    		頭_上顎_鱗左11CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_上顎_鱗右11CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_下顎_顎基CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗1CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗2CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗3CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗4CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左1CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_下顎_鱗右1CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗0R);
    		頭_下顎_鱗左2CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右2CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左3CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右3CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左4CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右4CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左5CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右5CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左6CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右6CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左7CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右7CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左8CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右8CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左9CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右9CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左10CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右10CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗左11CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		頭_下顎_鱗右11CD = new ColorD(ref ColorHelper.Black, ref 体配色.鱗1R);
    		輪1_革CD = new ColorD();
    		輪1_金具1CD = new ColorD();
    		輪1_金具2CD = new ColorD();
    		輪1_金具3CD = new ColorD();
    		輪1_金具左CD = new ColorD();
    		輪1_金具右CD = new ColorD();
    		輪2_革CD = new ColorD();
    		輪2_金具1CD = new ColorD();
    		輪2_金具2CD = new ColorD();
    		輪2_金具3CD = new ColorD();
    		輪2_金具左CD = new ColorD();
    		輪2_金具右CD = new ColorD();
    	}

    	public void 輪1配色(拘束具色 配色)
    	{
    		輪1_革CD.色 = 配色.革部色;
    		輪1_金具1CD.色 = 配色.金具色;
    		輪1_金具2CD.色 = 輪1_金具1CD.色;
    		輪1_金具3CD.色 = 輪1_金具1CD.色;
    		輪1_金具左CD.色 = 輪1_金具1CD.色;
    		輪1_金具右CD.色 = 輪1_金具1CD.色;
    	}

    	public void 輪2配色(拘束具色 配色)
    	{
    		輪2_革CD.色 = 配色.革部色;
    		輪2_金具1CD.色 = 配色.金具色;
    		輪2_金具2CD.色 = 輪2_金具1CD.色;
    		輪2_金具3CD.色 = 輪2_金具1CD.色;
    		輪2_金具左CD.色 = 輪2_金具1CD.色;
    		輪2_金具右CD.色 = 輪2_金具1CD.色;
    	}

    	public void 鎖配色(鎖色 配色)
    	{
    		鎖1.配色鎖(配色);
    		鎖2.配色鎖(配色);
    		鎖3.配色鎖(配色);
    		鎖4.配色鎖(配色);
    	}
    }
}
